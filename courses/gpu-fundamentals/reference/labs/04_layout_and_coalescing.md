...
**Theory preparation:** Read Lessons 1 and 4 for timing, views, strides, materialization and logical read/write byte counts. The first run in Lesson 4 studies storage and accessed addresses. Revisit it after Lesson 7 to interpret coalescing, useful bandwidth and whether reuse repays a layout copy.
...
At Lesson 4, use this lab to identify shared storage versus new allocations and count the bytes read and written. Lesson 7 returns to the same experiment for transaction efficiency and repacking break-even analysis. You do not need that later performance interpretation to complete the first memory-hierarchy activity.
...
Use Lab 04 to identify the accessed addresses and build logical read/write byte counts. Preview Lab 05's inputs and repeated operations, but defer its full arithmetic-intensity and roofline interpretation until Lesson 10. Revisit effective bandwidth after coalescing in Lesson 7.
