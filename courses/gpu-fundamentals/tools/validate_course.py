...
def main() -> None:
    ...
    expected_header = (
        ...
        '<p class="course-hours"><strong>30 guided hours</strong></p>'
    )
    if not header_match or "".join(header_match.group(1).split()) != "".join(
        expected_header.split()
    ):
        fail("top banner must contain only the course title and 30 guided hours")
    ...
    reference_numbers = [
        int(number) for number in re.findall(r'<li id="ref-f(\d+)">', document)
    ]
    ...
    if any(
        plain_text(label) != f"F{target}"
        for target, label in re.findall(
            r'<a href="#ref-f(\d+)">(.*?)</a>', document, re.DOTALL
        )
    ):
    ...
    if parser.lesson_count != 15:
        fail(f"course needs exactly 15 detailed lessons, found {parser.lesson_count}")
    incomplete_lessons = [
        index + 1
    ...
    clean_environment = {**os.environ, "PYTHONDONTWRITEBYTECODE": "1"}
    for lab in lab_files:
        ...
        if completed.returncode != 0:
            fail(f"dependency-free --help failed: labs/{lab.name}")
    forbidden = (
        "/users/",
        "youtube.com",
        "youtu.be",
    ...
    layout_lab = (ROOT / "labs" / "04_layout_and_coalescing.py").read_text(
        encoding="utf-8"
    )
    layout_markers = (
        '"layouts"',
    ...
    if any(marker not in layout_lab for marker in layout_markers):
        fail("layout lab lacks layout, bandwidth, repack, or break-even evidence")
