#!/usr/bin/env python3
"""Validate three inference capstone trials and write a scoped decision."""
...
def load_trial(path: Path) -> dict[str, Any]:
    ...
    if payload.get("correctness", {}).get("output_allclose") is not True:
...
def main() -> None:
    ...
    contracts = [
        {
            ...
            "isl": trial["measurements"].get("isl"),
            "osl": trial["measurements"].get("osl"),
            "concurrency": trial["measurements"].get("concurrency"),
    ...
    if any(contract != contracts[0] for contract in contracts[1:]):
        raise SystemExit("Capstone profile, environment, workload, or options differ.")
    baseline = [
        positive_finite(
            trial["measurements"]["timing"]["materialized"]["median_ms"], "Trial median"
        )
        for trial in trials
    ]
    candidate = [
        positive_finite(
            trial["measurements"]["timing"]["sdpa"]["median_ms"], "Trial median"
        )
        for trial in trials
    ]
    ...
    write_exclusive(
        args.output,
        {
            ...
            "materialized_trial_medians_ms": baseline,
            "sdpa_trial_medians_ms": candidate,
            "materialized_median_of_medians_ms": round(baseline_median, 4),
            "sdpa_median_of_medians_ms": round(candidate_median, 4),
            ...
            "correctness": {
                ...
                "shared_profile_environment_workload_and_options": True,
                "all_outputs_equivalent": True,
            },
        },
    )
    print(f"Wrote scoped inference capstone decision: {args.output}")
