"""Shared, dependency-light helpers for the LLM training and inference labs."""
...
def require_hf_commit_revision(value: str, *, option: str = "--revision") -> None:
    """Reject mutable branches and tags in reproducibility-critical labs."""
    if re.fullmatch(r"[0-9a-f]{40}", value) is None:
        raise SystemExit(
            f"{option} must be an immutable 40-character Hugging Face commit ID"
        )
...
def load_torch() -> Any:
    try:
        ...
        raise SystemExit(
            "PyTorch is required; activate the training environment."
        ) from exc
    return torch
...
def sgd_updates_match(
    ...
    *,
    ...
    """Check gradient agreement and each plain-SGD update at its actual scale."""
    changed = False
    with torch.no_grad():
        for before, reference, observed in zip(
            initial, baseline, candidate, strict=True
        ):
            if reference.grad is None or observed.grad is None:
                return False
            if not all(
                ...
                return False
            scale = float(reference.grad.abs().max())
            if scale == 0:
                if not torch.equal(reference.grad, observed.grad):
                    return False
            elif not torch.allclose(
                observed.grad.float() / scale,
                reference.grad.float() / scale,
                rtol=1e-2,
                atol=1e-2,
            ):
                return False
            for parameter in (reference, observed):
                expected = before.clone().add_(parameter.grad, alpha=-learning_rate)
                if not torch.equal(parameter, expected):
                    return False
            changed = changed or not torch.equal(before, reference)
    return changed
