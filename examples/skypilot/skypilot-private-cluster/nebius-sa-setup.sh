#!/bin/bash
# nebius-sa-setup.sh: Create a Nebius service account and configure global AWS profiles for Nebius S3
# Usage: ./nebius-sa-setup.sh
set -euo pipefail  # Fail fast: exit on error, unset vars, and pipeline failures

HEADER="# Auto-generated by nebius-sa-setup.sh — created locally; review and update values as needed"
if [ ! -f .env ]; then
  if [ -f .env.placeholder ]; then
    { echo "$HEADER"; cat .env.placeholder; } > .env
    echo "Created .env from .env.placeholder. Please review and update values if needed."
  else
    echo "Error: .env not found and .env.placeholder missing. Please add them to continue."
    exit 1
  fi
fi
...
echo "$TENANT_ID" > "$CREDENTIALS_DIR/NEBIUS_TENANT_ID.txt"
echo "Saved TENANT_ID to $CREDENTIALS_DIR/NEBIUS_TENANT_ID.txt"
...
TMP_CREDENTIALS_FILE="$(mktemp "${CREDENTIALS_DIR}/credentials.json.tmp.XXXXXX")"
trap 'rm -f "$TMP_CREDENTIALS_FILE"' EXIT

nebius iam auth-public-key generate \
  --parent-id "$PROJECT_ID" \
  --service-account-id "$SA_ID" \
  --output-format service-account-json \
  --output "$TMP_CREDENTIALS_FILE"

# Validate generated credentials before replacing the active credentials file.
if ! jq empty "$TMP_CREDENTIALS_FILE" >/dev/null 2>&1; then
  echo "Error: Generated credentials at $TMP_CREDENTIALS_FILE are not valid JSON; cannot enable compute." >&2
  exit 1
fi
chmod 600 "$TMP_CREDENTIALS_FILE" || true
mv "$TMP_CREDENTIALS_FILE" "$CREDENTIALS_FILE"
trap - EXIT
