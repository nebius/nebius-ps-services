locals {
  ...
  data_disk_specs = {
    for disk in var.data_disks : disk.name => {
      ...
      encryption_enabled  = try(disk.encryption_enabled, false)
      deletion_protection = try(disk.deletion_protection, false)
  ...
  cloud_init_user_data = var.cloud_init_user_data_override != null ? var.cloud_init_user_data_override : templatefile("${path.module}/vm-cloud-init.tftpl", {
