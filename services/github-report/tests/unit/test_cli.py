...
def test_top_users_command_renders_csv() -> None:
    ...
    with patch(
        ...
        result = runner.invoke(
            app,
            ["top-users", "--since", "2026-03-01", "--format", "csv"],
        )

    assert result.exit_code == 0
    assert "rank,user_name,num_modifications,num_commits,repos" in result.stdout
    assert "1,Alice Example (@alice),15,5,nebius/pysdk" in result.stdout
...
def test_top_users_per_repo_renders_repo_breakdown_csv() -> None:
    ...
    with patch(
        ...
        result = runner.invoke(
            app,
            ["top-users", "--per-repo", "--since", "2026-03-01", "--format", "csv"],
        )

    assert result.exit_code == 0
    assert "rank,user_name,repo_name,num_modifications,num_commits" in result.stdout
    assert "1,Alice Example (@alice),nebius/pysdk,15,5" in result.stdout


def test_root_org_option_flows_into_subcommands() -> None:
    runner = CliRunner()
    ...
    with patch("github_report.cli.service.build_report", new=mock_build_report):
        result = runner.invoke(
            app,
            ["--org", "acme", "top-users", "--since", "2026-03-01", "--format", "csv"],
        )

    assert result.exit_code == 0
    assert mock_build_report.await_args.args[0].org == "acme"
    ...
    with patch("github_report.cli.service.build_report", new=mock_build_report):
        result = runner.invoke(
            ...
            [
                ...
                "2026-03-01",
                "--exclude",
                "csa-soperator-deployments, nebius/api",
                "--format",
                "csv",
            ],
        )
...
def test_top_users_command_enters_spinner_context() -> None:
    ...
    assert (
        ...
        == "Collecting GitHub activity for nebius (all accessible repos)..."
    )
...
def test_top_users_spinner_mentions_excluded_repositories() -> None:
    ...
    assert (
        mock_status.call_args.args[0]
        == "Collecting GitHub activity for nebius (all accessible repos, excluding 1 repo)..."
    )
...
def test_top_users_output_csv_infers_format_from_extension(tmp_path) -> None:
    ...
    with patch(
        ...
        result = runner.invoke(
            app,
            ["top-users", "--since", "2026-03-01", "--output", str(output_path)],
        )
...
def test_list_repos_output_csv_infers_format_from_extension(tmp_path) -> None:
    ...
    repositories = [
        RepositoryRef(
            name="pysdk",
            full_name="nebius/pysdk",
            default_branch="main",
            is_archived=False,
        ),
    ]
    with patch("github_report.cli.service.list_repositories", return_value=repositories):
        result = runner.invoke(app, ["list-repos", "--output", str(output_path)])
...
def test_top_users_output_html_infers_format_from_extension(tmp_path) -> None:
    ...
    assert "<h1>Top Contributors for nebius</h1>" in html
...
def test_top_users_output_text_infers_format_from_extension(tmp_path) -> None:
    ...
    assert "Top Contributors for nebius" in text_output
...
def test_top_users_output_overwrites_existing_file(tmp_path) -> None:
    ...
    assert text_output.startswith("Top Contributors for nebius")
...
def test_top_users_explicit_format_overrides_output_extension(tmp_path) -> None:
    ...
    assert markdown_output.startswith("# Top Contributors for nebius")
...
def test_list_repos_output_text_infers_format_from_extension(tmp_path) -> None:
    ...
    assert result.exit_code == 0
    text_output = output_path.read_text(encoding="utf-8")
    assert "Accessible Repositories for nebius" in text_output
    assert "Repositories" in text_output
    assert " 1. nebius/pysdk" in text_output
...
def test_list_repos_output_html_infers_format_from_extension(tmp_path) -> None:
    ...
    assert "<h1>Accessible Repositories for nebius</h1>" in html_output
...
def test_list_repos_command_renders_markdown() -> None:
    ...
    assert result.exit_code == 0
    assert "# Accessible Repositories for nebius" in result.stdout
    assert "nebius/pysdk" in result.stdout
