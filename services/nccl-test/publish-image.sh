...
show_usage() {
  ...
  printf '%b\n' "  ${S_CYAN}./publish-image.sh${S_RESET} ${S_DIM}--publish X.Y.Z [--allow-non-main]${S_RESET}"
  printf '\n'

  printf '%b\n' "${S_BOLD}Modes:${S_RESET}"
  printf '%b\n' "  ${S_YELLOW}--prep${S_RESET}     Update ${CHANGELOG_FILE}, commit it, and push current branch (default)."
  ...
  printf '%b\n' "  ${S_CYAN}./publish-image.sh --prep 0.2.0${S_RESET}"
  printf '%b\n' "  ${S_CYAN}./publish-image.sh --publish 0.2.0${S_RESET}"
  printf '\n'
  printf '%b\n' "${S_DIM}Notes:${S_RESET}"
...
main() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --prep)
        [[ -n "${mode}" ]] && { log_error "Choose only one of --prep or --publish."; exit 1; }
        mode="prep"
        shift
        [[ $# -gt 0 ]] || { log_error "--prep requires X.Y.Z"; exit 1; }
        raw_version="$1"
        ;;
      --publish)
        [[ -n "${mode}" ]] && { log_error "Choose only one of --prep or --publish."; exit 1; }
        mode="publish"
        shift
        [[ $# -gt 0 ]] || { log_error "--publish requires X.Y.Z"; exit 1; }
        raw_version="$1"
        ;;
      --allow-non-main)
        allow_non_main=1
  ...
  case "${mode}" in
    prep)
      ...
      prep_release "${tag}" "$(( no_push == 0 ))" "${current_branch:-HEAD}"
      ;;
    publish)
      ...
      if [[ "${allow_non_main}" -eq 1 ]]; then
        ...
        if [[ "${current_branch}" != "${MAIN_BRANCH}" ]]; then
          log_error "--publish must run on ${MAIN_BRANCH}; current branch is '${current_branch:-detached}'"
          exit 1
        fi
