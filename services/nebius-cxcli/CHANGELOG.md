...
- Resume target accounting metrics reconciliation when the MariaDB Operator
  canonically omits its optional disabled `metrics` block or `enabled` field.
  cxcli adopts only omission or explicit false as disabled, preserves the
  complete remaining child spec in its replay fingerprint, and still rejects
  present non-boolean or non-mapping state before checkpoint or Kubernetes
  mutation.
- Resume the exact protected login restore when Helm has deployed its configured
  replica successor but the process stopped before the restore-owner receipt was
  sealed. The active Jail rebind now re-proves the role-repair predecessor,
  protected release history and manifests, stored values, live target spec,
  replica-only transition, and two-step generation chain without checkpoint
  mutation; direct one-generation restores and all unrelated drift remain
  fail-closed.
- Resume the protected login restore after Helm has already returned the
  SlurmCluster owner to its configured replica count while the workload remains
  surged. The root/head predecessor now uses the canonical Helm-to-live
  normalization for only the two cxcli-owned null shadow-login annotation
  tombstones, while retaining raw revision hashes and rejecting unrelated null
  or non-login drift before checkpoint or Kubernetes mutation.
- Recover an exact post-switch login Service role-repair surge after a resume
  regresses its already released reconciliation to a later `applied`
  observation. cxcli now binds the v2 surge contract, target, verified role
  repair, canonical and nested hold, target-slot peers, release gate, replica
  observation, and complete chronology in one semantic-hash receipt before
  restoring `released`; replay is write-free and unrelated drift remains
  blocked before Helm, Kubernetes, or Slurm mutation.
- Resume an exact verified login Munge runtime after its controller replaces
  one ordinal or restarts the Munge container. cxcli now seals a bounded,
  fingerprinted predecessor-to-successor chain only after re-proving the
  workload UID, selector, target Secret CAS, Ready endpoints, node continuity,
  chronology, and two-controller Slurm authentication; unrelated runtime,
  owner, Secret, node, or lineage drift remains recovery-required without Pod
  deletion.
- Restore live MK8s node-group rows in upgrade status with Nebius SDK 0.4+
  direct-message ProtoJSON conversion. Unsupported SDK objects now fail fast
  instead of silently becoming empty mappings, and status polling prefers the
  locked checkpoint cluster ID while rejecting conflicting phase-local
  identity.
- Resume external first-adoption Jail migration after the checkpoint-scoped
  SlurmCluster identity advances to the target whose roles use the active slot.
  cxcli now keeps the immutable legacy PVC name and UID as source-copy and
  rollback authority instead of comparing the target slot with historical
  source storage. Source-role conflicts, incomplete or overlapping slot
  bindings, backup drift, and replaced PVC UIDs remain fail-closed.
- Resume external Soperator source cleanup after the exact source
  SlurmCluster deletion lands but before the terminal cleanup proof is sealed.
  cxcli now separates the immutable checkpointed source cleanup authority from
  the active target observation and accepts only the schema-valid
  source-cleanup scope whose target role and source/target namespace, name, and
  UID bindings exactly match the rolling handoff. Raw target-only discovery,
  malformed scope, reappeared source, foreign UID, and unrelated identity drift
  remain fail-closed before Kubernetes, Slurm, or provider mutation.
- Resume a deferred external Soperator discovery bundle after the exact
  checkpoint-bound SlurmCluster handoff. cxcli now reapplies the strict
  source/target identity scope in memory before locked-campaign validation,
  preserves the immutable source UID for campaign comparison, and routes
  execution through the selected target successor after source cleanup. The
  cached bundle is not rewritten, and foreign, recreated, ambiguous, or
  mode-incompatible UIDs still fail before provider or Kubernetes mutation.
- Repair renewable Nebius SDK handoff and exact protected-login resume. cxcli
  now starts a caller-owned SDK event loop before constructing supported 0.5.x
  SDKs, submits synchronous work and close operations thread-safely, and cleans
  up failed constructors. Endpoint overrides now use the SDK's `domain`
  parameter, and CLI failures retain their underlying cause chain.
  Post-switch replay also admits only the sealed
  login-role Helm/Jail successor and the exact hold-bound pre-CAS login Pod
  whose legacy Munge Secret projection must be replaced serially; current
  successors still require the target Secret and unrelated identity, workload,
  node, container, restart, chronology, or projection drift remains fail-closed.
- Recover a completed protected post-switch login surge whose historical Helm
  values still give the target login Service the preserved `LoadBalancer` role.
  cxcli now classifies only the exact checkpoint-bound transition to the target
  `NodePort` shadow, seals its predecessor and intent before a protected Helm
  reapply, proves the compatible successor, and keeps all unrelated semantic or
  OpenMetrics drift fail-closed. The classifier now accepts the historical
  `LoadBalancer` CRD default `sshdServiceNodePort: 0` only when the desired
  `NodePort` is positive and the checkpointed allocation, annotation tombstones,
  source/target identities, and every other target-spec field remain exact. The
  surge now also runs the existing serial
  MUNGE runtime convergence before declaring itself applied, so Kubernetes-Ready
  login Pods that predate the target Secret CAS cannot mask failed Slurm
  authentication.
- Resume an exact legacy-rootfs Jail-boundary Deployment successor through the
  protected accounting-restore path and the ordinary existing active-slot
  rebind. Both paths now seal or replay the same strict bridge-client source
  receipt as the login-restore and writer-release consumers only after
  re-proving the live ConfigMap UID and exact target controller owner, then
  accept the verified same-UID, same-spec `0 -> 1 -> 0` writer pulse. The
  ordinary source read is limited to that exact later-successor state; original
  boundary-zero and non-active rebinds remain source-free, while missing or
  drifted receipt, generation, replica, UID, owner, content, data, or spec
  evidence remains fail-closed.
- Preserve that ordinary rebind's newly sealed bridge-client source when the
  already accepted same-digest canonical handoff becomes verified afterward.
  cxcli now recognizes only the exact accepted-handoff fingerprint and ordered
  compatibility, writer, rollout, source-receipt, and handoff-verification
  timestamps, then seals the existing semantic-hash source successor. Unchanged
  rollout evidence is admitted only for this late-source chronology; reordered
  or fingerprint-drifted state remains pending without a successor write.
- Derive external Soperator resume from the checkpoint's immutable ordered
  phase plan and exact completed prefix, use that locked order in reports, and
  invalidate a drifted phase's dependent suffix before redispatch. CLI entry,
  executor preflight, and backup recovery now share one mutation-boundary
  classifier, including accepted controller-bridge provider operations. During
  inert-controller worker recovery, cxcli seals a missing exact controller-gap
  semantic binding before its partition-pause consumer and reuses checkpointed
  `DOWN` records without a Slurm RPC; incomplete evidence remains fail-closed.
- Resume the active-slot Jail boundary after cxcli's own completed protected
  `post-switch-resume` Helm dispatch advances the target beyond the exact
  accounting-rebound writer fence. A single same-campaign classifier now binds
  source retirement, rearmed manager-pause reuse, protected root-to-head Helm
  history, stored values, installed manifest, defaulted live SlurmCluster spec,
  UID, generation, resource version, Jail, submount, maintenance, and
  OpenMetrics state before sealing one shared post-Jail parent receipt. Exact
  replay is write-free; partial, reordered, foreign, or drifted lineage remains
  recovery-required without a field-specific compatibility allowance.
- Resume source-HA controller-bridge health after the legacy-rootfs handoff has
  intentionally retired the historical source JailedConfig. cxcli now reuses
  the immutable all-file repair contract only when the exact source identity,
  complete item mapping, source/target rebind, target successor, UID/resource
  version delete receipts, post-retirement source absence, and chronology all
  match; it still proves the live bridge ConfigMap, Pods, every file digest,
  roles, and empty queue. Partial, foreign, or drifted retirement evidence
  remains recovery-required before checkpoint or infrastructure mutation.
- Resume `post-switch-resume` from the exact verified target-manager rearm that
  active service fencing may produce after accounting and target handoff have
  completed. The existing strict rearm contract, live zero-manager proof,
  target binding, operation authority, chronology, and semantic-hash receipt
  now cover post-switch as the first ordered rearm-reuse boundary; drift still
  fails before webhook publication, Helm dispatch, or checkpoint mutation.
- Resume post-cleanup rolling-values reconciliation after a completed dual-CR
  login handoff promotes the exact target Service from its sealed `NodePort`
  shadow role to the final `LoadBalancer`. cxcli now replaces only the archived
  preserved-source allocation with the current allocation on that same target
  Service UID after validating the completed campaign identity transition,
  archived role receipt, and source allocation decision. Later value replays
  retain the target allocation; foreign Service, receipt, transition, or
  allocation drift remains fail-closed.
- Treat an exact still-live archived source login LoadBalancer as verified
  superseded evidence once the target Service successor is proven. Allocation
  stabilization now persists only the target allocation, produces the same
  values regardless of Service-list order, and rejects any extra chart owner.
- Continue large external in-place upgrades past a worker group that is already
  missing, non-Ready, or safely unavailable in Slurm before provider dispatch.
  cxcli now seals a campaign/group/template/strategy-bound readiness receipt,
  deduplicates Kubernetes and Slurm symptoms with an explicit unidentified
  fixed-size deficit, charges that outage against the global unavailable
  ceiling without consuming a provider slot, and admits only work that still
  fits the concurrency and availability budgets. Unsafe jobs, epilog or alias
  ambiguity, unhealthy shared authority, compensation work, and provider
  identity drift remain recovery-required. Owner-only reports identify exact
  Kubernetes/Slurm nodes where provable and mark completion partial; the same
  approved command resolves a recovered receipt and processes only unfinished
  groups. Capacity isolation now uses failure domains and reservation lanes,
  so `AUTO` overlaps common and reservation capacity. Durable group journals
  preserve rolling mutation truth even when a worker returns pending. The
  checkpoint and report schemas advance to v7; an exact valid v6 checkpoint is
  converted once under the checkpoint lock with a hash-sealed migration receipt.
  Every selected wave now rendezvous after a fresh full provider/Kubernetes/Slurm
  recheck, and active work is charged by the greater of its observed outage and
  configured allowance, preventing a newly degraded peer from crossing the
  dispatch barrier. The immutable readiness trigger retains its first census
  while a hash-bound latest observation refreshes exact affected-node reporting.
  Exact capacity errors older than the current provider request are treated as
  historical evidence instead of aborting the newer accepted operation.
- Continue safe independent worker node groups when one exact accepted initial
  update is waiting for Nebius capacity. cxcli keeps that provider operation
  active, checkpoints a sanitized immutable capacity-domain receipt, and
  rebinds every active receipt to the current campaign, segment, group,
  operation, resource, request, and locked domain before scheduler or receipt
  writes. Self-consistent foreign receipts fail recovery-required before they
  can affect admission. cxcli suppresses new overlapping-domain work and lazily
  uses only the remaining concurrency and unavailability budget. A healthy
  disjoint group seals and releases its
  own Jail, mount, Slurm, and GPU-runtime health boundary; the rolling phase,
  fleet health, cutover, cleanup, and campaign success remain pending until the
  blocked operation succeeds. Replay reconciles that exact operation and
  mutates only unfinished groups; terminal failure, service-role replacement,
  strategy restoration, quota preflight, identity drift, and every inexact
  capacity signal remain fail-closed without retry or operation replacement.
- Reconcile an exact accepted worker node-group provider operation before
  waiting for replacement worker workloads that the operation has temporarily
  made unavailable. Replay reuses only the sealed pre-dispatch paused-manager
  workload identities after revalidating the live SlurmCluster, manager pause,
  and worker values; initial dispatch and post-provider health still require
  live readiness, and all receipt or identity drift remains fail-closed.
- Fail promptly when an exact non-terminal MK8s node-group operation exposes a
  structured Nebius `ServiceError` with `retry_type=NOTHING`, except for the
  bounded initial-worker capacity continuation above. A later warning-only
  fallout event cannot erase the fatal proof, while arbitrary messages,
  retryable events, stale pre-request events, and inexact resource evidence
  remain on the strict continuation or recovery path.
- Admit a verified login Pod's exact later-segment topology config successor
  without deleting the unchanged Pod. The recovery requires the sealed
  predecessor-to-target receipt, deployed apply authority, current handoff,
  timeout-only semantic restoration, unchanged Pod UID, and bounded live jailed
  config convergence. Interrupted per-Pod progress retains the predecessor
  aggregate and accepts only exact UID-bound, chronologically valid mixed
  records; every other cross-material journal remains fail-closed.
- Preserve the sealed predecessor of that later-segment topology successor after
  exact login and direct propagation reconciliation. Resume now recognizes only
  the checkpointed predecessor-to-target config-reconciliation event, matching
  current consumer identities, target digest and semantics, and causal
  timestamps. A later successful live RPC observation that refreshes only
  ping/show-config evidence is represented by one semantic-hash-sealed receipt
  bound to the unique final event, exact topology successor, and stable consumer
  identity projection; later exact observations replay write-free. Missing,
  duplicate, partial, foreign, identity-drifted, or malformed receipts still fail
  closed before infrastructure mutation.
- Rebuild the complete all-node controller census after a narrowly classified,
  temporary admission-webhook transport outage during checkpointed node
  replacement. cxcli first deletes and proves absence for every exact inspector
  Pod from the failed attempt, then rediscovers current Pod and Node identities
  under one bounded deadline. Webhook denials, policy failures, partial evidence,
  and unsupported API errors remain fail-closed without authority mutation.
- Pin the cluster-visible external-upgrade Lease to the exact `KUBECONFIG`
  environment captured at lease construction. Nested discovery/provider
  kubeconfig scopes can no longer redirect foreground fences or the renewal
  thread to another file and falsely report a lost lease; genuine holder or
  access failures remain fail-closed.
- Retire the exact Helm-kept source `NodeConfigurator` orphan during final
  validation when its source Helm release, Flux desired state, and referenced
  ServiceAccount are all absent. The approved workflow checkpoints the exact
  custom-resource and owned DaemonSet identities and specs before a
  UID/resourceVersion-preconditioned foreground delete, proves both resources
  absent, and replays write-free. Desired state, a live ServiceAccount,
  replacement identity, foreign ownership, or receipt drift remains
  fail-closed; cxcli never recreates the deleted source ServiceAccount as a
  workaround.
- Materialize every file in the source controller's active `JailedConfig`
  before accepting source-version bridge authority. cxcli now seals the ordered
  ConfigMap item/path/digest contract, uses per-file PVC compare-and-set staging,
  and proves the complete file set on both bridge controllers. An interrupted
  source-HA checkpoint with the former slurm.conf-only receipt stops and fences
  only its exact bridge writers, stages the missing files, restarts them, and
  checkpoints an exact successor takeover for the replacement of the original
  active backup instead of waiting for Slurm's configured controller timeout.
  A crash after dispatch adopts only that replacement Pod's primary marker
  without dispatching again. The terminal receipt binds the accepted takeover,
  unique HA roles, empty queue, and controller logs free of the cloud-node
  AutoDetect error before provider or worker continuation. The old receipt
  remains immutable and exact terminal replay is write-free. Partition-pause
  replay now recognizes the exact all-file restart-intent as a bounded
  no-authority interval and reuses only the complete checkpointed DOWN records
  until successor takeover; writer, Pod, role, file, staging, pause, or
  chronology drift still refuses that path.
- Materialize a node-specific `AutoDetect=off` GPU mapping when a target
  NodeSet's `gresConfig` is absent, empty, retains the old autodetect value, or
  equals revision 29/30's exact generated explicit mapping without the override.
  Rolling-values revision 31 preserves a non-matching customer GPU definition,
  rejects malformed or duplicate mappings before mutation, and advances the
  post-OpenMetrics topology only through the exact sparse-generation Helm proof
  chain. The approved workflow also checkpoint-normalizes the generated
  ConfigMap by removing only the legacy main-file PluginDir and defunct
  AccountingStorageUser while retaining the canonical multiarch PluginDir;
  its historical aggregate proof reverses both the exact GRES and custom
  PluginDir successors before comparison. Downstream bridge-client source
  revalidation now consumes that normalized aggregate only through the exact
  predecessor, normalization, nested topology, and final-runtime receipt chain.
  The verified normalizer also replays after topology promotion by reproducing
  its immutable predecessor from the final-runtime receipt's sealed
  `previous_replay`, never by comparing it directly with the later revision.
  The outer post-OpenMetrics receipt now validates its older guarded-values
  successor against that same exact sealed predecessor, while the independently
  exact final-runtime receipt alone proves promotion to the current revision.
  Verified dual-CR login Service cleanup and roles receipts likewise retain
  their historical values proof only through that exact sealed topology lineage
  while all live Service, endpoint, target, and current Helm checks stay strict.
  Exact replay is write-free and every other drift remains fail-closed.
- Restore an exact terminal Jail predecessor before classifying post-cleanup
  mutation boundaries when a newer rolling-values contract requires Helm
  reconciliation. Completed retirement, cleaned bridge, sealed successful Job,
  handoff, smoke, and writer-quiescence evidence are required; historical copy
  artifacts no longer suppress the guarded values replay or retry retired
  slot-preparation infrastructure.
- Revalidate an already sealed post-values worker successor when the immutable
  historical-delete consumer is replayed. The adapter accepts only the exact
  typed-GRES, optional completed-rollout, final-runtime, and post-values receipt
  chain after rerunning live worker identity/config proof. An exact later
  final-runtime normalization may supersede only the receipt's original
  post-OpenMetrics and topology fingerprints while all worker, owner, revision,
  container, Helm, history, config, timestamp, and semantic-hash fields remain
  exact. Older guarded-values authority is reproduced from the final-runtime
  receipt's sealed predecessor rather than compared directly with the later
  normalized revision. The live jailed-file read uses the exact current
  normalized digest while the immutable receipt keeps its historical digest.
  Handoff replay is checkpoint-write-free; history branches, reordering, UID
  drift, aggregate drift outside that sealed successor, or receipt drift fail
  before Pod mutation.
- Keep a verified target MariaDB metrics repair receipt immutable across later
  exact rolling-values revisions. Replay now derives the receipt's historical
  target generation only from its exact Helm-proof entry in the recursively
  sealed guarded-values chain while independently retaining the current target
  proof; ambiguous or drifted history remains fail-closed and write-free.
- Accept an exact worker Pod recreated by a later checkpointed target-values
  Helm apply after immutable-child handoff and final-runtime reload. The new
  fail-closed successor binds the complete runtime lineage, live
  StatefulSet/NodeSet/ControllerRevision and Pod identities, the unique causal
  Helm proof, and the immutable post-OpenMetrics ConfigMap/topology digest
  before checkpointing the new UID; it never weakens the existing worker
  delete or serial-rollout fences.
- Preserve the immutable completed Slurm restore when a guarded post-cleanup
  values replay has checkpointed a second target-only pause. The restore
  adapter now proves the exact phase/global overlay, sealed target record,
  retired-source remainder, terminal bridge stage, and target-singleton
  reassertion producer order before the existing post-cleanup reconciler
  restores it. The release path reuses the exact historical action and
  preservation receipt while retaining current-health checks for destructive
  cleanup and fresh terminal validation; malformed or foreign overlays remain
  fail-closed.
- Replace the cloud-incompatible `AutoDetect=nvidia` target-worker default with
  an explicit GPU device mapping derived from the proven static type and count,
  because an empty NodeSet `gresConfig` lets SConfig recreate the default.
  Preserve other GRES directives, fail closed without an exact count, update
  the component defaults, and advance the guarded values revision so existing
  cutovers receive the fix. The approved worker-runtime reconciler now
  also clears only the exact stale `Extra=InstanceId={{ .PodName }}` state that
  slurmctld can retain after its source value is removed; terminal validation
  parses and rejects that legacy Extra while preserving any other customer
  value.
- Consume a guarded rolling-values Helm proof when it is strictly newer than a
  sealed post-singleton values receipt, rather than letting the historical
  receipt shadow current exact target/spec/values/manifest evidence. A verified
  GPU-sysfs reuse path now first advances an existing post-OpenMetrics
  downstream receipt through that exact topology proof and archives its
  predecessor, preventing an early return from leaving Helm revision lineage
  stale. Malformed or unpromoted newer claims remain fail-closed.
- Advance a sealed post-OpenMetrics nested topology receipt across one or more
  contiguous guarded rolling-values revisions when every intent-bound Helm
  proof remains exact. The adapter reconstructs and seals each intermediate
  replay/proof/generation edge, requires strictly advancing Helm revisions and
  chronology, embeds any predecessor receipt, and preserves the same target and
  ConfigMap projection. An embedded receipt carried through one exact
  next-revision, same-generation Helm replay is now validated against the replay
  it sealed plus the separately advancing outer proof, allowing the following
  generation-changing successor without weakening drift rejection. A nested
  receipt chain now recovers its immutable outer target generation from only
  the oldest exact predecessor instead of forgetting it after a later edge,
  including the worker-successor ConfigMap proof consumer. That consumer also
  requires the producer's terminal jailed-file digest timestamp with exact
  chronology instead of rejecting that valid field as schema drift.
  The historical outer OpenMetrics receipt remains immutable, and exact replay
  reproduces the complete chain without another write.
- Default target MariaDB accounting metrics off when values do not explicitly
  opt in, avoiding repeated ServiceMonitor reconciliation warnings on clusters
  without that CRD while preserving an explicit enabled setting. When
  Soperator retains the old enabled value on its owned MariaDB child despite
  the exact current target value, the approved workflow now checkpoints its
  Helm/target and UID/owner/spec identity, patches only that boolean with a
  resourceVersion CAS, and proves the single-generation stable-spec successor.
- Preserve the immutable bridge-client ConfigMap source when a later exact
  post-OpenMetrics Helm payload is already sealed. The consumer now accepts the
  live successor only when a read-only replay of the strict binder reproduces
  the checkpointed target/Helm/topology and ConfigMap proof byte-for-byte; it
  never rewrites the historical source or its successor chain.
- Keep the preserved source login Service as the sole LoadBalancer during a
  dual-CR handoff and render the target shadow Service as `NodePort`, the
  non-LoadBalancer type accepted by Soperator. Rolling-values revision 26
  supersedes revision 25 with explicit JSON-merge null tombstones for the two
  cxcli-owned annotations, and revision 27 reapplies that checkpoint-bound
  target projection after allocation stabilization so its generic same-CR
  values write cannot overwrite the tombstones before Helm. This removes
  live-only target allocation state through the guarded Helm path while
  normalizing only those exact deletion markers; release seals exact Service
  identities/owners, selectors,
  ingress/allocation, and equal Ready endpoint Pod UIDs before accepting the
  split role. If the Soperator controller still retains the exact preserved
  allocation metadata on the target NodePort shadow, the checkpointed role
  reconciler now journals and UID/resourceVersion/type/value-CAS removes only
  those copied annotations, verifies the complete shadow role, resumes safely
  after an interrupted dispatch, and rejects foreign values or reappearance.
  The downstream continuity guard now scopes load-balancer metadata stability
  to checkpointed LoadBalancer Services while keeping every retained UID and
  ClusterIP strict, so that exact NodePort cleanup is not misclassified as
  public-route drift.
- Include MariaDB `Database`, `User`, and `Grant` children in source-retirement
  inventory. Existing checkpoints may run one exact successor cleanup with
  UID/resourceVersion-fenced deletes and a compare-and-swap removal of only
  known MariaDB-operator finalizers after complete identity/spec proof.
- Continue an exact target accounting Deployment rollout after a guarded values
  reapply instead of treating normal image-pull/readiness latency as writer
  drift. cxcli now proves the target and Deployment UIDs, owner, selector,
  ReplicaSet-owned Pods, and exact SlurmDBD command/args first, then uses the
  shared bounded checkpoint continuation only for observed-generation, replica,
  phase, deletion, and Ready convergence. Foreign identity or configuration
  changes still fail immediately without infrastructure or checkpoint mutation.
- Preserve a completed final-partition restore receipt when later Slurm phase
  observations change only non-owned aggregate fields. cxcli now validates the
  sealed target records and hash from their own payload, proves ownership
  equivalence plus the exact target-singleton reassertion lineage, and never
  rebuilds the immutable restore from mutable historical journals. Clusters
  without a separate GPU target-retirement authority require an empty active
  pause journal and a byte-exact complete canonical phase journal. Segment
  completion is also deferred after bridge cleanup until the current guarded
  rolling-values revision has applied and validation/post-upgrade checks rerun.
- Keep completed final-Helm manager restoration bound to its exact installed
  predecessor when a later values-contract revision changes the desired
  payload. cxcli now reproduces the immutable apply-intent fingerprint from
  live stored Helm values before admitting the historical restore; the guarded
  revisioned reapply remains the only owner of the subsequent change.
- Make final external-upgrade health prove the real SConfig JWT consumer path.
  cxcli now checkpoint-binds target singleton/Secret material, the exact
  SConfig Deployment and Pod, and both target JailedConfigs; it UID/resourceVersion
  fences a stale pre-handoff Pod restart and requires a distinct Ready
  zero-restart successor plus post-successor REST-driven update completion.
  The target-values revision also normalizes the managed Slurm 25 runtime to
  one canonical multiarch `PluginDir`, removes the invalid literal worker
  `InstanceId={{ .PodName }}` default, and removes its
  cloud-incompatible NVIDIA AutoDetect default; it rejects unknown unevaluated
  templates and reuses the guarded Helm/Slurm reconciliation path for live
  repair instead of direct infrastructure edits.
- Restore the managed CPU/GPU profile `PluginDir` after live completed-Jail
  revalidation proved that Soperator's fallback `/usr/lib/slurm` cannot load
  `chroot.so` from the supported Nebius login/worker images. Rolling-values
  revision 28 now reapplies exactly one multiarch path through the checkpointed
  post-cleanup Helm/Slurm workflow and still requires login-side
  `sbatch --test-only` before accepting the completed Jail action.
- Compose the exact target cluster-ID marker Pod transition with a later
  generic client-handoff observation during typed-GRES target-HA replay. cxcli
  now keeps the accepted bridge successor and preservation receipt immutable,
  checkpoints the existing controller-lineage child first, then exact-key and
  semantic-hash seals a separate predecessor-observation receipt bound to both
  full projections, the controller and preservation proofs, target material,
  both jailed digests, and strict chronology. Any additional successor change,
  malformed child, projection, config, Jail, hash, or chronology drift remains
  fail-closed before reconfigure, worker signaling, or infrastructure mutation.
- Resume typed-GRES target-HA recovery from its exact staged config successor
  without re-running the immutable generic worker-rollout verifier against the
  successor digest. cxcli now exact-key validates the client-handoff
  preservation, compatibility and bridge successors, immutable recovery hash,
  worker-init projection, and optional bridge-authority rebind before retaining
  the predecessor receipt and continuing through the existing live recovery
  proofs. Optional rebind validation now independently exact-key checks the
  worker-lineage and authority chain, predecessor/current fingerprints, bridge
  authority transition, and strict chronology even if outer hashes are
  recomputed. Partial or drifted evidence remains fail-closed before
  reconfigure, worker signaling, checkpoint writes, or infrastructure mutation.
- Preserve a verified accounting Munge runtime across the exact dual-controller
  restart already sealed by the target cluster-ID marker repair. cxcli now
  requires bridge Pod UIDs to be the only changed runtime binding, reproduces
  the marker repair's predecessor/current roles, StatefulSet, queue, accounting
  writer/database, registered IDs, probes, fingerprints, and chronology, then
  freshly proves bidirectional Munge authentication against the current bridge
  Pods. Exact replay is write-free; any unrelated or partial lineage remains
  recovery-required before worker replacement or infrastructure mutation.
- Preserve the exact historical bridge-client source authority required by
  typed-GRES post-OpenMetrics replay after the product-owned preservation
  receipt advances the current handoff fingerprint. cxcli now validates the
  original semantic receipt, released target and ConfigMap identities, exact
  writer-release runtime and intent, and chronology before the existing current
  typed-GRES successor proof may bypass the distinct-transform path. Generic
  replay still requires current lineage, and missing or drifted historical
  evidence remains fail-closed without checkpoint or infrastructure mutation.
- Defer post-OpenMetrics config binding when the live target ConfigMap is the
  exact checkpointed canonical typed-GRES successor and therefore has no
  MetricsType transform left to perform. The release path now binds that state
  to the accepted ConfigMap UID/resourceVersion/content, target owner, bridge
  successor, recovery/reconfigure, and client-handoff preservation receipts,
  reuses its digest as the achievable runtime, and creates no false
  post-OpenMetrics receipt. Any identity, data, lifecycle, or lineage drift
  remains fail-closed before checkpoint or infrastructure mutation.
- Refresh and revalidate the verified cluster-ID repair stager before its final
  cleanup delete. Status-only Pod resourceVersion advances now use the fresh
  version with bounded retries and an exact UID-only fallback after complete
  inert-material revalidation, while replacement UID, image, command, security,
  PVC, readiness, or provenance drift still fails before deletion or checkpoint
  completion.
- Accept an exact already-sealed typed-GRES worker successor when target-HA
  config recovery revalidates its generic client-handoff predecessor. The gate
  now binds each immutable predecessor to the original rollover Pod and each
  current successor to the typed-GRES replacement, then reuses the canonical
  complete rebind and checkpoint-gap validator. Missing, partial, foreign, or
  drifted lineage still stops before checkpoint or infrastructure mutation.
- Accept cxcli's already-canonical multiarch `PluginDir` during the exact
  post-OpenMetrics compatibility projection. The parser still admits only the
  legacy pre-transform path or the canonical target path, always emits the
  canonical value, and rejects foreign, missing, or duplicate directives before
  ConfigMap or checkpoint mutation. Same-version Helm replay now also reuses an
  unchanged canonical bridge config only through its exact sealed runtime/source
  receipts and defers the post-OpenMetrics receipt until distinct full data is
  present.
- Accept the terminal login-restore workflow's exact strengthened SConfig
  writer-release successor proof. cxcli now binds its release/rebind receipts,
  semantic-hash scale and runtime children, and CRD-defaulted historical Helm
  predecessor, while no-op release replay preserves the existing verification
  timestamp. Partial, foreign, history-drifted, or prerequisite-reordered proof
  still fails before Helm, checkpoint, or workload mutation.
- Seal a later exact login replacement through the canonical bridge-client
  source producer before terminal rollout replay consumes an earlier protected
  surge-removal edge. This preserves the historical removal proof and exact
  rollout timestamps while admitting the worker producer's terminal mixed
  successor only when every worker UID, node, boundary, and predecessor/current
  container identity reproduces its verified bridge-config or typed-GRES
  rebind. A live-RPC result additionally requires the sealed controller-gap
  binding and exact per-client hashes; mixed login or unsealed worker state
  remains fail-closed. Post-observation replay now also accepts the product's
  exact RPC-inclusive producer suffix only when raw hashes chain edge-to-edge,
  strict reverse replay rejoins the sealed prefix, no suffix removal exists,
  and the reconstructed predecessor rollout matches the observation receipt's
  timestamp-free identity projection. When cxcli produced that suffix only
  after terminal SConfig release, the count-advancing receipt may now be sealed
  only from a prior durable rollout prefix, an exact-key verified release with
  valid Deployment-scale/runtime child receipts, unchanged identities and
  digests, and suffix events strictly newer than both release timestamps.
- Reconcile the bridge-client worker handoff before typed-GRES target-HA
  recovery consumes its lineage. cxcli now runs the existing strict producer
  after the verified worker rollover, seals every per-worker replacement and
  aggregate checkpoint-gap successor receipt, and only then admits the
  topology/config recovery. This adoption performs no additional Pod delete;
  partial or drifted lineage remains fail-closed, and exact producer replay no
  longer refreshes the terminal rollout timestamp or rewrites the checkpoint.
- Accept the product's own terminal SConfig writer-release successor during
  protected login-restore Helm replay. The canonical validator now keeps the
  exact intent crash window distinct from the exact `verified` state and, for
  the latter, requires the released bridge, complete verified Deployment-scale
  and runtime receipts, target resourceVersion and ready-Pod identity, and
  ordered child/release/bridge timestamps. Partial, extended, status-drifted,
  or reordered terminal state still fails before checkpoint or cluster
  mutation.
- Preserve the canonical target-manager rearm across the exact final SConfig
  writer release. When the mutable compatibility producer has legitimately
  advanced to `released`, OpenMetrics admission now reconstructs the unchanged
  rearm only from the earlier exact reuse receipt, its completed bootstrap and
  operation authority, the live zero-manager identity/spec/generation, and the
  ordered release checkpoint. A v2 reuse receipt binds both predecessor and
  immutable released-bridge authority fingerprints; missing, malformed, reordered, target-drifted,
  release-drifted, or self-consistently changed lineage fails before another
  checkpoint, webhook publication, or Helm mutation.
- Release the target SConfig writer against an achievable, sealed runtime while
  its manager is deliberately paused. cxcli now checkpoints an exact semantic
  receipt binding the immutable release intent, target/ConfigMap identities,
  sealed bridge-client content, deferred full-target digest, and chronology;
  then verifies Ready/Jail delivery without claiming later full-target or REST
  recovery. Exact replay is write-free and any receipt, identity, data,
  lineage, intent, or chronology drift still fails before mutation.
- Revalidate the protected login-surge restore Helm proof after the exact final
  SConfig writer release. The proof now classifies the positive live size only
  through the canonical sealed writer-release successor, reconstructs the
  zero-size predecessor, and requires it to equal the CRD-defaulted installed
  Helm spec. Only that restore replay opts in, and the proof binds the release
  generations, immutable intent, active Jail rebind, and predecessor spec;
  missing checkpoint authority, an inexact producer, or any additional live
  drift remains pending before Helm, checkpoint, or workload mutation.
- Resume the final SConfig release after the admitted target size patch without
  weakening the earlier Jail boundary. The target CR fence and direct post-Helm
  rebind now accept positive size only through the exact checkpointed release
  intent, reconstructed verified login-restore predecessor, single generation
  increment, unchanged non-size spec, and complete target, Deployment,
  ServiceAccount, ConfigMap, rootfs, digest, resource-version, and chronology
  bindings. Any present Deployment-scale receipt is semantic-hash and live-state
  validated before bridge-client lineage checkpointing; malformed, foreign,
  partial, skipped-generation, or active-without-intent state remains pending
  before checkpoint or cluster mutation.
- Complete final SConfig writer release while the target manager remains
  deliberately paused. After the admitted SlurmCluster size patch, cxcli now
  re-proves the manager pause and exact target-owned Deployment/ServiceAccount,
  seals a semantic-hash, resourceVersion/current-replicas guarded Deployment
  scale intent, and accepts only the next-generation `0 -> original-size`
  successor before readiness verification. Crash-after-dispatch replay is
  exact and terminal replay is write-free; absent intent, partial or extended
  receipts, semantic drift, identity/spec drift, and generation gaps fail
  closed instead of waiting for a paused reconciler that cannot converge.
- Keep validating admission served while releasing the fenced target SConfig
  writer after active-slot verification. The zero-to-original SlurmCluster
  patch now runs through the checkpoint-owned temporary manager webhook bridge
  with `failurePolicy: Fail`, exact Service endpoint and server-side admission
  proofs, crash-left bridge cleanup, and a fresh name/UID/resourceVersion/
  generation/zero-size/stable-spec precondition immediately before the guarded
  mutation. The exact bridge lineage is retired after success or failure;
  missing authority/readiness or target drift blocks before the patch, and cxcli
  never substitutes an `Ignore` policy bypass.
- Make terminal login-surge restoration replay byte-stable. The reconciler now
  reloads the checkpointed rolling state before terminalization so a restore
  that converges in one invocation retains `restore_started_at`, and an exact
  configured/configured replay preserves the original observation and
  completion timestamps without another checkpoint write. The already exposed
  pre-fix timestamp refresh is accepted only through the exact anchored
  same-count v2 rollout-observation companion and producer chain, with locked
  configured counts and strictly later observation/completion chronology;
  missing or drifted receipt authority, counts, or timestamps fails closed
  before cluster mutation.
- Resume active Jail-boundary validation after the protected login-restore Helm
  lifecycle has already restored the target SlurmCluster from surge to its
  configured login replicas. The boundary fence accepts only the exact sealed
  login-owner successor: the same target, one generation beyond the prior
  rebind, unchanged normalized spec and Jail contract, current bridge, manager,
  controller-gate and client authority, exact restore intent/bootstrap and
  protected Helm lineage, and the verified login workload removal. Replay
  rewrites no receipt, while partial, extra-field, foreign, skipped-generation,
  replica, chronology, stable-spec, Helm-history, or Jail drift blocks before
  worker mutation. Both the CR-fence and direct post-Helm rebind consumers use
  the same proof; the latter also revalidates the original SConfig Deployment
  rebind and any exact later sealed Deployment successor before returning
  without replaying its completed intent or writer pulse.
- Preserve sealed bridge-client source authority across later product-owned,
  session-free login Pod replacements and the exact protected login-surge Pod
  removal. The original source and canonical
  adoption receipts remain immutable; each exact propagation-producer advance
  appends a semantic-hash rollout-successor receipt binding the prior receipt,
  replacement or removal Pod/container/workload identities, the verified surge
  restore contract, reverse-replayed canonical consumer transitions, causal
  restore/rollout/propagation chronology, current consumers, and exact terminal
  login rollout. An unanchored removal cannot create a receipt or
  self-authenticate with an unsealed predecessor. The
  protected login-restore rebind checkpoints that producer receipt before the
  downstream Jail-boundary Deployment successor consumes it. Exact terminal
  replay preserves the sealed aggregate and per-Pod rollout timestamps when
  current consumers omit only that proven removal. A bounded pre-fix recovery
  first checkpoints a separate companion receipt only after the prior chain and
  producer prefix are exact and read-only live observation proves the login
  workload owner/generation plus every retained Pod UID, node, container,
  Running/Ready state, zero restarts, and jailed Slurm configuration digest.
  Removed Pods remain authorized only by the sealed removal edge; session
  selection and replacement-history reinterpretation are rejected. Aggregate
  and per-Pod timestamps are explicitly superseded as observation metadata;
  existing all-client propagation and handoff times remain unchanged and must
  precede recovery start and live observation. A second checkpoint appends one
  same-count v2 rollout successor bound to that companion. Both writes are
  crash-safe, exact replay is write-free, later producer transitions append
  normally, and duplicate recovery or
  malformed, foreign, reordered, retained-removed, workload/Pod-drifted, or
  producer-drifted lineage fails before cluster mutation.
- Resume protected login-surge restore and later admission bootstraps when the
  rolling phase already owns the exact verified target-compatibility manager
  rearm at the live zero-replica generation. A new sealed rearmed-pause reuse
  receipt binds the freshly reconstructed rearm producer, full terminal journal
  including its exact optional Helm repause, live target binding, manager
  UID/spec/generation, campaign, chart, values, prerequisite identities,
  rendered manager, restore values, and operation chronology without replacing
  the rolling pause. Creation and replay repeat the target binding proof;
  missing, foreign, over-advanced, extra-field, reordered, or receipt-drifted
  state fails before webhook publication, Helm dispatch, or checkpoint mutation.
- Canonicalize target Soperator packaged-chart fingerprints across Helm
  dependency rebuilds. Dependency archives now hash normalized member paths,
  types, nested chart content, and file bytes instead of gzip/tar writer
  metadata, while duplicate, unsafe, linked, special, truncated, corrupt,
  empty, or excessively nested archives fail closed. The protected post-switch
  login-surge adoption now seals a v2 one-way fingerprint transition when an
  older completed bootstrap contains the raw archive hash; the transition binds
  that predecessor to the current canonical fingerprint and requires equal
  freshly rendered and installed target manifest/spec fingerprints plus the
  existing exact release, values, history, target, chronology, semantic proof,
  and receipt. Replay is write-free and any transition or proof drift blocks
  before Helm, login, or worker mutation.
- Admit the exact bridge-client ConfigMap written by the later target-HA
  controller-version handoff as the source for resumed Jail compatibility and
  final SConfig writer release. A new durable semantic-hash receipt binds the
  live ConfigMap UID and content/data hashes to the target version transition,
  current HA epoch and fence, prior exact Jail writer zero, borrowed manager
  pause, immutable target and JailedConfig identities, login writer
  `0 -> 1 -> 0` refresh, and exact verified per-Pod rollout. Missing writers or
  any authority, identity, generation, chronology, rollout, content, or receipt
  drift remains pending before compatibility or release mutation.
  Canonical rollout validation covers both direct Pod observations and exact
  source/replacement/current-UID records plus the producer's optional session
  selection. The receipt additionally seals the manager pause generation,
  selected target-history entry, and handoff acceptance, and stays replayable
  after the same stage reaches `released` without permitting retroactive
  adoption.
  Crash-resumed login records are normalized at the producer before terminal
  verification, and a present session-selection value must have the exact typed
  shape rather than being collapsed to absence.
  Writer receipt validation now also requires the exact terminal producer keys
  and the complete causal timestamp chain through per-Pod and aggregate rollout
  verification.
  The Jail-boundary rebind now recognizes the exact same-Deployment generation
  successor produced by that sealed writer refresh, while rejecting every
  other generation, replica, identity, spec, or receipt shape.
  A later exact same-digest `0/0` canonical adoption now retains the immutable
  source only through a separately sealed successor receipt. The successor
  revalidates the original semantic hash and unchanged ConfigMap, target,
  manager, target-HA, boundary, and writer bindings, then seals the canonical
  adoption plus its later exact login rollout. Exact ConfigMap and Pod
  re-observation preserves existing acceptance and verification timestamps so
  replay is write-free; malformed, reordered, post-release-created, live-data,
  identity, rollout, or receipt drift fails before protected mutation.
  Compatible-stage login digest revalidation now includes that exact sealed
  bridge-client digest only when both active variants are explicit and the
  full-target transition key is absent; missing variants or malformed present
  transitions fail closed, and either variant advancing removes the candidate.
- Resume the verified borrowed-manager-rearm Jail boundary after controller
  authority has advanced from source HA to target HA. cxcli retains the sealed
  receipt's source epoch only when its exact-key full-target recovery, pause
  generations, gate identities, and exact boundary transition match and the
  bridge history ends with the unique canonical source/target writer-scale
  transition in chronological order. Before later mutation, a new exact
  semantic-hash receipt binds both complete records, both selected history
  entries, and both authority epochs. Missing, extra-field, duplicate,
  reordered, reason-drifted, time-inconsistent, or receipt-drifted lineage
  remains pending before the SConfig writer pulse.
- Fix target-HA legacy-rootfs health verification reusing a retained login
  client after controller authority moved to the bridge. The target-HA stage is
  now a hard direct-bridge boundary that revalidates the campaign, target owner
  and epoch, permanent source-restart fence, accepted two-replica StatefulSet,
  exact image and Pod identities, cluster-wide controller exclusivity, and the
  journaled active/standby roles before querying the queue from the exact active
  Pod. A sealed authentication-restart successor remains canonical, while a
  second Pod snapshot rejects any identity change during the role/queue probes.
  Authority, workload, Pod, role, or queue drift remains pending without
  checkpoint mutation or login fallback.
- Resume the direct Jail-boundary SConfig pulse under the exact later
  restored-manager rearm. cxcli now treats the immutable-child replica count
  as historical restore capacity, revalidates the sealed zero-manager pause,
  inert target-controller workload and Pod, and source-HA bridge, then
  checkpoints a boundary-local borrowed authority before any ConfigMap or
  SConfig mutation. If the paused manager left the exact previously generated
  full-target ConfigMap behind, that state is classified only through its
  ConfigMap UID and hashes plus the rearm's immediately preceding restore
  generation. The pulse changes only SConfig `0 -> 1 -> 0`, returns the active
  variant to compatible, and never restores the manager; any lineage, live
  identity, generation, gate, ConfigMap, or bridge drift remains pending.
- Resume the direct active-slot Jail-boundary rebind after the exact target
  accounting writer has been restored. Before comparing the live SlurmCluster
  with its prepared fence or recovering a missing boundary intent, cxcli now
  applies the existing strict accounting-writer successor validator and
  checkpoints that receipt. The same target UID, one-generation transition,
  restored/enabled writer bindings, source retirement, Ready target accounting
  Deployment, login submounts, and normalized writer-command preimage remain
  mandatory; any additional drift stays pending without fingerprint adoption.
- Resume legacy-rootfs fast verification after active-bridge service replay
  intentionally gates the target controller. At the exact source-HA and
  verified-gate boundary, cxcli now proves the campaign-bound bridge workload,
  Pods, images, shared state, cluster-wide slurmctld exclusivity, journaled
  active/standby takeover, unique primary marker, and live queue RPC directly
  instead of probing a target-only login config whose controller must be DOWN.
  Missing or drifted bridge/gate authority remains pending without a fallback
  client path or checkpoint mutation.
- Resume active-bridge service replay from the producer's exact completed
  `target-compatibility-active` receipt. Restored-manager rearm reconstruction
  now recognizes either the interrupted intent with its child-handoff-gap proof
  or the completed active state with its terminal stable marker, zero writer,
  complete jail-boundary state, post-retirement bindings, accounting retirement,
  and exact manager identity/spec/generation chain. Status-only, incomplete, or
  drifted active states remain fail-closed before manager or controller-gate
  mutation.
- Resume controller-inspector admission from its exact sealed Node set while a
  provider-pending bridge Node is still registering. Continuations now refresh
  resource versions and revalidate every recorded Node name, UID, provider ID,
  system UUID, deletion state, and set fingerprint without widening that
  earlier admission proof. Command entry also defers fresh source discovery when
  the incomplete bridge owns a real accepted temporary node-group operation, so
  that checkpoint-owned provider delta reaches its exact executor reconciler
  instead of failing config-waypoint admission. An exact active mutation
  boundary retains the journal's waypoint even if a prior failed entry already
  refreshed the report to the intermediate state; it cannot advance campaign
  completion. Initial admission and the later pre-writer all-Node census remain
  strict, and discovery deferral rejects incomplete provider-operation records.
- Resume active-controller service replay when the rolling phase retains the
  exact verified `active-slot-switch` manager pause that precedes a canonical
  restored-manager rearm. cxcli now requires the same campaign, target,
  manager UID, original replicas, non-replica spec, and pause generation, plus
  a valid complete bridge journal with the exact campaign, stage owner, epoch,
  and permanent source-restart fence. It then checkpoints `rearm-authorized`
  before re-pausing the restored manager or recreating its controller command
  gate. Every checkpointed rearm status repeats the bridge validation and
  rejects a newly conflicting direct-bridge or target-admission pause before a
  live target read. Drifted predecessors or bridge authority still require an
  exact direct bridge, target-admission successor, or sealed controller-gap
  authority and remain mutation-free.
- Reconcile the producer's exact dual-owner checkpoint when the controller
  bridge still carries its original sealed standard pause and rolling owns the
  later restored active-slot bootstrap pause. cxcli now proves the earlier
  bridge generation and reserialization chain, linked manager identity/spec,
  chronology, campaign, and bridge epoch; then removes that historical owner
  and hash-seals its adoption together with `rearm-authorized` before the
  low-level pause verifier runs. Replay remains bound to that epoch or its exact
  immediate source-to-target successor. Drifted or reappearing bridge ownership
  remains fail-closed before live reads.
- Route the earlier post-switch started-gate continuation and both post-Jail
  in-place completion branches through that same authority-aware fence. This
  prevents a restored bootstrap manager—or a crash immediately after
  `rearm-authorized`—from reaching the low-level pause verifier before the
  checkpointed rearm selector runs.
- Fix external Jail/rootfs compatibility resumes that discarded the campaign
  checkpoint and durable writer before selecting an exact target login backend.
  The legacy digest, Slurm-health, active-compatibility, immutable-child
  cleanup, Jail-boundary, writer-release, and Helm-replay verifiers now retain
  that authority. The existing fail-closed protected-login exception can admit
  only the one checkpointed old-revision Pod while missing or incomplete
  authority still blocks the resume.
- Resume the nested accounting command-fence admission bootstrap at both exact
  producer stages: directly after target-handoff admission and its verified
  Helm intent/proof, or after an interrupted earlier accounting intent re-enters
  behind an exact completed `post-switch-resume`. cxcli replays either the
  sealed handoff adoption and reuse lineage or the completed post-switch
  authority and reuse receipt, with strict source/target identities, compatible
  semantic drift, manager binding, and chronology before checkpointing one
  accounting-boundary reuse receipt. Missing, incomplete, unsealed, reordered,
  foreign, ambiguous, or drifted evidence remains write-free and fails before
  webhook publication or the accounting operation.
- Resume a target-handoff Helm intent through its exact pre-dispatch admission
  lifecycle instead of comparing that unapplied intent with the previously
  deployed release. The recovery probe now requires the same target, selected
  chart content, current effective values, prepared intent, pre-Helm status,
  and absence of a terminal proof or values revision before any provider,
  cluster, or checkpoint-capable work and before returning to the guarded path;
  drift remains write-free and fail-closed. Missing or unknown statuses in
  non-empty lifecycle states are rejected, while the canonical empty no-webhook
  placeholder,
  `helm-applied`, and `complete` retain strict post-dispatch recovery and an
  already-started accounting-writer gate still blocks fresh-target Helm.
- Resume external Soperator target handoff from the exact checkpoint split in
  which rolling retains the completed passive-slot manager pause while
  `populate-jail-refresh` owns the completed active-slot pause. cxcli now
  accepts the same Deployment generation for a no-op manager-spec reapply or
  exactly the next generation for a real Deployment change, and proves both
  completed bootstrap lifecycles, the live annotated
  zero-manager, campaign, target UID, manager UID/spec, original replicas,
  chart and prerequisite renders, generation sequence, and chronology, then
  atomically moves the active pause to rolling and seals owner-adoption plus
  target-handoff reuse receipts before webhook publication or Helm dispatch.
  Incomplete, decreased, skipped-generation, foreign, malformed, or drifted
  state remains recovery-required without a checkpoint write.
- Resume the following `post-switch-resume` admission lifecycle from that
  exact target-handoff-adopted active-slot pause when the manager Deployment
  was never restored or rewritten. cxcli now reconstructs the completed
  target-handoff pause authority, replays both sealed handoff adoption and
  continuous-reuse receipts against the completed passive, active, and
  handoff lifecycles, and requires unchanged campaign, target, manager,
  generation, spec, chart, prerequisite, and chronology evidence before
  checkpointing one post-switch continuous-reuse receipt. Missing, incomplete,
  reordered, or drifted handoff lineage remains recovery-required before
  webhook publication or Helm dispatch.
- Clarify external Soperator onboarding, render/deploy, deploy-block, and help
  guidance so `--dry-run` is an optional inspection step and
  `--execute --approve` is a direct upgrade action. The commands and approval
  contract are unchanged; dry runs are no longer described as accepted or as
  an execution prerequisite.
- Make temporary controller-bridge node-group deletion converge when the exact
  identity-bound provider dispatch returns `NOT_FOUND`. cxcli now seals that
  response as a deleted postcondition only after durable request journaling and
  live ID, name, resource-version, ownership-label, and slot validation; a
  later accepted operation may likewise converge when the exact resource is
  absent and lookup of its real checkpointed operation ID returns `NOT_FOUND`.
  Pre-dispatch absence, identity drift, transient lookup failures, unrelated
  errors, and malformed receipts remain recovery-required.
- Keep post-upgrade MK8s readiness verification from treating retired temporary
  controller-bridge node groups as retained source infrastructure. Only exact
  IDs from a valid `external-temporary`/`provider-create-delete`/`delete-domain`
  bridge record are excluded; managed-existing, customer, worker, and
  service-role groups remain mandatory.
- Derive final Slurm target-retirement from the exact mirrored canonical
  target-partition pause receipt when the optional GPU-smoke scheduling gate
  was never written. Resume now excludes source-era partitions from restore
  only after verifying the complete pause and controller-gap inventories,
  target names, and target `DOWN` records. A later target-only record must be
  bound by the exact target-singleton runtime-reload reassertion while every
  source record stays identical; an already-started legacy restore plan is
  rebound only after the full live snapshot proves those source names absent
  and the surviving target state exact. Cleanup consumes that exact restored
  receipt after the target record leaves the active pause journal instead of
  reconstructing the completed plan from historical phase predecessors.
- Let protected-login successor selection consume the canonical target-ready
  hold-release authority without requiring the deliberately absent legacy
  zero-session probe. The exact release journal, source Pod, target workload,
  current successor Pod, image, container/restart state, and SSH host keys must
  still match before retirement; malformed authority or identity drift remains
  fail-closed, and the existing drained-session receipt path is unchanged.
- Fix later-segment protected-login successor chronology to follow the producer
  lifecycle: bridge cleanup, final login revalidation, predecessor completion,
  then current-segment start. The exact archived target and successor lineage,
  released hold, runtime identity, host keys, and final Helm proof remain
  mandatory, while pre-cleanup or post-completion evidence stays blocking.
- Let an already-sealed later-segment protected-login successor journal an exact
  same-workload Pod UID rollover instead of failing before the rollover adapter.
  Replay must reconstruct every other successor field and both proof
  fingerprints, re-prove target-ready zero-session runtime identity, and
  checkpoint the UID hop before continuing. Every later resume validates the
  unique and causally ordered chain from the sealed target Pod, including each
  hop's workload, runtime hashes, and proof fingerprints; any drift remains
  blocked.
- Split protected-state remediation authorization from the raw comparison audit
  fingerprint and make exact remediation continuation automatic by default for
  managed and external Soperator upgrades. cxcli checkpoints the first plan,
  performs one fresh read-only verification, and consumes only the unchanged
  exact fingerprint; a moving plan or blocked delta still fails closed.
- Replace the remediation approval flag pair with the single opt-in
  `--stop-for-remediation-approval` review mode. Exact journaled Slurm
  intent-to-held recovery now follows the same automatic default without a
  second hold RPC, while review mode persists and stops.
- Resume terminal external in-place checkpoints from their exact sealed
  authority instead of refreshing mutable chronology. Verified marker and
  controller-lineage receipts remain bound to their original successor proofs;
  intentionally empty provider segments use the campaign's accepted worker
  scope; and exact continuous manager pauses can span later boundaries under
  the same rolling owner. Source cleanup keeps the rolling provider-release
  gate as its primary proof, but an explicitly empty provider segment may use
  the effective typed-GRES-projected post-Jail release only when the
  self-hash-valid topology-drain authority seals the release, replay, partition
  pause, fleet, bridge, and runtime lineage and fresh final worker plus
  immutable-child bindings still match. Provider segments and any receipt,
  target, generation, scope, or fingerprint drift remain fail-closed without a
  synthetic checkpoint receipt or compatibility path. Rolling fast
  verification now treats an exact empty planned segment and explicitly
  recorded empty journal as converged `0/0`, while an absent journal and any
  unexpected, malformed, missing, or incomplete group scope remain failed.
  Immutable-child cleanup now zero-fences each exact captured old ReplicaSet
  and proves selector-wide Pod absence before deleting the owner, preventing a
  replacement Pod from being recreated during child-first retirement. A
  previously completed cleanup may retire residual non-Ready orphan Pods before
  validation only through a checkpointed exact source-absence, target-readiness,
  deleted-owner, name/selector/UID-lineage, stable-fingerprint, and
  UID/resourceVersion-preconditioned deletion proof; Ready, target, foreign,
  live-owner, unapproved, or drifted candidates remain fail-closed. External
  protected-state Node presence deltas now select their exact provider authority
  per Node: rolling replacements remain bound to their completed group operation,
  while a zero-group segment may prove a temporary bridge Node only through its
  terminal mirrored create receipt and exact recorded Node name, UID lineage,
  and node-group label. Empty journals and unrelated bridge receipts cannot
  authorize arbitrary Node drift.
- Resume GPU topology drain recovery when Ready zero-restart workers still run
  the exact pre-replay `slurmd` process generation: typed GRES is already
  correct, but the idle zero-allocation registration retains a smaller static
  CPU topology and no replayed `Parameters`. cxcli now binds the Pod/process
  chronology, live Helm-owned NodeSet UID/generation/spec/static topology,
  jailed `slurmd -C`, release/workload identity, and partition/queue fences
  before using the existing checkpointed UID/resourceVersion-CAS parallel
  rollover. Distinct successors must start after replay with the exact runtime
  config. A fresh target-HA observation skips the shared config rewrite for an
  exact replacement and invokes the existing topology-only recovery for either
  the exact generic-GRES `INVALID_REG` successor or the exact stale-static
  controller registration after its replacement runtime is rebound to the
  rollover proof. The idle registration receives no unnecessary
  `State=RESUME`. Resume preserves the historical full `slurmd -C` fingerprint
  but freshly compares only the canonical node, CPU/socket/core/thread, and
  typed-GRES fields, avoiding false drift from non-contract uptime, memory, or
  temporary-disk output. Before any topology Slurm RPC, cxcli now runs the
  existing exact target cluster-ID marker repair so a topology-triggered Slurm
  25.11 re-exec cannot encounter the preserved source ID against the target
  SlurmDBD. At this pre-smoke boundary, the repair uses the full checkpointed
  pause/zero-allocation queue chain instead of an unavailable controller RPC,
  revalidates the enabled authenticated target accounting writer plus DB/PVC
  identity and live registration IDs, and re-proves accounting authentication
  after the active/standby restart. When both exact journaled controllers have
  already entered the stale-marker exit-1 `CrashLoopBackOff`, cxcli binds that
  two-Pod/StatefulSet preimage and uses only the already-existing Ready inert
  stager, with its image bound to the identical immutable campaign source image
  in the bridge source binding and version transition. The current bridge
  target image is bound separately, and the stager's command, security,
  UID/resourceVersion, and shared state PVC remain exact. Resume adopts that
  same reader without another apply; the normal checkpoint-first stop, exact
  marker deletion, restart, proof, and cleanup stay unchanged. After the
  verified restart, cxcli now retains the typed-GRES successor's historical
  Pod identities and checkpoints one exact child-lineage receipt to the current
  active/standby generation. The receipt binds the marker repair's predecessor
  UIDs, StatefulSet, pre-Slurm successor, role fingerprint, restart chronology,
  live config, both jailed digests, and each Ready controller's container ID,
  image ID, restart count, and start time. Guarded full-lineage fences now run
  before the pre-reconfigure pause reassert, immediately before
  `scontrol reconfigure`, before the post-reconfigure pause reassert, and after
  the successful verification ping, so foreign Pod, process, or role
  replacements remain blocking before mutation or verified checkpointing.
  Verified marker replay now validates its sealed historical queue/accounting
  authority directly instead of reconstructing pre-Slurm chronology from a
  later semantically identical queue observation; unverified repairs still
  require fresh authority derivation.
  Allocation, reason, state, GRES, semantic
  topology, NodeSet, chronology, marker, accounting, or successor drift remains
  fail-closed.
- Reload an exact pre-CAS target accounting Munge runtime through the existing
  verified Secret handoff. cxcli binds the active target Deployment,
  ReplicaSet, Ready zero-restart Pod, Service selector, Secret projection,
  restored writer, and both retained bridge roles, proves local health plus
  bidirectional runtime isolation with non-persisted ephemeral credentials,
  and requires checkpoint-paused partitions plus a fresh active/standby role
  proof and an empty Slurm queue read directly through the exact active bridge
  Pod before a UID/resourceVersion-preconditioned Pod deletion. Retained login
  routes are not accepted as target-HA authority, and direct RPC failure remains
  distinct from successful non-empty output. Only a distinct same-Deployment
  post-CAS successor that authenticates with both controllers is sealed. Replay
  is mutation-free, and mixed authentication, queue, owner, selector, Secret,
  writer, bridge, or successor drift fails before deletion.
- Compose the controller-gap client successor proofs when one config-only
  observation contains both an exact session-free replacement of a retained
  login and the exact terminal removal of the temporary surge ordinal. cxcli
  proves the replacement over the retained set, normalizes only that proven
  identity, then proves the full surge removal and journals one combined
  successor bound to the same workload and target. Foreign proof material,
  active sessions, extra client changes, or config and owner drift remain
  mutation-free recovery-required.
- Resume the later SConfig bridge-config refresh after an adopted protected
  post-switch login surge has already restored the configured login set and the
  target writer. cxcli now recognizes the prepared writer fence's exact
  successor through the full campaign-bound adoption, proof, semantic,
  reconciliation, protected restore-owner, client-removal, manager, controller,
  propagated-client, and restored accounting-writer chain without synthesizing
  the absent ordinary SlurmCluster-regression receipt. The ordinary route still
  requires its accounting rebind; the protected route accepts the exact older
  no-rebind checkpoint only through its release-root/head and live-boundary
  proof. Drift still blocks before any writer restart or replica mutation.
- Keep OpenMetrics target-admission bridge and manager-pause ownership on the
  canonical rolling phase. Resume can normalize a checkpoint written by the
  earlier operation-local owner only after the completed bootstrap, both sealed
  pause authorities, campaign, target UID, manager UID/spec/generation, chart,
  prerequisites, restore values, and lifecycle timestamps agree. cxcli writes
  the existing continuous-pause reuse receipt and removes only the redundant
  operation-local pause; drift remains mutation-free recovery-required.
- Carry the exact verified login-surge removal receipt into later
  controller-gap bridge recovery. A durable three-login client proof may now
  advance to the configured set only when the same target and workload UIDs,
  replica transition, terminal generation, retained Pod, container, and config
  identities, and absent surge ordinal all verify. The removal is appended to
  the bridge consumer-successor journal before live RPC proof; any other client
  drift remains recovery-required.
- Adopt a deployed protected login-restore successor when Helm has already
  changed the SlurmCluster owner from surge to configured replicas while the
  workload remains surged. cxcli replays the exact root-to-head history,
  verifies both immutable revision manifests against the surge and restore
  proofs, applies live CRD defaults, and requires a login-size-only spec delta
  under the existing target and controller fences. It checkpoints the
  protected-successor owner adoption before workload scaling and performs no
  duplicate SlurmCluster patch; any history, manifest, proof, identity, or
  non-login drift remains blocking.
- Disable Go HTTP/2 client negotiation only for the target Soperator Helm
  `upgrade --install` child after repeated protected login-restore release
  Secret connection losses. TLS continues over HTTP/1.1, unrelated `GODEBUG`
  settings and non-target commands retain their inherited environment, and the
  chart values, release identity, protected-dispatch journal, and fail-closed
  outer-invocation recovery remain unchanged.
- Protect the temporary login-surge restore Helm pulse with the exact deployed
  surge release revision, chart/app, stored-values fingerprint, and target UID.
  An interrupted `admission-ready` checkpoint that lacks this precondition may
  recover it only when the full intent/proof or protected post-switch adoption
  authority names the same still-deployed head and Helm history has no
  successor revision; cxcli checkpoints that recovery before dispatch. Later
  failed successors are retryable only through the bounded protected history
  for webhook startup or the exact Kubernetes ambiguous release-create timeout.
  Changed heads, arbitrary failed revisions, generic timeouts, and values-only
  matches remain fail-closed.
- Reuse the exact continuously paused target manager across later temporary
  login-surge, restore, and OpenMetrics admission lifecycles. cxcli keeps the
  canonical `post-switch-resume` pause unchanged and writes a boundary-local,
  idempotent receipt binding campaign, target UID, manager UID/spec/generation,
  chart, prerequisites, and both sealed pause authorities. Foreign ownership,
  generation movement, material drift, malformed receipts, or unbounded reuse
  history still fail before webhook publication or Helm.
- Resume temporary login restore after exact source retirement even when the
  rolling phase still carries its earlier pre-retirement source-presence
  snapshot. That scheduling observation is no longer treated as durable replay
  authority: cxcli requires the full source-retirement, target writer,
  import/schema, selector-handoff, and target-only command-fence chain, then
  freshly rejects any reappeared source immediately before Helm. Incomplete
  retirement or writer proof still fails before mutation.
- Recover a temporary login surge already applied by the exact completed
  `post-switch-resume` protected Helm dispatch before a surge-local intent was
  recorded. cxcli now binds the immutable release root, expected deployed
  successor, target/chart/value identities, causal Helm history, rendered live
  target, and compatible semantic drift into a protected-dispatch-bound proof
  and adoption receipt without replaying Helm or inventing a retroactive
  intent. Historical applied values remain the dispatch authority; freshly
  reconstructed desired values are bound separately and accepted only through
  exact semantic normalization with unchanged OpenMetrics and no unclassified
  target-spec drift. A later `peer-ready` or `released` resume rebinds that
  adoption authority and enters restore instead of demanding a nonexistent
  retroactive surge intent. Successful protected commands also checkpoint their
  successor immediately, while late recovery preserves an admission lifecycle
  that is already complete. Values-only and wall-clock legacy recovery remain
  blocked.
- Resume an exact accepted bridge-client compatibility successor when the
  ConfigMap and checkpoint already hold its sealed target digest but the first
  cluster-wide propagation proof has not yet been written. The replay stays
  `accepted`, performs no receipt rewrite, and continues through the existing
  login, worker, and live-RPC gates. Partial propagation state, a foreign
  ConfigMap, a conflicting login digest, or any unsealed target still fails
  closed without a checkpoint write; no legacy checkpoint path is added.
- Seal an exact compatibility-adoption receipt when the selected bridge-client
  payload is already canonical. The receipt binds the target and ConfigMap,
  records equal predecessor and target digests with zero transforms, and lets
  the existing serial worker rollout handle an exact unready predecessor. An
  accepted replay before first propagation now recognizes that canonical reason
  only through the same exact receipt, observed digest, and verified login
  rollout. A crash after the exact verified propagation checkpoint but before
  the handoff-status checkpoint resumes through the existing complete
  predecessor proof; partial propagation and missing or mixed
  adoption/transform authority remain fail-closed before Pod enumeration or
  mutation.
- Recover an unjournaled old worker whose projected ConfigMap already exposes
  the exact bridge-config successor but whose runtime cannot become Ready.
  Digest classification now routes that exact Pod through the existing
  partition-pause, MUNGE, zero-job, identity, and UID/resourceVersion-CAS
  serial recreation path; only a distinct Ready exact successor is verified.
  An initially Ready exact successor remains no-replacement, and any foreign
  digest still fails before Pod mutation.
- Preserve the no-RPC controller-gap partition proof after those workers are
  recreated under a canonical target-admission bootstrap manager pause. Pause
  validation now consumes the sealed UID, generation, replicas, spec, and
  bootstrap contract instead of requiring a standard-pause-only Kubernetes
  resource version. The exact target, inert command gate, restored admission
  window, target-HA bridge, and `DOWN` records still must match before cxcli
  seals or reuses the controller-gap binding; unsealed or drifted authority
  remains fail-closed.
- Replace opt-in runtime-auth bootstrap with one automatic, idempotent
  `nebius-cxcli-sa` per Nebius project. Every project-aware command now ensures
  the exact project `editor` identity before product writes, uses existing
  operator profile/environment credentials only for bootstrap and recovery,
  and then exports the canonical renewable authorized-key identity for SDK,
  Terraform, MK8s handoff, and long-running upgrade work. The project-only v2
  cache is lock-serialized, owner-only, atomic, and recoverable across the
  authorized-key upload/commit crash window; Object Storage credentials are
  issued lazily and preserved across authorized-key rotation. Healthy cached
  keys are token-validated before IAM mutation,
  while deleted keys and explicit recreation use operator recovery authority;
  a new key propagation timeout cannot issue a duplicate replacement.
  Generation-specific private-key files keep metadata replacement as the atomic
  rotation commit and secret-bearing runtime material is redacted from object reprs.
  CI sync now carries an exact project-binding marker, allowing clean runners to
  validate and import the canonical identity through read-only IAM calls into an
  ephemeral strict cache without issuing another authorized key or attempting IAM
  repair under editor authority. Lazy first-use Object Storage key creation retains the
  process-captured operator bootstrap identity until issuance completes, then removes it
  before product execution.
  Conflicting same-name identities, unexpected permit-group
  members, roles, or resource scopes, legacy client-keyed caches, and the retired
  `nebius-cxcli-tf-sa` path fail closed without migration or automatic deletion.
  Remove public auth-bootstrap toggles and `auth --create`; targeted `auth`
  ensures automatically, while validation, explicit rotation, and CI sync
  remain available. First-use and recovery bootstrap now retry an expired active
  Nebius CLI profile once through its normal browser flow when stdin is interactive;
  non-interactive failures include a sanitized token-acquisition reason instead of
  hiding the CLI failure behind the generic SDK-auth message.
- Require renewable service-account credentials for external Soperator upgrade
  execution before cluster handoff, Lease acquisition, or checkpoint mutation.
  The parent Nebius SDK now owns background token renewal, bounded provider
  requests wait for a renewal already due, and the temporary Kubernetes exec
  plugin repeatedly exchanges fresh tokens through the same renewable authority
  and running cxcli installation. Execute mode replaces stored kube contexts
  with that renewable handoff when a Nebius cluster ID is available and rejects
  kube-context-only targets whose command-lifetime renewal cannot be proven.
  Provider SDK cleanup is wall-clock bounded so a stalled close cannot hold the
  Kubernetes Lease indefinitely. Static IAM, CLI, config-profile, and
  impersonation tokens no longer start this long-running workflow, and an
  inherited static token cannot mask a healthy project runtime-auth cache.
- Fix first-entry Jail Upgrade post-switch resumes that need to re-establish the
  temporary OpenMetrics compatibility override but have no earlier
  `post-switch-resume` admission lifecycle. cxcli now proves the deployed Helm
  values against either the completed active-slot lifecycle or an exact later
  intent/proof successor, including the exact release revision, chart/app
  identity, and live target UID, before recording a new switch intent. It
  preserves that deployed predecessor for the fresh admission apply and
  reserves historical post-switch recovery for a lifecycle that actually
  completed. Admission Helm replays now carry a separate canonical ungated
  manager-restore authority, so a valid bridge-gated deployed predecessor with
  zero manager replicas cannot be misread as the future restore contract;
  login-surge and GPU stored-values replays use the same fail-fast boundary.
  The gated and ungated manager renders must also have the same non-replica
  spec before cxcli checkpoints an intent or applies bootstrap prerequisites.
  If that fresh lifecycle is interrupted before Helm, resume now accepts only
  the exact deployed predecessor whose canonical compatibility transform
  reproduces the checkpointed desired fingerprint; after `helm-applied`, the
  raw deployed values must still contain the OpenMetrics override. The selected
  revision, chart/app, values fingerprint, and target namespace/name/UID now
  remain an immutable precondition through admission and session preparation;
  cxcli revalidates it before CRD mutation and immediately before the first Helm
  command, blocks a concurrent release replacement, and does not internally
  retry a protected failed command. The dispatch is now checkpointed with its
  exact release head and expected successor, allowing the next invocation to
  recover only an unchanged head, an exact pending revision, a bounded
  intent-bound transient-webhook failed chain, or the exact deployed successor.
  Proven pending revisions are cleared, failed heads require durable one-attempt
  authorization, and a proven deployed successor advances the admission
  lifecycle without duplicate Helm dispatch; foreign revisions remain blocked.
  Pending cleanup now validates the exact Helm storage Secret and deletes it
  with UID/resourceVersion preconditions, then requires the exact prior release
  head before recording retry authorization. A concurrent pending-to-deployed
  transition or Secret replacement therefore remains pending without deleting
  Helm history. The outer admission lifecycle now retains the original deployed
  root while the protected journal carries an authenticated failed or deployed
  current head, so next-command recovery can re-enter without immutable-contract
  drift. Pending cleanup requires the exact intent- or journal-bound revision;
  generic timeout and `another operation` paths no longer delete an unbound
  latest pending release. Post-switch replay also forces the admission lifecycle at later
  bridge stages so the journal and controller/manager fences cannot be skipped.
  Completed historical compatibility replay now carries the current deployed
  successor through the same checks.
- Fix external in-place accounting handoff resumes that deadlocked when cxcli's
  own verified login-session PodUnavailableBudget intentionally held one Ready
  Pod on the old revision. The readiness gate now accepts only that exact
  checkpointed shortfall while preserving generation, replica, owner, endpoint,
  and Slurm RPC checks; unheld or otherwise unexplained rollout lag still fails
  closed.
- Render only the `FAIL` token in phase-validation failures in red on
  color-capable terminals while keeping plain output, checkpoints, and
  Markdown/JSON reports free of markup.
- Fix External Soperator chart/Jail-only in-place resumes that could not derive
  the login provider identity from an intentionally empty current-segment MK8s
  mutation list. The payload-aware in-place dispatcher now reads the
  fingerprint-verified campaign-wide inventory for both ownership-only and
  normal compute segments, combines its stable ID/name with the active
  segment's source/target versions, checkpoints exactly one complete login
  binding, and leaves segment operation intent unchanged. Multi-hop campaigns
  therefore bind each provider rollover to its current hop instead of the
  campaign-final version; deeper provider and gateway replay paths consume the
  durable binding and fail closed on missing, ambiguous, incomplete,
  fingerprint-drifted, or changed authority.
- Fix External Soperator chart/Jail-only in-place campaigns that reached the
  GPU pre-activation fleet gate with an intentionally empty segment-local MK8s
  group list. The gate now reads the locked campaign-wide node-group inventory,
  filters it through the exact checkpointed worker scope, and revalidates the
  checkpointed owned-group binding before activation without rewriting segment
  operation intent or dispatching a provider mutation.
- Repair the exact controller-bridge crash window where Nebius accepted and
  completed a temporary node-group create but an interrupted resume marked the
  group `created` without sealing terminal provider proof. Command-start
  progress now admits only the matching mirrored accepted operation for
  reconciliation; the executor verifies terminal success and the exact live
  group identity without redispatching create, then atomically seals both
  journal copies. New writer paths cannot persist `created` before that proof.
- Resume external in-place upgrades when the target admission bootstrap created
  `deployment/soperator-manager` already fenced at zero replicas. cxcli now
  seals a distinct campaign-, rendered-spec-, prerequisite-UID-, zero-Pod-,
  and ungated-restore-bound pause authority instead of inventing a scale-down
  generation transition. The exact producer-sealed `accepted` or `terminating`
  scale-down checkpoint is migrated only when its complete base key set,
  optional canonical same-generation spec-reserialization chain, contract
  fingerprint, generation transition, and every live proof match; idealized,
  foreign, malformed, or drifted paused-manager records still fail closed.
  The `post-switch-resume` admission bootstrap can now supersede an
  `active-slot-switch` completed pause only when the target-compatibility
  journal proves the same manager's exact restore generation and the current
  annotated zero-manager is its single-generation successor. cxcli checkpoints
  the restored-authority, predecessor, target, and successor fingerprints
  together; a stale live predecessor, foreign lifecycle authority, missing
  lineage, skipped generations, identity/spec drift, or receipt drift remains
  recovery-required before webhook publication or Helm dispatch. That
  successor is now written to the rolling service-replay owner. The exact
  previously produced split checkpoint is reconciled only after validating its
  sealed successor receipt, restore lineage, live zero-manager, target binding,
  and current bridge stage/owner/epoch; cxcli atomically checkpoints a bridge-
  bound adoption and removes the duplicate populate pause before any service or
  provider replay. That adoption now distinguishes the receipt-generation
  predecessor from an exact same-generation passive/active rolling sibling and
  seals the role; a sibling is accepted only when its chart, manager render,
  prerequisites, prerequisite UIDs, and manager UID match the post-switch
  successor. Missing or drifted authority still requires the sealed
  controller-gap path rather than falling back to an unbound pause. A later
  target-HA replay now retains the historical source-HA adoption only when the
  bridge authority history proves its unique immediate canonical writer-scale
  successor and the transition timestamp follows the adoption; reordered,
  duplicated, foreign, skipped, or time-inconsistent lineage remains blocked.
- Make the active locked v6 campaign segment the Kubernetes target authority
  for command-start onboarding reanalysis and external node-template
  execution. The executor no longer reaches the static discovery default or
  mistakes a campaign-final config target for the active segment. Upgrade
  output now distinguishes a
  same-minor Soperator/Jail staging segment with `Kubernetes remains X` from
  the separately labeled locked campaign Kubernetes path.
- Stop controller target-version transition failures after best-effort recovery
  proves the exact source bridge authoritative again. Deterministic and
  unclassified failures no longer become automatic continuations, and Ctrl+C
  is propagated after recovery instead of being converted into a retry. The
  shared managed/external continuation driver now also enforces an absolute
  per-boundary-instance deadline that mutable progress timestamps or
  interleaved operations cannot extend.
- Let later external-upgrade segments revalidate login continuity from the
  latest immutable completed segment's exact login node-group and provider
  postcondition when their fresh rolling journal intentionally has no login
  state. Partial current-segment login state remains authoritative and fails
  closed rather than being masked by archived evidence.
- Route later-segment accounting provider continuity reads through the latest
  completed segment's verified target writer and exact SlurmCluster binding
  when the fresh current segment has no accounting authority. Catalog,
  registration, history, and writer checks remain live, and partial current
  accounting/handoff state still fails closed.
- Resume a later external-upgrade controller bridge from the immediately
  completed segment's exact terminal shared-state link. When the predecessor
  consumed cold-backup recovery, cxcli now derives the live successor from its
  checkpointed restore epoch, manifest hash, entry count, operation token, and
  terminal bridge proof instead of rejecting it for not matching the ordinary
  campaign epoch shape; unjournaled symlink targets still fail closed.
- Resume interrupted later external-upgrade segments from the immutable
  campaign-scoped source-to-target SlurmCluster transition when fresh discovery
  proves the exact target-only namespace/name/UID. The executor preserves the
  checkpoint source-report fingerprint through that proof and no longer tries
  to use an earlier segment's backup as current-segment execution identity;
  source reappearance, extra SlurmClusters, or target drift still fail closed.
- Keep later-segment source-identity recovery immutable around the exact
  source/target identities and terminal cleanup transition instead of the
  entire mutable target SlurmCluster resource. Historical whole-resource
  observations are discarded only when every canonical proof field matches;
  identity or cleanup drift still fails closed.
- Make `ext-soperator upgrade --execute --approve` and the applicable shared
  controller/Helm stages of managed `soperator upgrade` complete durably
  accepted convergence boundaries without an operator rerun. A typed shared
  driver retries only checkpointed cxcli-owned transitions, binds retries to a
  progress fingerprint, caps transition count and stall time, and leaves
  approval gates, identity drift, unsupported state, and all other safety
  failures fail-closed.
- Bound every synchronous Nebius SDK request and attached provider-operation
  wait used by the shared Soperator migration adapter. Once a mutation returns
  an exact operation ID, cxcli journals it before waiting; an unfinished wait
  now yields to the typed continuation driver, which re-reads that same
  operation and its live postcondition without redispatching the mutation.
  Mutation-dispatch requests remain one-shot until that ID exists; only
  read-only operation polling uses bounded SDK retries. External and managed
  upgrades share this recovery path, and the bounded transition ceiling is
  large enough to honor the provider operation's declared one-hour stall
  budget at the normal poll interval.
- Declare Click as a direct development dependency because CLI command-coverage
  tests use `click.testing`, and constrain Typer and Click to the latest release
  lines that preserve cxcli's established help and error-rendering contract.
...
- Make managed and external Soperator login-role upgrades use one canonical
  ...
  is Ready behind the stable login Service and its exact EndpointSlice route,
  then releases the UID/resourceVersion-bound hold and continues serial
  replacement without waiting for established SSH sessions. Connections on
  the replaced Pod may drop and reconnect through the stable Service. Serial
  bridge-config restarts likewise require two Ready endpoints but no longer
  inspect sessions. The same behavior is wired through `soperator upgrade` and
  `ext-soperator upgrade`; neither exposes a session-policy or drain-timeout
  override. First-adoption persistent copies that stop all login writers remain
  a separate fail-closed maintenance-window boundary.
- Resume an exact verified in-place login bridge writer refresh after later
  checkpointed accounting, controller-gate, client-propagation, and temporary
  login-surge transitions have restored the target SConfig writer to its
  original positive size. The successor proof binds the target UID, stable
  SlurmCluster spec fingerprint or the receipt-bound immutable historical Helm
  apply fingerprint, surge reconciliation receipt, manager pause, controller
  gate, client-config digest, accounting writer command fence, and
  post-accounting rebind. It also accepts the terminal restored login shape
  only through the exact owner/workload surge-removal receipt, configured
  replica count, and verified writer pulse; any unrelated drift still requires
  the original zero-writer fence.
- Make read-only Slurm routing during multi-login handoff try Running login
  peers in stable order and cache the first successful exact route. A stale
  peer's authentication failure no longer blocks a healthy peer, while every
  mutating Slurm compare-and-set remains bound to its preceding observed route.
- Reload login MUNGE daemons that predate the exact in-place worker Secret CAS
  before the post-OpenMetrics Slurm smoke. cxcli binds the retained bridge and
  target Secret UIDs/effective hashes, target login workload owner/selector and
  Secret projection, two exact bridge Pods, and every login Pod/runtime. A
  failing pre-CAS daemon is recreated serially behind two Ready Service
  endpoints with a UID/resourceVersion-preconditioned delete; each exact
  post-CAS successor must authenticate both journaled HA roles before the next
  Pod. Resume adopts an accepted delete successor, while foreign ownership,
  Secret drift, or an unexplained post-CAS authentication failure remains
  fail-closed without another restart. If one HA bridge StatefulSet ordinal
  self-heals before this reload, resume now records a bounded runtime rebind
  only after proving the unchanged peer, exact owner/revision/node/role/image
  lineage, Ready HA roles, and cluster-wide controller exclusivity; the
  canonical role UID then prevents the same stale-identity failure at later
  authority gates. Multiple or unbound bridge changes remain pending.
- Rebind an exact verified worker MUNGE handoff after the target-authentication
  transition changes only the retained bridge Secret's source-UID provenance.
  Resume requires the old and new material fingerprints to be the verified
  authentication source and desired successor, preserves both Secret UIDs and
  every effective credential hash, and records one canonical rebind. Credential,
  workload, authority, unexplained provenance, or second-successor drift remains
  fail-closed.
- Reconstruct the controller-bridge-owned Helm values before both the
  post-OpenMetrics GPU repair seal and topology replay compare stored and
  desired state. Resume now retains the
  exact zero-replica manager, canonical inert controller command/probes, and
  completed legacy-rootfs adoption record, so an already-current topology is
  adopted without restoring the manager or deleting rollback provenance. The
  adoption successor requires the checkpointed legacy PVC UID, switch timestamp,
  and verified active slot/volume/PVC. Readiness under that retained pause reuses
  the exact prior workload bindings instead of waiting for frozen parent NodeSet
  status; unrelated stored-values drift still fails closed.
- Seal the canonical GPU target-partition pause during the deliberately inert
  target-controller window without issuing a known-invalid Slurm RPC. The new
  proof requires the rolling partition inventory to equal the exact
  controller-bridge gap plus only canonical target partitions, requires every
  target to retain a fingerprint-valid `DOWN` record, records both inventory
  fingerprints and the later target set, and mirrors the target record into the
  Jail and rolling journals. Conflicting or unclassified partition evidence
  remains fail-closed, and post-activation paths continue to live-revalidate.
- Bootstrap the GPU topology drain journal through exact target-bridge RPC
  authority when no in-place provider rollout occurred. This mode binds the
  live release-worker Pod/UID/node/runtime set to the pre-activation fleet,
  topology replay, target-partition pause, controller-gap inventory, and sealed
  client propagation, then proves the two-node bridge runtime before querying
  Slurm. A complete provider group and driver refresh still select the
  provider-specific mode; partial provider evidence remains fail-closed. An
  exact completed zero-drain resume may reseal a stale topology-only authority
  from the same bound proofs without inventing provider history. Controller-gap
  restaging now also recognizes an already-accepted target-version Jail config
  only through its exact target-ha authority, staging receipt, source preimage,
  zero-to-two writer scale, and two-role fingerprint chain; unknown digests
  remain fail-closed. The topology drain queue gate now requires zero active
  allocations while retaining only unallocated `PENDING/PartitionDown` jobs on
  the exact controller-gap or target-pause checkpoint partitions, avoiding a pause-induced
  empty-queue deadlock without admitting active, foreign, or malformed jobs.
- Keep the Jail typed-GRES rollover journal as the canonical writer and reconcile
  its rolling mirror only through an exact forward successor: immutable old/new
  Pod lineage must remain identical, observation metadata cannot regress, and any
  registration proof must bind the same replacements and topology. Exact verified
  rollover and proof replays no longer rewrite timestamps or Pod resource versions,
  preventing checkpoint mirrors from drifting during resume.
- Keep an already verified typed-GRES bridge-successor rebind immutable when only
  the same Pod's Kubernetes `resourceVersion` advances. Cxcli reconstructs the
  exact bounded rollover predecessor from its stored fingerprint, requires
  monotonic rollover, rebind, and live observations plus identical immutable Pod
  fields, and performs no checkpoint rewrite or downstream fingerprint cascade.
- Complete typed-GRES registration after an exact worker rollover when the
  running daemons still consume the checkpointed generic client config. Because
  Slurm rejects both `UNDRAIN` and `RESUME` while `INVALID_REG` remains, cxcli
  now binds the exact target-HA bridge, manager pause, generic handoff,
  typed-GRES successor lineage, Helm/topology proof, and paused-only
  zero-allocation queue before one topology-only ConfigMap compare-and-swap.
  The recovery now resolves the current deployed Helm values against both
  checkpoint fingerprints and accepts a stale local document only when the
  existing checkpoint-owned replay reconstruction reproduces the deployed
  manager pause, canonical controller gate, and Jail-adoption provenance
  exactly. It stages the
  successor into the shared Jail, reconfigures the two-role bridge, proves every
  process-root digest and effective `slurmd -G` typed record/count, and sends one
  UID-bound SIGHUP. Only a valid registration may clear `INVALID_REG` before the
  existing exact-worker `State=RESUME`; no second Pod deletion or unrelated
  config, queue, authority, or registration drift is admitted.
- Rebind a verified worker bridge-config successor through the later exact
  typed-GRES rollover without recreating a Pod. The one-time checkpoint proof
  requires the complete old/new worker set, topology and workload fingerprints,
  identical node/owner/target lineage, and the live target config digest;
  partial, repeated, or foreign successor drift remains fail-closed.
- Advance an already verified inert-controller checkpoint-gap pause receipt
  through that same worker successor lineage by changing only its reconstructed
  worker-set fingerprint. Controller-gap, authority, config, bridge/full
  partition fingerprints, and partition sets remain immutable, and the rebind
  performs no Slurm RPC.
- Rebind an immutable-child handoff's historical bridge-config successor through
  the exact promoted typed-GRES worker lineage without rewriting its historical
  delete proof or deleting another Pod. The one-time journal requires the
  complete per-worker bridge and checkpoint-gap rebinds plus each live Ready,
  zero-restart Pod, node, and target StatefulSet owner; downstream loader
  adoption reuses only that sealed chain.
- Allow a completed target-admission bootstrap to start a new values lifecycle
  only when the deployed Helm values still exactly match its checkpointed
  predecessor and chart, prerequisite, and webhook fingerprints are unchanged.
  This lets post-surge login sizing and first-adoption cleanup advance without
  weakening the fail-closed response to foreign Helm or chart drift.
- Recover an interrupted login-surge restore by reapplying only the exact
  deployed values bound to its existing restore intent, then proving the
  configured owner/workload replica successor. Terminal restore state no longer
  restarts a surge, and the bounded webhook-bridge audit now retains 32 exact
  lifecycles so supported multi-boundary campaigns do not exhaust it at nine.
- Reuse a completed login-surge restore after the separately checkpointed
  OpenMetrics-restore Helm apply supersedes its historical values. The terminal
  path requires the original intent/admission/proof chain, compatible semantic
  drift, exact owner/workload surge-removal receipt and target UID, plus a
  completed OpenMetrics admission lifecycle whose chart and values fingerprints
  equal the currently deployed release; it performs no Helm or replica replay.
  Later reconstructed desired values may differ after direct writer/controller
  reconciliation only while campaign, release, target, requested-version, and
  chart-content identity remain unchanged.
- Reuse the exact completed OpenMetrics Helm successor when GPU Jail-init repair
  re-entry occurs while the handoff is still `restore-intent-recorded`. The
  selector requires the immediately following deployed revision, completed
  admission lifecycle and time window, target UID and live OpenMetrics value,
  matching chart, stored values, rendered target spec, and installed loader
  contract. It performs no Helm or worker mutation and deliberately leaves the
  downstream proof unsealed until login MUNGE reload and post-restore Slurm
  smoke mark the handoff restored.
- Preserve the checkpointed OpenMetrics compatibility state across temporary
  login surge and restore Helm pulses. Resume can recover the one historical
  post-switch values revision bound to its completed admission lifecycle,
  rotate a superseded restore intent only through that exact deployed
  compatibility successor, and finish every valid terminal replica shape
  without repeating the surge/compatibility loop. An exact post-switch
  admission lifecycle interrupted after readiness is now completed before the
  older restore intent is compared, even when live values already converged;
  resume accepts only the deployed lifecycle values after exact fingerprint,
  release-status, OpenMetrics, jail-alias, and maintenance proofs. When the
  paused manager leaves the live SlurmCluster login owner at the surge count,
  restore now accepts only that one defaulted-spec scalar delta, checkpoints a
  UID/resourceVersion/generation and admission-window-bound owner transition,
  and moves the owner before the separate target-workload replica CAS.
- Reconcile the sealed controller-gap client proof after exact login-surge
  restoration removes its highest temporary ordinal. The proof advances only
  when every retained client/config is unchanged, the owner and workload
  restore receipts bind the configured replica count, the target is Available,
  the workload UID/generation is exact, and the removed Pod is absent; any
  other missing client remains recovery-required.
- Recover immutable-child worker rollover when the same campaign's verified
  bridge-config rollout already deleted the captured ownerless Pod and created
  its exact target-owned successor. Recovery requires the zero-job proof,
  ordered delete journal, unchanged node/StatefulSet/NodeSet identity, and the
  current Ready UID/container before copying provenance into the rollover
  journal; incomplete or foreign UID changes still fail closed.
- Let the GPU driver Jail loader repair adopt an already-installed loader
  contract after that exact replacement binding instead of rejecting the valid
  ordering. The no-replay recovery requires the copied bridge-rollout successor,
  unchanged Ready Pod UID/owner, exact StatefulSet template and Helm manifest,
  current chart render, and unchanged stored values; every partial or drifted
  lineage remains pending.
- Verify target worker readiness from the exact NodeSet-owned
  AdvancedStatefulSet while the canonical in-place manager pause intentionally
  freezes parent NodeSet status. The recovery requires the verified pause-rearm
  authority and live zero-replica manager generation, exact target/Helm/owner
  bindings, unchanged replica contract, and a fully observed and Ready workload;
  manager or workload drift still fails closed and the identities are durably
  sealed before continuation.
- Reuse that exact sealed paused-manager workload proof at the downstream GPU
  release gate. A stale parent NodeSet condition is accepted only after the live
  zero-replica manager, target/Helm lineage, NodeSet and workload UIDs, replicas,
  and fully Ready child workload all revalidate; any identity or readiness drift
  remains blocking.
- Stabilize paused-manager worker-readiness authority across resume by preserving
  the original manager-pause verification timestamp and excluding it from the
  authority fingerprint. The one-time timestamp-fingerprint migration requires
  unchanged live manager, target, NodeSet, workload UID, replica, and Ready
  bindings and journals both fingerprints plus the stable authority; any real
  authority drift remains recovery-required.
- Treat an operator-generated zero-replica Jail alias consumer as converged only
  when its generation is observed and current, updated, Ready, and available
  replica counts are all zero. This prevents an intentionally paused
  SConfigController from consuming the shared convergence deadline while still
  rejecting terminating or stale replicas before later consumers are checked.
- Verify post-switch worker `/home` coverage from the exact NodeSet-owned
  AdvancedStatefulSet and count only Ready Pods controller-owned by that exact
  workload. This permits the canonical manager pause to leave parent NodeSet
  status stale while still rejecting target, Helm, owner, UID, generation,
  replica, availability, terminating-Pod, or Pod-controller drift.
- Rebind every serialized Jail Helm login-hold snapshot to the campaign's
  canonical lifecycle before reconciliation. A target-ready release now stays
  terminal across post-switch replay instead of revalidating the deliberately
  removed Pod label and PodUnavailableBudget from an older verified snapshot.
- Fix late in-place bridge-client staging restoring
  `PluginDir=/usr/lib/slurm` after the legacy-rootfs compatibility handoff had
  already selected the target image's multiarch plugin directory. The
  bridge-only config now passes through the same exact compatibility transform
  before any target ConfigMap write, controller-first release, shared-Jail
  writer pulse, worker readiness check, or controller-gap live RPC. Recovery
  checkpoints the predecessor and successor digests plus transform counts,
  preserves the immutable controller-version release proof, archives the exact
  completed writer pulse, and runs a new bounded `0 -> 1 -> 0` pulse for the
  successor. Existing worker Pods that still mount the predecessor are then
  recreated serially only after exact bridge authority, checkpoint-owned DOWN
  partitions, and a fresh zero-active-job proof; each replacement must retain
  its node, target SlurmCluster, and Kruise StatefulSet identities, become
  Ready with a new UID/container identity, and load the successor before the
  next worker. The pre-delete source may be the exact ownerless Pod preserved
  by immutable-child handoff: cxcli accepts it only when its captured Pod UID,
  former source StatefulSet UID, target selector, target StatefulSet successor,
  and NodeSet owner all reproduce the checkpoint. It never treats arbitrary
  ownerless Pods as target-owned. Unrelated config drift or identity changes
  remain recovery-required.
- Fix bridge-client propagation leaving the target ConfigMap newer than the
  shared Jail while the target SConfig writer is deliberately fenced at zero.
  cxcli now checkpoints the exact manager, target, ConfigMap, JailedConfig,
  Deployment, ServiceAccount, and Jail bindings; pulses only that target writer
  from zero to one; verifies every mapped file; and returns it to zero before
  re-reading login clients. The target manager remains paused and the bounded
  writer pulse returns to its exact successor fence. For non-legacy propagation
  where a Pod restart is still required, cxcli proves two Ready Service
  endpoints and restarts stale Pods serially without inspecting SSH sessions.
- Fix the in-place temporary login surge deadlocking after Helm updated the
  target SlurmCluster while the bridge-owned Soperator manager remained
  deliberately paused. Before either surge or restore, cxcli now reasserts the
  inert target controller and proves that every target login client has the
  exact two-controller bridge configuration with live RPC connectivity. It
  then reconciles only the target-owned OpenKruise login StatefulSet through a
  durable UID/resourceVersion/current-replica/generation-fenced scale and
  verifies the unchanged non-replica spec and target SlurmCluster owner. The
  full manager never resumes, so it cannot overwrite bridge client
  configuration or recreate the fenced controller. Foreign ownership,
  unproven bridge propagation, manager/controller drift, or an unbounded
  workload transition remains fail closed.
- Recover the missing in-place ownership marker from a real post-retirement
  target-compatibility checkpoint after config-binding rebind has intentionally
  replaced the original bridge digests. The recovery now requires the exact
  source-retirement proof, target identities, manager pause/restore generation
  chain, config fingerprints, and complete SConfig `0 -> 1 -> 0` writer-refresh
  receipt before reusing the sealed partition pause. Partial or status-only
  intents remain fail closed. The same receipt now authorizes later
  bridge-service manager rearm from the compatibility restore generation,
  including exact campaign-adopted immutable-child pause origins, without
  assuming unrelated Helm pulses preserve adjacent manager generations. Strict
  login rollout readiness is unchanged.
- Treat an exact source-owned zero-controller deferral as a deferral, not a
  started target command-gate transition, after immutable handoff reaches
  `target-children-created`, including after the exact terminal ownership
  receipt is checkpointed. Post-switch resume now proceeds to the ordinary
  manager-pause and target-gate path instead of requiring a pause journal that
  cannot exist before that path starts. Incomplete ownership receipts and any
  real target-gate status without its checkpointed manager pause remain
  recovery-required.
- Recover the legacy-rootfs handoff when a valid single-ConfigMap bridge
  reaches source retirement before the target manager materializes its
  separate ConfigMap and JailedConfig. With the manager paused, the source
  SlurmCluster absent, its orphan cleanup journal exact, and target children
  durably created, cxcli now checkpoints distinct preserved-source and
  target-owned name/UID bindings before the target SConfig pulse. Replay
  revalidates both live target owners and accepts missing source objects only
  after their exact UID/resourceVersion delete intent. Ambiguous writers,
  overlapping identities, foreign owners, or unjournaled disappearance remain
  fail closed.
- Defer the in-place controller bridge Slurm-version transition only until
  source retirement materializes the manager-generated target Slurm ConfigMap.
  The first bridge-client reconciliation now checkpoints an exact release and
  uses the existing simultaneous, backup-protected source-to-target controller
  transition before requiring any target-version login or other client RPC.
  A pre-target-write failure restores and proves the exact source bridge before
  returning pending; exact source/target images and versions, target identity,
  client-config digest, target HA authority, and downgrade prohibition remain
  fail-closed bindings. This prevents a newer login `scontrol` from being
  required to pass a live-RPC gate against older serving controllers. The
  post-provider compute tail idempotently revalidates or reuses target HA
  before singleton handoff.
- Recover a verified in-place controller gap whose older checkpoint predates
  its sealed semantic binding. After target-version bridge HA, exact target
  authority, manager pause, inert controller gate, admission target, workload,
  and partition-pause identities all revalidate, cxcli seals the existing v2
  contract before the config-only client proof. An existing malformed or
  semantically drifted binding remains recovery-required, and replay reuses the
  exact digest without rewriting it.
- Bind immutable-child capture of an already paused Soperator manager to the
  campaign's canonical pause journal. Bridge-gated Helm manifests correctly
  declare zero replicas, so they are no longer required to reconstruct the
  positive pre-pause replica authority. The adoption instead requires the
  exact live Deployment UID, original replicas, non-replica spec fingerprint,
  pause generation, verified state, and zero owned manager Pods; a manager
  paused outside the campaign still uses the stricter deployed-Helm fallback.
  The adoption fingerprint now contains only that stable authority material:
  routine live revalidation may refresh its observational `verified_at`
  timestamp without invalidating an unchanged pause. Exact older adoption
  proofs are canonicalized once after all duplicated live identity, spec,
  generation, replica, ReplicaSet, and Pod-absence fields revalidate; later
  stable-authority hash drift remains fail closed.
  The in-place Jail resume classifier recognizes the same exact campaign or
  manual adoption proof as a valid pause origin, so the intentional
  source-login/target-worker pre-ownership boundary is not misreported as
  mixed rootfs consumers merely because no local pause intent was needed.
- Fix the single-ConfigMap legacy-rootfs bridge rejecting an expected
  SConfig ServiceAccount that was already present in the exact source-owned
  Deployment captured by the pre-target zero-writer fence. The bridge now
  adopts that binding only when the Deployment UID, owner, selector,
  generation, non-replica spec, positive original replica count, and zero
  owned Pods still match the verified fence. Single- and dual-ConfigMap
  bridges use the same durable zero-writer lifecycle and the same exact
  compatibility payload contract before replacing config or binding the
  ServiceAccount, then restore the exact original replica count. A payload
  that already has no `MetricsType` and the canonical legacy-compatible
  `PluginDir` is adopted without a no-op ConfigMap replacement; unexpected
  directive values, unbound ServiceAccounts, and other drift remain fail
  closed.

- Fix external Soperator initial target Helm reconciliation publishing
  fail-closed NodeSet and SlurmCluster webhooks while the bridge-owned manager
  is deliberately scaled to zero. cxcli now fingerprints and applies only the
  exact rendered admission prerequisites, starts the isolated campaign-owned
  webhook bridge, proves its Ready Service endpoint, publishes the exact
  webhook configurations only after service is available, waits for injected
  CA bundles, and keeps that endpoint live through the full Helm apply.
  Every target Helm replay remains on this bootstrap path while controller
  bridge authority fences the manager, even after a target SlurmCluster UID
  exists; target existence is no longer treated as admission readiness.
  A failed webhook head left by an interrupted intent is now authorized only
  after cxcli proves the complete bounded, contiguous chain from one deployed
  predecessor with identical chart, app, values, intent-time binding, allowed
  failure class, and monotonic timestamps. Resume reuses that checkpoint once
  instead of re-authorizing it forever, marks it consumed immediately before
  Helm dispatch, rejects failed revisions as new predecessors, and accepts
  only the exact next deployed revision as verification.
  Foreign resource ownership or crash-resume chart/value drift fails closed,
  and the temporary Deployment is removed by UID after success or failure.
- Fix temporary login surge and restore Helm pulses nesting a rolling-phase
  webhook bridge around a second target-admission bootstrap whose phase-local
  cleanup journal used the same deterministic Deployment name. Admission
  bootstrap lifecycle state and bridge ownership are now decoupled: both
  pulses force one canonical bootstrap, keep their accounting live-boundary
  proof immediately before Helm, and bind all bridge creation, readiness, and
  UID cleanup to the single rolling-migration journal. A stale terminal bridge
  journal in another phase can no longer reject or replace the live canonical
  bridge.
- Revalidate a completed target-admission Helm bootstrap against the currently
  deployed release values before reusing its terminal proof. A later
  checkpointed Helm pulse can legitimately supersede an earlier temporary
  OpenMetrics compatibility override while preserving the same admission
  substrate. cxcli now archives the exact superseded bootstrap proof and runs a
  fresh fail-closed protected Helm pulse when the live release no longer has
  the intended values; exact live values still reuse the completed proof
  without creating another revision. Supersession history is bounded and
  malformed terminal or history state remains fail closed.
- Fix external Soperator accounting finalization calling fail-closed
  SlurmCluster admission after the target Helm bootstrap had removed its
  temporary endpoint. The source and target SlurmDBD command fences now run
  inside a separate durable admission boundary that reuses the exact rendered
  chart/webhook contract, proves the isolated Ready endpoint, keeps the real
  manager paused, and UID-cleans the bridge after success or interruption.
  Resume binds that operation to both exact SlurmCluster UIDs and reruns only
  its idempotent incomplete boundary; a completed target Helm proof is not
  mistaken for accounting admission readiness.
- Fix external accounting finalization waiting for a target MariaDB Pod before
  source retirement while the target manager is deliberately paused. The
  final source dump can now be sealed under both exact SlurmDBD CR fences and a
  live paused-manager/target-Pod-absence proof, with import durably deferred
  until source retirement. After manager restoration, cxcli promotes that
  absence checkpoint to the exact target Pod/StatefulSet/MariaDB/PVC binding
  before schema bootstrap or SQL import; it never starts reconciliation early
  or adopts an unbound database. The deferred-import journal now preserves the
  complete absence proof instead of its incomplete live-database-field projection.
  Resume canonically repairs only that exact pre-retirement projection when
  every other source, target, dump, deployment, and replay binding still
  matches. The canonical post-retirement target-fence waiter now discovers and
  binds the newly manager-materialized database through that same sealed
  absence proof before it validates the target PVC and writer fence, instead
  of rejecting the intentionally missing pre-retirement binding. Later or
  unrelated identity drift remains fail closed.
- Fix in-place post-Jail resume requiring the fixed-name controller workload
  to be target-owned before immutable-child handoff can create that successor.
  While source-bridge HA owns authority, cxcli now accepts only a durable
  source-owned deferral bound to the exact source and target SlurmCluster UIDs,
  paused manager, canonical target command gate, bridge epoch, unchanged
  workload spec, zero desired/status replicas, and absence of every live owned
  Pod. It does not relabel or adopt the source workload. Once target ownership
  exists, the canonical rolling-compute journal reasserts the target
  SlurmCluster gate through a real restored admission window even when Helm
  already rendered the same command, so later workload, Pod-lineage, and
  singleton proofs never depend on synthetic gate acceptance.
- Fix post-switch resume mistaking that source-owned zero-controller deferral
  for an already-started target command gate after the source workload was
  orphan-deleted. While immutable-child handoff has not bound target children,
  the deferral no longer re-pauses the restored manager or requires the absent
  target StatefulSet. A campaign already stopped in that exact gap may reopen
  manager materialization only from the checkpointed Deployment UID,
  non-replica spec, original replica count, and consecutive immutable-restore,
  service-pause, and reopen generations. The premature pause is archived
  before normal target-child reconciliation continues; foreign gate fields,
  ownership, identity, spec, replica, resource-version, or generation drift
  remains fail closed.
- Fix legacy-rootfs health verification falling through to target-owned login
  validation during the intentional immutable-child ownerless gap. In the
  `children-prepared` and `manager-restored` stages only, cxcli may select one
  exact captured Ready login Pod after proving its Orphan/relabel journal,
  target selector plus handoff token, owner absence, source SlurmCluster and
  fixed-name workload absence, and the exact ownerless retained Service and
  Service-owned EndpointSlice backend. Direct exec repeats Pod UID, label,
  owner, and readiness validation before and after every command; any target
  binding, new owner, source survival, selector/endpoint drift, or other stage
  remains fail closed.
- Fix external Soperator resume discovery after a failed target Helm dispatch
  that created no target `SlurmCluster`. The checkpoint-scoped target-handoff
  inventory now accepts the exact immutable source alone only while no target
  UID has ever been bound; a bound target that disappears, an unknown second
  object, or source/target UID drift still fails closed.
- Require an explicit `y` or `n` at every interactive action confirmation that
  previously defaulted to No, including project/config overwrite, Soperator
  campaign and backup-recovery approval, render replacement, generated-resource
  destroy, Jail SFS resize, MK8s destroy recovery, and default-No component
  wizard decisions. Blank input now reprompts instead of declining the action;
  existing `q`/`qq` wizard controls remain available. For `ext-soperator
  onboard`, an explicit `n` is a clean user cancellation instead of passing the
  message-less wizard sentinel to the generic handler and printing a blank
  `ERROR:`. The proposed campaign and discovery report remain unaccepted and
  unwritten; any base project/config scaffold created before the prompt is
  retained.
...
- Recover a missing in-place ownership-handoff marker only from the exact
  ...
  partition pauses must all agree. At `target-children-created`, the exact
  source-owned zero-controller deferral is accepted only for this recovery and
  only while its bridge epoch and full pause proof remain sealed. Resume then
  reconciles the independently proven temporary login surge before reaching the
  canonical protected-session gate instead of waiting for a held OpenKruise
  rollout and reporting a generic accounting-readiness failure; strict rollout
  and source-retirement checks remain unchanged.
...
- Drive an approved external Soperator v6 campaign to completion in one
  ...
  config and journal authority, and continues. Typed, durably accepted
  cxcli-owned convergence boundaries also continue automatically; approval
  gates, drift, unsupported state, other safety failures, explicit stop points,
  and interrupts still stop immediately for exact-command resume.
...
- Classify GPU topology restoration from completed campaign segments before
  ...
  before any worker provider mutation. The topology-specific client-config
  canonicalizer now leaves a first-segment generic handoff untouched when no
  restore, apply intent, apply proof, or successor exists; any partial topology
  lifecycle still fails closed on its missing verified predecessor.
...
- Retry target Soperator Helm dependency preparation only for bounded transient
  ...
  before the checkpointed phase stops. HTTPS port `443` is no longer
  misclassified as an HTTP client status; deterministic chart or status-shaped
  client errors still fail immediately, except retryable `408`, `425`, and
  `429`. Exact declarative Soperator CRD server-side applies now also retry the
  bounded Kubernetes responses that say a timed-out request may still be
  processing or did not complete before its context deadline, while non-apply
  mutations and authorization failures remain fail-closed. Multi-document CRD
  bundles are prevalidated and applied per named document, and an exhausted
  ambiguous timeout is adopted only after a read-only GET proves the exact
  declared, non-deleting, UID/resourceVersion-bound `Established` CRD.
...
- Simplified external-upgrade MK8s TUI generation counters to
  `current/updating/not-started`. The formatter and Rich table styling share
  one canonical label tuple, while the counters retain their Nebius
  provider-generation meaning independently of the locked campaign target.
