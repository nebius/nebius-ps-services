...
- Interactive wizard phase prompts still use `q` to stop the wizard, while answering `n` to a component-level `Configure '<component>' component fields now?` prompt skips only that component and continues with the remaining selected components. Field prompts use `q` to revisit the previous answered field, and `qq` stops the wizard entirely.
...
- `helm` is used for `validate-sources` and other live Helm chart source/metadata validation paths. When source validation is enabled, `create` checks for it before prompting for tenant/project/client details. It is not required for the normal `render`, `deploy`, `destroy`, `flux apply`, or `flux destroy` flow.
- `aws` CLI is used for `terraform unlock`.
- `git` is used for `bootstrap-ci`, local `origin` auto-detection, and Helm chart sources that resolve from Git tree URLs.
...
- `create <deployments-root>`
  ...
  - In interactive mode, phase prompts still use `q` to stop the wizard; field prompts use `q` to revisit the previous answered field and `qq` to stop the wizard. The wizard prints visible `Infra` and `Apps` section separators and echoes each answered field as `Selected <path> = <value>` with secret-like fields redacted, so operators can scan the terminal history before reviewing the saved `config.yaml`. The command still writes the current project config and warns only when required fields remain unresolved.
