...
- The separate default-off VM standalone collector branch is `deploy.observability.vm.collector.*`. When `deploy.observability.enabled=true`, `deploy.observability.vm.collector.enabled=true`, and the selected VM has `inputs.service_account_id`, cxcli installs the public `nebius-o11y-agent` deb package from the canonical public Artifactory APT repo at `https://artifactory.nebius.dev/artifactory/nebius-o11y-agent` plus a Prometheus agent companion on supported Ubuntu-family boot images. That stack uses the VM metadata token at `/mnt/cloud-metadata/token`, pushes journald logs to Logging through the public agent gRPC endpoint, and remote-writes host metrics to Monitoring through the public Prometheus Remote Write endpoint. It is intentionally separate from the built-in Monitoring agent path so the customer can choose between Nebius-managed VM telemetry and cxcli-managed public ingest without conflating them.
...
- Read-side Grafana wiring is automated but still kept out of `config.yaml` secrets. The bundled `grafana` app uses the maintained Grafana community Helm repository, the official `grafana/grafana` image, and pinned chart/image/dashboard versions from `component_sources.yaml`; operators can change the bundled dashboard IDs there when they need a different Grafana.com dashboard set. During `deploy`, cxcli creates or reuses Kubernetes Secrets for the Grafana admin password and the Observability read token, ensures a project service account with `viewer`, issues an Observability static key when the token Secret is missing, and mounts that token into Grafana datasource provisioning. The service-metrics datasource is named `Nebius Services` to match the upstream Nebius dashboard variable default while retaining the stable `nebius-service-metrics` UID for links and automation. The generated deploy report now separates public write endpoints, public read endpoints, and live Grafana links. After the Envoy Gateway `Gateway` has an address, the report includes clickable Metrics, Logs, Traces, and dashboards links plus the `kubectl` command to retrieve the admin password. VM paths still report both the built-in agent's managed-write note and, when the standalone collector is enabled, the public write endpoints that collector actually uses. cxcli keeps static tokens and Grafana passwords out of repo config and generated Flux manifests.
