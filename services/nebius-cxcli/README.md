...
- `nebius-cxcli create <target-path>`: create one name-derived tenant/project folder scaffold under a deployments root; reruns overwrite only after confirmation.
...
- When built-in cluster targets such as `mk8s` are enabled, each enabled app chart binds to one target with `apps.charts[].target_ref`, using the target cluster's `instance_id`.
...
- Nebius API credentials/profile are required for commands that talk to Nebius APIs such as `validate`, `quota-check`, `quota-request`, `render`, `deploy`, `discover`, and `auth`.
...
- Observability is now a first-class deploy contract under `deploy.observability.*` and stays disabled by default. `config.yaml` keeps the customer-facing roots to `infra`, `apps`, and `deploy`; the observability switch lives under `deploy` because cxcli materializes the requested telemetry behavior during render/deploy into infra labels and app chart values. That switch controls whether cxcli deploys/configures collectors; it does not create the Nebius Monitoring/Logging/Tracing service endpoints themselves.
- `create` and config normalization keep that customer contract scoped to the enabled infra set. MK8s-only projects get `deploy.observability.kubernetes.*`, VM-only projects get `deploy.observability.vm.logs.*` plus `deploy.observability.vm.collector.*`, mixed projects get both, and unrelated project-scope defaults such as MK8s GPU deploy validations are omitted when no MK8s component is selected.
- When `deploy.observability.enabled=true` and an MK8s component is enabled, cxcli auto-enables the bundled `nebius-observability-agent` Helm release and materializes the customer-facing chart values from `deploy.observability.kubernetes.{logs,metrics,traces}.*`. In the interactive wizard, that auto-selection notice appears while answering the `deploy.observability.*` prompts; the later `Configure 'nebius-observability-agent' component fields now?` prompt only asks whether to customize chart values, and answering `n` keeps the selected app with defaults. Those fields are chart-native `values.config.*` settings on the Nebius Observability Agent for Kubernetes, but cxcli keeps them on the deploy contract so the wizard, config review, and auto-enable path stay cluster-oriented instead of Helm-release-oriented. When multiple MK8s targets are enabled in one project, cxcli now materializes that managed collector config into every target-bound `nebius-observability-agent` chart row instead of only the first matching app id.
- The bundled `mk8s` wizard exposes that same contract directly: `deploy.observability.enabled`, the main logs/metrics/traces toggles, and the `collect_k8s_cluster_metrics` switch are guided prompts; advanced namespace exclusions and agent self-scrape/self-log toggles stay available in `config.yaml` but are hidden from the interactive wizard by default. Because the cxcli-managed deploy contract is default-off, cxcli treats cluster-level K8s metrics as part of the enabled baseline once the operator turns this path on. This MK8s contract is separate from the preinstalled Nebius Monitoring agent that already runs on the cluster's node VMs and sends VM/node system metrics through Nebius-managed ingestion to Monitoring dashboards and read APIs.
...
- GPU monitoring stays aligned with NVIDIA's current split: cxcli still requires `dcgmExporter.enabled=true`, but it does not invent a `ServiceMonitor` dependency. The bundled GPU Operator exposes the `nvidia-dcgm-exporter` service with `prometheus.io/scrape: "true"`, so the Nebius observability agent can discover it through its normal Prometheus-annotation path without Prometheus Operator CRDs. On Nebius driverful GPU images, nodes can default to `nvidia.com/gpu.deploy.operands=false`, which leaves GPU Operator operand DaemonSets, including `nvidia-dcgm-exporter`, at desired count `0`. When `deploy.observability.enabled=true`, Kubernetes metrics are enabled, and the GPU Operator app is selected on the `nebius_image` stack, cxcli now materializes the catalog-owned GPU node labels required to run only DCGM Exporter plus the GPU Operator validator while keeping GPU Operator device-plugin/GFD disabled so they do not duplicate the Nebius-managed path. `deploy` also reconciles those labels onto existing live GPU nodes using the catalog-owned selector, because MK8s node-group label updates apply cleanly to future nodes but may not relabel already-running Node objects. The deploy report still tells operators to verify live `nvidia-dcgm-exporter` endpoints after deploy, because the final signal is runtime readiness, not config presence alone.
- Read-side Grafana wiring stays intentionally out of `config.yaml` because the public Nebius Grafana/data-source path needs project-scoped read credentials. The generated deploy report shows the public read endpoints that match the selected signals: Nebius service metrics at `https://read.monitoring.api.nebius.cloud/projects/<project-id>/service-provider/prometheus`, user-ingested metrics at `https://read.monitoring.api.nebius.cloud/projects/<project-id>/prometheus`, concrete Prometheus federation bucket URLs for the service buckets that apply to the deployment, Loki logs at `https://read.logging.api.nebius.cloud/projects/<project-id>`, and Tempo traces at `https://read.tracing.api.nebius.cloud/projects/<project-id>/tempo`. In the service-metrics Grafana URL, `service-provider` is a literal path segment. The report also renders a Grafana read data-source section with the Prometheus, Loki, and Tempo data-source types, real read URLs, server/proxy access mode, a note that opening datasource base URLs directly can return `404` because Grafana calls API subpaths below them, auth-required API probe URLs for direct reachability checks, and the `Authorization: Bearer <observability static token or IAM token>` header guidance. VM paths report both the built-in agent's managed-write note and, when the standalone collector is enabled, the public write endpoints that collector actually uses. cxcli keeps those credentials out of public repo config.
...
- When tenant/project/region context is available, the GPU preset and InfiniBand fabric prompts also query the live Nebius Capacity Dashboard `resource-advice` surface for that exact GPU shape. For cluster-capable presets, those live rows are the source of truth for the offered fabric names, immediate availability annotations, and the recommended default, without forcing the optional fabric value into `config.yaml`
...
- `apps.charts[]`: `id`, `group`, `enabled`, optional `target_ref`, `repo`, `version`, `namespace`, `release-name`, `values`
...
- `render`, `deploy`, `terraform apply`, `flux apply`, `flux bootstrap`, and `report` refresh that report artifact for the active project instead of leaving the starter placeholder behind.
...
- The bundled `mk8s` flow now exposes source-driven observability controls under `deploy.observability.*`. When enabled, cxcli auto-enables the bundled `nebius-observability-agent` chart, persists the project-facing logs/metrics/traces toggles in `config.yaml`, and keeps the auth path public-safe by relying on the chart's metadata/IAM token-file default instead of asking operators to paste secrets into repo config. In wizard mode, the auto-enable notice is emitted as soon as those observability answers make the chart required; the later app prompt only controls whether to customize values or keep defaults.
- The bundled `mk8s` flow now exposes source-driven GPU validation controls under the project-facing `deploy.validations.mk8s_gpu.*` contract. In wizard mode, when `gpu_enabled=true`, operators can enable or disable operator-readiness, GPU-visibility, and NCCL checks and tune `gpu_visibility.max_nodes` plus `nccl.max_nodes`; the RDMA bus-bandwidth threshold remains part of the same project contract, but the wizard only exposes that threshold field when the current MK8s shape is actually on the GPU-cluster / fabric path. The catalog still owns the defaults in `components.infra.mk8s.cli.gpu.validations`, but the chosen per-project values now persist in `config.yaml` as deploy settings instead of pretending to be Terraform inputs.
- If an operator leaves `deploy.validations.mk8s_gpu.nccl.enabled=true` on a non-cluster/Ethernet-only GPU shape, cxcli now warns that the run will use Socket/TCPIP rather than InfiniBand / GPUDirect-RDMA. On 1-GPU shapes that warning is explicit that the result is degraded and not representative of a production distributed-training environment, but the NCCL benchmark still runs so operators can compare the measured bandwidth with RDMA-capable environments.
...
- Reconcile/apply commands are sequentially idempotent or convergent for the same target: `render`, `deploy`, `terraform apply`, `flux apply`, `flux bootstrap`, `report`, `bootstrap-ci`, `auth --create`, and `auth --bootstrap-ci`.
...
- `quota-request <config.yaml>`
  ...
  - Requests only the constraining tenant/project scopes; unresolved live limits and estimator coverage gaps are still reported, but they are not auto-requested.
...
- `validate-generated <generated-dir>`
  ...
  - Reports visible phases for strict readiness, MK8s preflight, live quota/capacity, backend auth/bootstrap, Terraform validation, Flux manifest validation, and optional portability enforcement.
...
- Deploy-time MK8s GPU checks are configured per project under `deploy.validations.mk8s_gpu.*`. Use `--skip-validations` to bypass all of them for one run, or repeat `--skip-validation <kind>` for targeted one-run skips such as `nccl` or `gpu-visibility`; those CLI flags do not rewrite `config.yaml`. If a validation fails before its normal report is complete, `deploy` still writes a failure JSON report so the combined deploy summary shows `FAIL` with the underlying error instead of `NOT RUN`.
...
nebius-cxcli flux destroy /path/to/generated --yes
nebius-cxcli report /path/to/config.yaml
nebius-cxcli destroy /path/to/config.yaml --yes
...
- Positional target quick map:
  ...
  - `component`, `validate`, `quota-check`, `render`, `bootstrap-ci`, `deploy`, `destroy`, `report`, `email`: pass the project `config.yaml`.
...
- `component add <config.yaml>`
  ...
  - Repeating the same component id adds another instance. You can also request an explicit instance id with `<component-id>@<instance-id>`, for example `object-storage@logs-bucket` or `mk8s@training-cluster`.
...
- `create <deployments-root>`
  ...
  - After writing the resulting `config.yaml`, `create` runs the internal warning-only post-create validation by default. Use `--no-validate-config` only when you intentionally want to skip that post-write check.
  - `create` also runs a best-effort live Nebius quota check for bundled infra components and warns when the selected shape already exceeds current quota. GPU quota dimensions come from the Capacity Dashboard for the selected platform/preset/fabric shape. It does not block render or config edits, and confirmed shortages now print the exact `quota-request <config.yaml>` follow-up command.
...
- `bootstrap-ci <config.yaml>`
  ...
  - Generated workflows validate changed bundles with `nebius-cxcli validate-generated --portable` before Terraform plan/apply. That generated-bundle validation now includes the same strict readiness, MK8s preflight, and live quota/capacity gate used by local deploy preflight.
...
- `destroy <config.yaml>`
  ...
  - Example: `nebius-cxcli destroy ~/deployments/tenant-name-example/project-name-example/config.yaml --yes`
- `report <config.yaml>`
  - Rewrites `generated/inventory/deploy-report.md` from the rendered bundle resolved from sibling `generated/`.
  - Example: `nebius-cxcli report ~/deployments/tenant-name-example/project-name-example/config.yaml`
- `email [config.yaml]`
...
`bootstrap-ci <config.yaml>` remains the full CI workflow bootstrap command and can still perform complete CI auth bootstrap/sync for that config. The generated customer workflow is artifact-driven: it watches and deploys only canonical `<tenant-folder>/<project-folder>/generated/**` paths. Re-running the command automatically reconciles the CLI-managed workflow file to the latest template, always reconciles local SMTP settings into the matching GitHub Environment, and uses `--github-repo` only as an explicit override when repo auto-detection is wrong or unavailable.
