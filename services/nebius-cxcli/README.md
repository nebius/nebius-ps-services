...
- `--to-k8s-version`: target Kubernetes `major.minor` version for onboarding
  analysis and accepted external MK8s node-template work. When onboarding
  selects `upgrade-external-node-template`, non-interactive runs must pass this
  explicitly; cxcli does not silently choose latest Kubernetes. Use the next
  provider-supported minor hop, for example `1.31 -> 1.32` before `1.33`.
