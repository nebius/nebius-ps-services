...
- Observability is now a first-class project contract under `observability.*` and stays disabled by default. That project switch controls whether cxcli deploys/configures collectors; it does not create the Nebius Monitoring/Logging/Tracing service endpoints themselves.
- `create` and config normalization keep that customer contract scoped to the enabled infra set. MK8s-only projects get `observability.kubernetes.*`, VM-only projects get `observability.vm.logs.*` plus `observability.vm.collector.*`, mixed projects get both, and unrelated project-scope defaults such as MK8s GPU deploy validations are omitted when no MK8s component is selected.
- When `observability.enabled=true` and an MK8s component is enabled, cxcli auto-enables the bundled `nebius-observability-agent` Helm release and materializes the customer-facing chart values from `observability.kubernetes.{logs,metrics,traces}.*`. Those fields are chart-native `values.config.*` settings on the Nebius Observability Agent for Kubernetes, but cxcli keeps them on the project contract so the wizard, config review, and auto-enable path stay cluster-oriented instead of Helm-release-oriented. When multiple MK8s targets are enabled in one project, cxcli now materializes that managed collector config into every target-bound `nebius-observability-agent` chart row instead of only the first matching app id.
- The bundled `mk8s` wizard exposes that same contract directly: `observability.enabled`, the main logs/metrics/traces toggles, and the `collect_k8s_cluster_metrics` switch are guided prompts; advanced namespace exclusions and agent self-scrape/self-log toggles stay available in `config.yaml` but are hidden from the interactive wizard by default. Because the cxcli-managed project contract is default-off, cxcli treats cluster-level K8s metrics as part of the enabled baseline once the operator turns this path on. This MK8s contract is separate from the preinstalled Nebius Monitoring agent that already runs on the cluster's node VMs and sends VM/node system metrics through Nebius-managed ingestion to Monitoring dashboards and read APIs.
- The built-in VM Monitoring agent path is always present for Nebius Compute VMs. `observability.enabled` does not install or remove that built-in agent and does not change how VM service metrics are ingested. The platform-native branch remains `observability.vm.logs.enabled` plus optional `observability.vm.logs.systemd_units`, which tells cxcli whether to turn on the supported Compute journald collection labels for systemd services.
- The separate default-off VM standalone collector branch is `observability.vm.collector.*`. When `observability.enabled=true`, `observability.vm.collector.enabled=true`, and the selected VM has `inputs.service_account_id`, cxcli installs the public `nebius-o11y-agent` deb package from the canonical public Artifactory APT repo at `https://artifactory.nebius.dev/artifactory/nebius-o11y-agent` plus a Prometheus agent companion on supported Ubuntu-family boot images. That stack uses the VM metadata token at `/mnt/cloud-metadata/token`, pushes journald logs to Logging through the public agent gRPC endpoint, and remote-writes host metrics to Monitoring through the public Prometheus Remote Write endpoint. It is intentionally separate from the built-in Monitoring agent path so the customer can choose between Nebius-managed VM telemetry and cxcli-managed public ingest without conflating them.
...
- Nebius docs say omitting `observability.vm.logs.systemd_units` should collect all systemd units. cxcli keeps that documented default, but for deterministic smoke tests and narrower blast radius it is better to set explicit units such as `["sshd.service"]` or your own service unit names.
...
- The bundled `mk8s` flow now also exposes source-driven observability controls under `observability.*`. When enabled, cxcli auto-enables the bundled `nebius-observability-agent` chart, persists the project-facing logs/metrics/traces toggles in `config.yaml`, and keeps the auth path public-safe by relying on the chart's metadata/IAM token-file default instead of asking operators to paste secrets into repo config.
...
- `quota-check <config.yaml>`
  ...
  - GPU quota dimensions are centralized on the live Nebius Capacity Dashboard `resource-advice` surface for the exact platform + region + preset + fabric shape. cxcli no longer overlays a separate Capacity Block Group-specific GPU path or a synthetic `compute.gpucluster.count` check.
