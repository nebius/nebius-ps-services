...
- The bundled `soperator` app is a target-scoped Helm chart for Nebius
  ...
  GPU MK8s targets, cxcli's existing MK8s
  GPU policy auto-enables NVIDIA GPU Operator and keeps the GPU Operator DCGM
  Exporter path authoritative; GPU fabric / InfiniBand shapes still auto-enable
  Network Operator through the existing GPU policy. The Soperator chart does
  not deploy the upstream Soperator DCGM Exporter chart.
...
- Soperator uses the catalog-owned `soperator_nodesets_profile` (`nebius-gpu-v1` by default) to seed generic MK8s node groups instead of hardcoded Python dictionaries. Built-in profile choices are `nebius-cpu-v1`, `nebius-gpu-v1`, and `nebius-mixed-v1`. The mixed profile creates separate homogeneous Slurm worker NodeSets, `worker-cpu` and `worker-gpu`, then maps Slurm partitions to those NodeSets. The app wizard also exposes `values.partitionProfile`: `shape-default` keeps the selected worker-shape partitions, while `with-debug-long` materializes extra `debug` and `long` policy partitions into the rendered `SlurmCluster`. Slurm features such as `h100`, `a100`, `highmem`, or `infiniband` remain NodeSet `nodeConfig.features`, not partition fields. The default shard size is 100 MK8s nodes, so a 1000-node worker pool becomes 10 host pools and a 3000-node pool becomes 30 host pools. NFS stays a VM-based sibling infra component, not an MK8s node group.
