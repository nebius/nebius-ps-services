...
- App components come from Helm chart sources. Flux and Helm own app reconciliation. cxcli apps are installed only onto enabled MK8s targets in the same project; app-only and external-cluster bundles are not supported.
...
- Root blocks:
  ...
  - `compute.boot_disk_defaults`: shared cxcli policy for Compute-backed boot-disk recommendations. MK8s, VM, SSH jump host, and WireGuard VPN gateway components consume this single policy.
...
- `components.infra.<component-id>`:
  ...
  - `status`: optional Nebius deployment-status watcher metadata. When present, `status.kind` is required, `status.parent_input` defaults to `parent_id`, and `status.name_input` defaults to `name`. Compute instance watchers report public or private IP readiness, so private-only VMs do not look stuck just because they do not have a public IP.
...
- That same bundled `mk8s` profile still suppresses the advanced passthrough maps `inputs.mk8s_cluster_overrides`, `inputs.mk8s_cpu_node_group_overrides`, and `inputs.mk8s_gpu_node_group_overrides`, but it now surfaces first-class MK8s boot-disk choices in the interactive flow once the effective node-group shape is known. cxcli pre-fills the boot-disk size from the first matching ordered `compute.boot_disk_defaults` rule for the selected preset resources, shows guided disk-type choices with settings-owned Nebius performance/resiliency labels, and still lets operators override the final `inputs.<cpu|gpu>_nodes_boot_disk_*` values directly in `config.yaml` when they need exact storage sizing. VM, SSH jump-host, and WireGuard VPN gateway wizard profiles use the same policy for `inputs.boot_disk_size_gib` and `inputs.boot_disk_type`; the interactive prompt refreshes those values immediately after platform/preset selection so the shown size is not the raw nullable Terraform default. If no rule matches a resolved shape, cxcli fails fast so maintainers update the shared policy instead of relying on a hidden fallback. GPU boot-disk inputs are hidden and pruned when `inputs.gpu_enabled` is not true, so CPU-only clusters do not carry inactive GPU storage settings. The guided prompt intentionally offers the recommended SSD-backed types declared in `compute.boot_disk_defaults.disk_types`; other module-supported values such as `NETWORK_HDD` remain available only through direct config edits with explicit sizing. For VM, SSH jump-host, and WireGuard VPN gateway boot disks, the wizard prompts `inputs.boot_disk_deletion_protection` with default `false`; after selecting `NETWORK_SSD_NON_REPLICATED` or `NETWORK_SSD_IO_M3`, it also prompts `inputs.boot_disk_encryption_enabled` with default `false`. Public MK8s node-group `boot_disk` currently exposes size/type only, so optional SSD NRD / SSD IO M3 encryption cannot be toggled from cxcli today. For current disk characteristics and pricing, see [Types of storage volumes in Compute](https://docs.nebius.com/compute/storage/types) and [Compute pricing in Nebius AI Cloud](https://docs.nebius.com/compute/resources/pricing).
...
- At the end of `deploy`, cxcli prints a compact `Deployment summary` footer with three sections: validation PASS/FAIL, copy-paste commands such as `wg-quick up/down`, SSH `ProxyJump`, and GitOps bootstrap follow-ups, and important paths such as `generated/inventory/deploy-report.md`, the generated bundle, the manifest, and validation JSON reports.
...
- Ongoing GPU health and performance monitoring is intentionally outside that fast deploy loop. NVIDIA positions DCGM Exporter as the Kubernetes telemetry path for Prometheus/Grafana, while deeper DCGM diagnostics are invasive administrator workflows with different run levels and runtimes, so cxcli does not fold those checks into every local `deploy`.
  ...
  - Each watcher resolves its `parent_id` and `resource_name` from the enabled component row in `config.yaml`, using the catalog's `status.parent_input` and `status.name_input` paths. For example, `mk8s` reads `inputs.parent_id` plus `inputs.cluster_name`, `managed-postgresql` reads `inputs.parent_id` plus `inputs.name`, `object-storage` reads `inputs.parent_id` plus `inputs.name`, SSH/WireGuard public VM wrappers read `inputs.parent_id` plus `inputs.name`, and `mysterybox` expands the canonical `inputs.secrets` list into one watcher per configured secret `name`.
...
- Positional target quick map:
  ...
  - `component`, `validate`, `validate-dashboards`, `quota-check`, `quota-request`, `render`, `deploy`, `bootstrap-ci`, `wireguard`, `ssh-jumphost`, `destroy`, `email`: pass the project `config.yaml`.
...
- `component add <config.yaml> [component-selector...]`
  ...
  - Non-interactive mode accepts component selectors directly: `<component-id>`, `infra:<component-id>`, `apps:<component-id>`, `all`, `none`, or `<component-id>@<instance-id>`. For example: `nebius-cxcli component add /path/to/config.yaml managed-postgresql object-storage --no-interactive`.
  - Repeating the same selector is a no-op when that exact row is already enabled. To add another infra instance, request a new `config.yaml` `instance_id` with `<component-id>@<new-instance-id>`, for example `object-storage@logs-bucket` or `mk8s@training-cluster`. For app charts, the explicit suffix is the cluster target id and becomes `apps.charts[].instance_id`; duplicate `<chart-id>@<target-id>` installs are skipped. When more than one cluster target is enabled, app charts must be added with an explicit target instance, for example `n8n@cluster2`.
...
- `wireguard`
  ...
  - Requires the current `config.yaml` and sibling `generated/` bundle to
    contain the same selected component instance.
