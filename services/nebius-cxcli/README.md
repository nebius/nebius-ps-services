...
- `nebius-cxcli create <deployments-root>`: create one name-derived tenant/project folder scaffold under a deployments root; reruns overwrite only after confirmation.
...
- `nebius-cxcli quota-request <config.yaml>`: plan quota requests for the confirmed live shortages in one project config. Automatic submission works only on the internal Nebius network for Nebius employees; everyone else gets the exact manual follow-up targets to request in the web console.
...
- `create` is the bootstrap path for one name-derived tenant/project folder with `config.yaml` plus the generated-folder scaffold under a deployments root.
- `create` still prompts for and accepts Nebius `tenant_id` / `project_id`, not display names. After those IDs are validated, the CLI resolves the Nebius tenant/project names, normalizes them into filesystem-safe folder segments, and writes the starter config to `<deployments-root>/<tenant-name>/<project-name>/config.yaml`.
- Runtime identity still comes from `client_info.nebius.{tenant_id,project_id}` inside the config and other runtime surfaces; the folder names are locators only.
- Re-running `create` against an existing resolved project folder for the same `tenant_id`/`project_id` no longer reconciles existing component state. Interactive runs warn and ask for overwrite confirmation; non-interactive reruns require `--force`.
...
- When `create` overwrites an existing resolved project folder, it recreates that one folder from scratch, restarts client-info prompts from the normal create defaults, and rebuilds infra/apps selections plus component values from the current create inputs.
...
- `component_sources.yaml` defines reusable component types; `config.yaml` stores enabled component instances with infra rows identified by `instance_id` and app rows identified by `id` plus `instance_id`, so reusable types can be added more than once when the target identity is unique.
- For infra rows, `instance_id` is the logical resource instance and Terraform output namespace. For MK8s, new wizard-created rows use `inputs.cluster_name` as that target id when it is available, so operators see targets such as `cluster1` and `cluster2`.
- For app rows, `id` names the chart type and `instance_id` names the chart instance. When built-in cluster targets such as `mk8s` are enabled, each enabled app chart binds to one target by setting `apps.charts[].instance_id` to the target cluster id, so the full install identity is clear as `<chart-id>@<target-id>`, for example `nvidia-gpu-operator@cluster2`. A target-bound chart can be enabled once per chart id and target; add the same chart to another cluster with `<chart-id>@<target-id>`. Authored `config.yaml` does not use `apps.charts[].target_ref`; any internal generated `target_ref` is derived from and must equal the same target `instance_id`.
...
- `component add` is idempotent for already-enabled exact selectors. Repeat selectors are skipped; use `<component-id>@<new-instance-id>` when you intentionally want another infra or app-only row.
- If `component add` introduces the first built-in cluster target into an app-only config, cxcli target-binds each existing single app row by setting `apps.charts[].instance_id` to that target id. Ambiguous duplicate app rows still fail before write and must be edited explicitly.
...
- `component remove` removes target-bound app rows and matching `deploy.targets[]` settings when their cluster target is removed, then blocks only changes that would still leave unresolved component bindings or dependency breakage in `config.yaml`.
- `create` validates `component_sources.yaml` by default (`--no-validate-sources` to skip) and checks required source-validation tools such as `helm` before starting identity prompts.
- `create` also runs the internal warning-only post-create validation against the resulting `config.yaml` by default (`--no-validate-config` to skip).
- `create` now performs a best-effort live Nebius quota assessment for bundled infra components and warns when current tenant/project quota is already insufficient, without blocking the config workflow. This is a post-write warning path, not a reservation, not a quota request, and not a wizard-selectable deploy gate.
- `create`, `render`, and `bootstrap-ci` auto-manage one deployments-root `.gitignore` block when target path is inside a git repo (keeps `config.yaml` and deployable generated artifacts versioned while ignoring Terraform transient/runtime files and generated tfvars only; it does not try to become a generic developer `.gitignore` for things like coverage files or packaged chart archives). That root-level block covers every `<tenant-folder>/<project-folder>` under the same deployments root. Passing a nested directory below an existing cxcli-managed deployments root now fails fast instead of creating a second nested managed `.gitignore`.
- App dependency resolution from Helm `Chart.yaml` metadata.
- Interactive optional-wizard prompts use one navigation model: `q` goes back to the previous wizard step, and `qq` stops the wizard immediately while preserving the current in-memory config. In TTY list and checkbox prompts those controls are direct key shortcuts, not selectable Back/Quit rows. At the first wizard step, `q` asks whether to exit instead of looping on the same prompt. Answering `n` to a component-level `Configure '<component>' component fields now?` prompt still skips only that component and continues with the remaining selected components.
...
- Flux output stays flat per rendered target: app-only bundles render flat files under `generated/flux`, while bundles with built-in cluster targets render one flat subtree per target under `generated/flux/targets/<target-id>`; legacy `apps/` and `sources/` nesting is still unsupported.
- `validate` now always runs the deployment-readiness stack: strict readiness checks, MK8s preflight, and fail-fast confirmed live quota/capacity checks. `create` still uses the internal warning-only post-write validation path so config bootstrap does not stop on quota.
...
- Generated-bundle CLI commands recreate ignored `generated/infra/terraform.auto.tfvars.json` from the committed manifest before Terraform runs, so deployable repos and generated workflows do not need to version that sensitive duplicate file. The source chain is `config.yaml` -> `render` -> `generated/nebius-cxcli-manifest.json` -> `nebius-cxcli deploy` / `nebius-cxcli terraform plan` / `nebius-cxcli terraform apply` -> recreated Terraform tfvars. Use `nebius-cxcli terraform plan <generated>`, `nebius-cxcli terraform apply <generated>`, or `nebius-cxcli deploy <config.yaml>` from a fresh checkout; raw `terraform apply` inside `generated/infra` needs that ignored file and the same backend/auth context restored first.
- `deploy`, `destroy`, and `email` take project `config.yaml` as the CLI entrypoint and resolve sibling `generated/` automatically. `destroy` is the project-wide destructive path: it tears down all rendered resources represented by that generated bundle/runtime snapshot rather than rerendering from post-render source edits. Lower-level generated-bundle commands keep narrower artifact boundaries: `validate-generated` accepts any path under `generated/`, `terraform *` accepts `generated/` or `generated/infra/`, and `flux *` accepts `generated/` or `generated/flux/`.
...
- Nebius API credentials/profile are required for commands that talk to Nebius APIs such as `validate`, `quota-check`, `quota-request`, `render`, `deploy`, and `auth`.
...
- If the target MK8s control-plane endpoint is private, `deploy`, `destroy`, `flux apply`, `flux destroy`, and `flux bootstrap` require an existing private network path to that cluster.
...
- `terraform` is used for Terraform-backed validation, render lockfile generation, `terraform plan`, `terraform apply`, `terraform unlock`, and backend-backed Terraform output lookups. For those supported command paths, `nebius-cxcli` downloads Terraform into its local cache when it is missing instead of requiring a system-wide install.
- `flux` is used for `flux bootstrap`. For that supported command path, `nebius-cxcli` downloads Flux into its local cache when it is missing instead of requiring a system-wide install.
- `kubectl` is used for `validate-generated`, `deploy`, `flux apply`, `flux destroy`, `flux bootstrap`, and MK8s GPU validation flows.
- `helm` is used for `validate-sources` and other live Helm chart source/metadata validation paths. When source validation is enabled, `create` checks for missing source-validation tools before prompting for tenant/project/client details. It is not required for the normal `render`, `deploy`, `destroy`, `flux apply`, or `flux destroy` flow.
- `aws` CLI is used for `terraform unlock`.
- `git` is used for `bootstrap-ci`, local `origin` auto-detection, and Helm chart sources that resolve from Git tree URLs. When source validation is enabled and the catalog declares Git tree chart sources, `create` preflights `git` before identity prompts.
...
- `component_sources.yaml`:
  ...
  - `components.apps.<component-id>`: optional `source.portable`, optional `source.local`, optional `ui`, optional `release`, optional `defaults`, optional `wizard`, optional `input`.
...
- `components.infra.<component-id>`:
  ...
  - cxcli-owned policy for that component lives in sibling `component_cli_settings.yaml` at `components.infra.<component-id>.cli`. The bundled `mk8s` settings use `cli.gpu.*` for MK8s GPU image preferences and deploy-time validation defaults, `cli.boot_disk_defaults.<cpu|gpu>` for cxcli-owned explicit boot-disk defaults, and `cli.observability.*` for the MK8s observability contract (`primary_agent.{kind,chart_component_id,logs,metrics,traces,validation}`) that can auto-enable the collector chart, seed `deploy.targets[].observability.*` wizard defaults, and toggle the deploy-time Observability Agent guardrail. Infra settings can also declare `cli.observability.service_metrics.buckets` and `cli.observability.service_logs.buckets`; cxcli uses those records to decide which service-provider Prometheus federation buckets and Loki `__bucket__` values apply to enabled VM, Object Storage, PostgreSQL, MK8s, or storage components. cxcli resolves the selected preset's live resources (vCPU, RAM, GPU count), matches the first ordered `rules` entry whose bounds fit that shape, and materializes explicit `inputs.<cpu|gpu>_nodes_boot_disk_*` values from the matched size plus disk type. If no rule matches, cxcli falls back to a heuristic so quota checks still have a deterministic size. Explicit first-class inputs or `template.boot_disk` overrides remain authoritative. The bundled `vm` settings use `cli.image_preferences.*` for VM public-image ordering and `cli.observability.*` for the built-in Monitoring-agent path (`primary_agent.{kind,logs,metrics}`) plus the separate default-off `public_ingest` collector policy that cxcli can install on supported Ubuntu-family VMs when the customer wants public write-side VM logs and host metrics.
...
- `vm` uses `wizard_profile: vm` because `subnet_id`, `platform`, `preset`, `source_image_family`, `public_ip_mode`, optional GPU-cluster fabric choices, and preemptible GPU VM follow-up fields should come from guided catalog wiring instead of raw manual entry.
...
- Observability is now a first-class deploy contract and stays disabled by default. MK8s Kubernetes observability is target-scoped under `deploy.targets[].observability.*`, with each row bound by the target cluster `instance_id`; VM observability remains under `deploy.observability.vm.*` because it is not a Kubernetes target install. These switches control whether cxcli deploys/configures collectors; they do not create the Nebius Monitoring/Logging/Tracing service endpoints themselves.
- `create` and config normalization keep that customer contract scoped to the enabled infra set. MK8s-only projects get `deploy.targets[].observability.kubernetes.*`, VM-only projects get `deploy.observability.vm.logs.*` plus `deploy.observability.vm.collector.*`, mixed projects get both, and unrelated project-scope defaults such as MK8s GPU deploy validations are omitted when no MK8s component is selected.
...
- The built-in VM Monitoring agent path is always present for Nebius Compute VMs. `deploy.observability.enabled` does not install or remove that built-in agent and does not change how VM service metrics are ingested. The platform-native branch remains `deploy.observability.vm.logs.enabled` plus optional `deploy.observability.vm.logs.systemd_units`, which tells cxcli whether to turn on the supported Compute journald collection labels for systemd services. The bundled VM catalog keeps that logs branch enabled by default, but cxcli only materializes the VM journald labels when the parent `deploy.observability.enabled` switch is true.
- The separate default-off VM standalone collector branch is `deploy.observability.vm.collector.*`. When `deploy.observability.enabled=true`, `deploy.observability.vm.collector.enabled=true`, and the selected VM has `inputs.service_account_id`, cxcli installs the catalog-defined public collector deb package plus the catalog-defined Prometheus agent companion on supported Ubuntu-family boot images. The bundled catalog points that package source at Nebius's public Artifactory APT repo for `nebius-o11y-agent`, but the package name, version, repository URL, key URL, suite, component, origin, and Prometheus companion package are all source-catalog metadata under `components.infra.vm.cli.observability.public_ingest`. That stack uses the VM metadata token at `/mnt/cloud-metadata/token`, pushes journald logs to Logging through the public agent gRPC endpoint, and remote-writes host metrics to Monitoring through the public Prometheus Remote Write endpoint. It is intentionally separate from the built-in Monitoring agent path so the customer can choose between Nebius-managed VM telemetry and cxcli-managed public ingest without conflating them.
- On the VM path, cxcli materializes `nebius.o11y.systemd-logs-collection.enabled=true` and, when units are specified, `nebius.o11y.systemd-logs-collection.units=<unit1;unit2>` into `infra.components[id=vm].inputs.labels`. Existing VMs may need a Nebius stop/start cycle before changed labels take effect, so cxcli treats that as the supported day-2 activation boundary instead of trying to manage the built-in packages itself. Logs land in Logging as user-ingested logs, so the read path stays the project Loki endpoint plus the `default` bucket when querying with `nebius logging`.
...
- The write path is intentionally split by resource type. For MK8s, the Nebius observability agent collects pod logs, Prometheus-style metrics, and OTLP/gRPC traces from the cluster and forwards them to public regional Nebius write endpoints. The bundled in-cluster OTLP service is for traces and OTLP-forwarded app telemetry; Prometheus-style metrics still flow through the agent's scrape pipeline rather than an in-cluster OTLP metrics receiver. For built-in Nebius VM monitoring, cxcli only reports the managed-write note because that path stays Nebius-owned. For the cxcli-managed standalone VM collector, logs use `dns:///write.logging.<region>.nebius.cloud:443` with a `project-id` header and host metrics use `https://write.monitoring.<region>.nebius.cloud/projects/<project-id>/prometheus/api/v1/write`.
...
- Read-side Grafana wiring is automated but still kept out of `config.yaml` secrets. The bundled `grafana` app uses the maintained Grafana community Helm repository and leaves Grafana image registry/repository/tag on the chart defaults so the chart version and chart `appVersion` stay the single source of truth. It also pins Nebius service dashboard IDs with imported UIDs and references cxcli-owned Kubernetes dashboard JSON assets from `component_sources.yaml`; operators can change the bundled dashboard imports or dashboard-file references there when they need a different dashboard set. Source `config.yaml` does not embed cxcli-owned dashboard JSON. During `render`, cxcli writes those JSON files under `generated/grafana_dashboards/<target-id>/<folder>/`, renders a Grafana dashboard ConfigMap into the generated Flux target, and points the generated HelmRelease at that ConfigMap with `dashboardsConfigMaps`. The bundled catalog keeps Grafana.com service dashboards under the `nebius` provider and cxcli-owned Kubernetes JSON dashboards under the `nebius-kubernetes` provider because the Grafana Helm chart does not support mixing chart-managed imports and external ConfigMaps on the same provider key. During `deploy`, cxcli creates or reuses Kubernetes Secrets for the Grafana admin password and the Observability read token, ensures a project service account with `viewer`, issues an Observability static key when the token Secret is missing, and mounts that token into Grafana datasource provisioning. If the token Secret already exists but a catalog-bound Prometheus read endpoint clearly rejects the token with `401` or `403`, cxcli replaces the Secret with a fresh Observability static key. The Grafana admin username, admin Secret name/keys, read-token Secret name/key/env var, datasource names, UIDs, types, default marker, read endpoint bindings, report-facing datasource descriptions, report dashboard bindings, org ID, fallback Explore queries, and idle session timeout are settings-owned in `component_cli_settings.yaml`; dashboard sources stay source-owned in `component_sources.yaml`. The binding chain is read endpoint -> Grafana datasource -> dashboard source. `Nebius Services` points at the service-provider Monitoring read endpoint, while `Nebius User Metrics` points at the user-ingested Prometheus read endpoint used by Kubernetes workload metrics. The bundled Kubernetes Metrics and Logs dashboards include a `Cluster` variable using Nebius agent labels (`k8s.cluster.id` for Prometheus metrics and `k8s_cluster_id` for Loki logs); target Metrics and Logs report links pass that variable when deploy/flux can resolve the MK8s cluster ID. The bundled Traces dashboard stays generic because Tempo resource attributes depend on the emitting workload and are not normalized to a required cluster label by cxcli. The generated deploy report now separates public write endpoints, public read endpoints, and Grafana links per configured target, and its Grafana section explains the Prometheus datasource split using the settings-owned datasource descriptions. Endpoint labels, URL templates, inclusion conditions, and bucket expansion are settings-owned under the global `observability.endpoints` section, so adding a future tenant/project read endpoint plus a matching Grafana datasource is a settings-catalog change rather than a Python allowlist change. Before live Gateway status is captured, target Grafana links are shown as pending; after the Envoy Gateway `Gateway` has an address and `deploy` or `flux apply` reads it, cxcli waits briefly for the address and the report includes clickable Metrics, Logs, Traces, and dashboards links. Metrics, Logs, and Traces links open their report-bound dashboards when Grafana has imported them; otherwise they fall back to matching Explore `panes` links using the datasource bound to that report dashboard. `validate-sources` validates every declared dashboard source and then validates that each `components.apps.grafana.cli.report_dashboards` signal from the settings catalog references one of those dashboard sources. `validate-dashboards <config.yaml>` goes further after deploy by querying the live Grafana datasource proxy for the bound Prometheus, Loki, and Tempo read endpoints and checking each bundled dashboard source against its datasource/read-endpoint chain; the command shows dashboard-level progress with the current `<target-id>: <folder>/<dashboard>` binding while it waits on live Grafana calls. All report links are shortened through public Grafana `/goto/...` URLs after cxcli sets the live Grafana root URL to the discovered Gateway/LoadBalancer address so Grafana's own redirect cannot point at `localhost`. The same section includes a `kubectl --context=...` command to retrieve the target cluster's admin password. VM paths still report both the built-in agent's managed-write note and, when the standalone collector is enabled, the public write endpoints that collector actually uses. cxcli keeps static tokens and Grafana passwords out of repo config and generated Flux manifests.
- For target-scoped Grafana rows, `validate-dashboards` resolves an explicit kube context from generated Grafana status, the deploy report, an unambiguous local kubeconfig context, or the generated MK8s handoff before running `kubectl`; unresolved targets fail fast instead of using the ambient current context.
...
- The bundled catalog keeps chart source selection, release metadata, and default Helm values in `component_sources.yaml`; activation rules, validation images, thresholds, and timeouts live in `component_cli_settings.yaml`. The CLI only evaluates those rules against the selected MK8s context.
...
- `inputs.infiniband_fabric` is prompted only after `inputs.gpu_nodes_preset`, and only when the exact selected platform/preset's live Nebius metadata says `allow_gpu_clustering=true`; setting that field is what switches cxcli from a merely cluster-capable shape to the actual GPU-cluster / InfiniBand path
...
- Still external prerequisites:
  - `kubectl` for `validate-generated`, `deploy`, `flux apply`, `flux destroy`, `flux bootstrap`, and Flux readiness checks
...
- App-only or external-cluster bundles:
...
- Built-in cluster-target bundles:
...
- The report starts with a `Client` section for the client name, tenant, project, and region. `Infra`, `Apps`, and `Grafana` use focused subsections: MK8s cluster details are nested per cluster, app status is split into platform/observability/workload groups, and Grafana links plus credentials are grouped per target. MK8s cluster rows include the Nebius cluster ID and derived kube context when Terraform state is available, and Grafana credentials use that target-specific `kubectl --context=...` command.
- The generated report is emitted without trailing blank lines so customer-repo Markdown linting stays clean.
- `render`, `deploy`, `terraform apply`, `flux apply`, and `flux bootstrap` refresh that report artifact for the active project instead of leaving the starter placeholder behind.
...
- Optional-wizard controls are consistent across component selection, component phase prompts, and field prompts: `q` backs up to the previous step so you can revise an earlier answer, while `qq` stops the wizard immediately and saves the current config state. Interactive list and checkbox prompts bind those controls directly to the keys instead of showing Back/Quit as selectable rows.
...
- The bundled `mk8s` wizard keeps `inputs.cpu_nodes_preemptible` and `inputs.gpu_nodes_preemptible` as first-class node-group switches. When either is `true`, the rendered MK8s Terraform module passes the matching Nebius node-group `template.preemptible = {}` block for that CPU or GPU node group.
...
`create` is the bootstrap path, not the day-2 component-editing path. When the same resolved project folder for the same `tenant_id`/`project_id` already exists, `create` now warns and overwrites from scratch instead of reconciling the existing component selection. Use `component list/add/remove` for normal edits after the project already exists.
...
- When the generated manifest declares deploy-time validations, local `deploy` uses the same handed-off kubeconfig after Terraform/Flux work to run them directly with `kubectl`, keeps compact ordered JSON detail reports under `generated/inventory/`, refreshes the combined customer-facing `generated/inventory/deploy-report.md`, and prints the same validation summary in the terminal. GPU-enabled targets can declare GPU readiness, visibility, and NCCL checks from `deploy.targets[].validations.mk8s_gpu.*`. Observability-enabled MK8s targets get a generated in-cluster Observability Agent ingestion guardrail when the active settings catalog leaves `primary_agent.validation` enabled; its live Kubernetes reads are bounded for large clusters. The JSON files remain the machine-readable detail contract; the Markdown report is the single human-readable rollup with grouped `Infra`, `Apps`, `Grafana`, and `Validations` sections. In multi-target MK8s deployments, the report lists each cluster shape under `Infra` > `MK8s Clusters`, groups Grafana links per target, and keeps validation headings target-scoped so repeated checks such as GPU visibility, NCCL, and Observability ingestion remain distinguishable. When a run selects one target with `--target <instance-id>`, the refreshed validation section is scoped to that selected target instead of marking unselected target validations as not run; `--all-targets` reports every selected target.
...
- During deploy-time validations, `deploy` keeps one continuous spinner alive across validation boundaries and live in-cluster progress updates, so the command does not go visually idle between operator readiness, GPU visibility, NCCL, or Observability Agent phases.
...
- Day-2 component commands are safe to repeat for the same selectors: `component list` is read-only, `component add` skips already-enabled exact rows, and `component remove` skips already-absent rows. Adding another component instance requires an explicit new selector such as `mk8s@training-cluster`.
...
- `validate-dashboards <config.yaml>`
  - Validates each enabled bundled Grafana dashboard source against the live bundled Grafana datasources for the project config. Report dashboards are the Metrics, Logs, and Traces subset used for deploy-report links.
  - Checks the concrete binding chain: `observability.endpoints.read` key -> Grafana datasource UID/type -> dashboard JSON query contract. It validates the fixed dashboard sources; it does not generate, mutate, or repair dashboards. Prometheus checks metric names and required label keys for every dashboard source, runs representative PromQL queries for report dashboards, and summarizes no-data service-dashboard metrics as warnings. For target-scoped report dashboards, it resolves the target MK8s cluster ID from generated Grafana status, generated inventory, or the persisted kube context and validates cluster-filtered queries instead of letting another cluster's data satisfy the check. Loki checks bucket and Kubernetes log labels plus representative LogQL in the same target-aware way; Tempo checks TraceQL reachability and reports a warning when the endpoint is reachable but no traces exist in the selected window.
  ...
  - Use `--target <instance-id>` to validate one target-scoped Grafana row in a multi-target config. When omitted, every enabled Grafana row is checked and each target must resolve an explicit kube context.
...
- `deploy <config.yaml>`
  - Full local reconcile from the generated bundle: `deploy` resolves the sibling `generated/` directory and loads `generated/nebius-cxcli-manifest.json` as the authoritative deploy input. That keeps the rendered bundle, not the latest source file edits, as the applied contract. Before Terraform apply, `deploy` runs a generated-bundle preflight covering strict deployment-readiness checks against the manifest runtime config, MK8s network preflight, live Nebius quota/capacity validation, and Terraform validation for `generated/infra`; on bundled MK8s that Terraform-validation pass now also catches live MK8s cluster / derived GPU-cluster name collisions that are not already managed in the current Terraform state, while treating Nebius `NOT_FOUND` responses as the normal "resource is absent" case. `deploy` then applies Terraform, refreshes the deploy report for infra and apps artifacts, and applies Flux when app charts are enabled. On success, the terminal footer prints the complete report location at `generated/inventory/deploy-report.md`. Local runs now merge every selected built-in cluster target into `~/.kube/config`; single-target runs still switch `current-context`, while multi-target runs preserve the operator's existing `current-context` and add switchable contexts for each target. `deploy --all-targets`, `flux apply --all-targets`, and `flux bootstrap --all-targets` therefore leave every selected target available through `kubectl config use-context ...`. Multi-target infra-only `deploy` now refreshes every built-in cluster handoff automatically after Terraform apply; for app or validation work, continue using `--target <instance-id>` or `--all-targets` so the execution target stays explicit. If GitOps bootstrap is not configured yet, the CLI warns and prints the follow-up `flux bootstrap` command when Flux work actually runs.
...
- Deploy-time MK8s GPU checks are configured per target under `deploy.targets[].validations.mk8s_gpu.*`, where each row uses `instance_id` to bind to the cluster target. The Observability Agent ingestion check is generated for each observability-enabled MK8s target when the active settings catalog leaves `components.infra.mk8s.cli.observability.primary_agent.validation` enabled; that settings-catalog switch defaults to enabled and is separate from customer `config.yaml`. Use `--skip-validations` to bypass all selected target validations for one run, or repeat `--skip-validation <kind>` for targeted one-run skips such as `nccl`, `gpu-visibility`, or `observability-ingestion`; those CLI flags do not rewrite `config.yaml`. If a validation fails before its normal report is complete, `deploy` still writes a failure JSON report so the combined deploy summary shows `FAIL` with the underlying error instead of `NOT RUN`.
...
- `destroy <config.yaml>`
  - Full local teardown from the generated bundle. `destroy` resolves sibling `generated/` from the project config, then uses the generated manifest as the authoritative teardown contract for all rendered project resources. When enabled apps target an external or current cluster, it deletes the rendered Flux resources first and then runs Terraform destroy against the rendered infra bundle. When the generated bundle destroys the handed-off cluster directly, `destroy` skips the separate Flux delete step and relies on cluster teardown instead.
...
- `flux destroy <generated-dir>`
  - Apps-only direct delete from the rendered Flux tree using the same manifests that `flux apply` manages. Destructive by intent and requires confirmation or `--yes`. Use `--target <instance-id>` or `--all-targets` when the generated bundle declares more than one built-in cluster target.
...
- `flux bootstrap <generated-dir>`
  - GitOps bootstrap/reconcile path from the rendered Flux tree. Use this when the cluster should watch the Git repo/path with Flux. Use `--target <instance-id>` or `--all-targets` when the generated bundle declares more than one built-in cluster target.
  - Accepts the project `generated/` directory or a path under `generated/flux/`; other generated subtrees are rejected.
...
- Positional target quick map:
  ...
  - `component`, `validate`, `validate-dashboards`, `quota-check`, `quota-request`, `render`, `bootstrap-ci`, `deploy`, `destroy`, `email`: pass the project `config.yaml`.
...
- `component list <config.yaml>`
  ...
  - Example: `nebius-cxcli component list ~/deployments/tenant-name-example/project-name-example/config.yaml`
- `component add <config.yaml> [component-selector...]`
  ...
  - Catalog entries are reusable component types. Each newly added infra row has its own `instance_id`; app chart rows are identified by `<chart-id>@<instance-id>`. For target-bound charts, the app `instance_id` is the cluster target id, and the same chart can be enabled only once per target.
  - Interactive mode prompts for infra first and can complete an infra-only add without selecting any app. It asks for app selection only when no infra was selected or when you explicitly choose to add apps too, then confirms the final selection, auto-resolves app chart dependencies, and runs the field wizard only for the newly added components.
  ...
  - Non-interactive mode accepts component selectors directly: `<component-id>`, `infra:<component-id>`, `apps:<component-id>`, `all`, `none`, or `<component-id>@<instance-id>`. For example: `nebius-cxcli component add /path/to/config.yaml managed-postgresql object-storage gateway-helm --no-interactive`.
  - Repeating the same selector is a no-op when that exact row is already enabled. To add another infra or app-only instance, request a new `config.yaml` `instance_id` with `<component-id>@<new-instance-id>`, for example `object-storage@logs-bucket` or `mk8s@training-cluster`. For target-bound app charts, the explicit suffix is the cluster target id and becomes `apps.charts[].instance_id`; duplicate `<chart-id>@<target-id>` installs are skipped. When more than one cluster target is enabled, app charts must be added with an explicit target instance, for example `n8n@cluster2`.
  ...
  - Example: `nebius-cxcli component add ~/deployments/tenant-name-example/project-name-example/config.yaml managed-postgresql --no-interactive`
- `component remove <config.yaml> [component-selector...]`
  ...
  - Non-interactive mode accepts enabled row selectors: `<component-id>`, `infra:<component-id>`, `apps:<component-id>`, `all`, `none`, `<instance-id>`, or `<component-id>@<instance-id>`. When multiple instances of the same component type are enabled, remove by exact `instance_id` or `<component-id>@<instance-id>`.
  ...
  - Example: `nebius-cxcli component remove ~/deployments/tenant-name-example/project-name-example/config.yaml managed-postgresql@managed-postgresql-2 --no-interactive`
...
- `destroy <config.yaml>`
  ...
  - Deletes rendered apps first only when they target an external or current cluster; otherwise it destroys the handed-off cluster directly with Terraform.
...
the expected first-attempt deleted-key token-refresh traceback so the operator sees the cxcli
warning/retry path instead of an unrelated SDK stack trace while Terraform continues.
...
nebius-cxcli component add /path/to/config.yaml mk8s@training-cluster mk8s@serving-cluster --no-interactive
nebius-cxcli component add /path/to/config.yaml gateway-helm@serving-cluster --no-interactive
nebius-cxcli component remove /path/to/config.yaml managed-postgresql@managed-postgresql-2 --no-interactive
nebius-cxcli component remove /path/to/config.yaml gateway-helm@serving-cluster --no-interactive
