...
- For app rows, `id` names the chart type and `instance_id` names the chart instance. When built-in cluster targets such as `mk8s` are enabled, each enabled app chart binds to one target with `apps.charts[].target_ref`; for target-bound app rows, `instance_id` must match that target cluster id, so the full install identity is clear as `<chart-id>@<target-ref>`, for example `nvidia-gpu-operator@cluster2`. A target-bound chart can be enabled once per chart id and target; add the same chart to another cluster with `<chart-id>@<target-id>`.
...
- If `component add` introduces the first built-in cluster target into an app-only config, cxcli target-binds each existing single app row to that target by setting both `apps.charts[].target_ref` and the target-scoped `instance_id`. Ambiguous duplicate app rows still fail before write and must be edited explicitly.
...
- `component remove` blocks changes that would leave unresolved component bindings, broken target-bound app `target_ref` rows, or dependency breakage in `config.yaml`.
...
- Flux output stays flat per rendered target: app-only bundles render flat files under `generated/flux`, while bundles with built-in cluster targets render one flat subtree per target under `generated/flux/targets/<target_ref>`; legacy `apps/` and `sources/` nesting is still unsupported.
...
- Root blocks:
  ...
  - `observability`: global Nebius Observability metadata for the tenant/project. Today `observability.endpoints.<read|write>.*` owns the public Monitoring, Logging, and Tracing endpoint keys, labels, URL templates, `include_when` signal selectors, and optional `bucket_placeholder` expansion used by all resource types.
...
- `components.infra.<component-id>`:
  ...
  - `cli`: optional cxcli-owned policy for that component. The bundled `mk8s` component uses `cli.gpu.*` for MK8s GPU image preferences and deploy-time validation defaults, `cli.boot_disk_defaults.<cpu|gpu>` for cxcli-owned explicit boot-disk defaults, and `cli.observability.*` for the source-owned MK8s observability contract (`primary_agent.{kind,chart_component_id,logs,metrics,traces}`) that can auto-enable the collector chart and seed `deploy.targets[].observability.*` wizard defaults. Infra components can also declare `cli.observability.service_metrics.buckets` and `cli.observability.service_logs.buckets`; cxcli uses those catalog records to decide which service-provider Prometheus federation buckets and Loki `__bucket__` values apply to enabled VM, Object Storage, PostgreSQL, MK8s, or storage components. cxcli resolves the selected preset's live resources (vCPU, RAM, GPU count), matches the first ordered `rules` entry whose bounds fit that shape, and materializes explicit `inputs.<cpu|gpu>_nodes_boot_disk_*` values from the matched size plus disk type. If no rule matches, cxcli falls back to a heuristic so quota checks still have a deterministic size. Explicit first-class inputs or `template.boot_disk` overrides remain authoritative. The bundled `vm` component uses `cli.image_preferences.*` for VM public-image ordering and `cli.observability.*` for the built-in Monitoring-agent path (`primary_agent.{kind,logs,metrics}`) plus the separate default-off `public_ingest` collector policy that cxcli can install on supported Ubuntu-family VMs when the customer wants public write-side VM logs and host metrics.
...
- `components.apps.<component-id>`:
  ...
  - `cli.observability`: optional app-side observability metadata. In the bundled catalog this is used for app-specific metrics endpoint metadata such as the GPU Operator's DCGM Exporter service and for source-owned GPU node-label prerequisites, selectors, and stack-source guards required to make that endpoint live.
  - `cli.grafana.admin`: optional Grafana-app admin Secret contract. It declares the runtime-only Kubernetes Secret name, admin username, and Secret data keys used by the Grafana Helm chart and generated deploy report.
  - `cli.grafana.read_token`: optional Grafana-app Observability read-token Secret contract. It declares the runtime-only Kubernetes Secret name/key and the environment variable used by Grafana datasource provisioning.
  - `cli.grafana.datasources`: optional Grafana-app datasource provisioning metadata. Each entry declares the Grafana display `name`, stable `uid`, datasource `type`, read endpoint key, and optional `isDefault`; cxcli materializes these entries into Grafana datasource values from the active Observability endpoint summary. `validate-sources` fails if a datasource references a read endpoint key that is not declared under `observability.endpoints.read`.
  - `cli.grafana.orgId`: optional Grafana organization ID used in generated dashboard and Explore links.
  - `cli.grafana.logout-timeout`: optional Grafana idle session duration, defaulting to `20m`. It is rendered into `grafana.ini` under `[auth]` as `login_maximum_inactive_lifetime_duration`; use Grafana duration syntax such as `10m`, `1h`, or `7d`.
  - `cli.grafana.explore_queries`: optional fallback Explore query templates keyed by `metrics`, `logs`, or `traces`; report links use them only when the catalog-bound dashboard has not been imported yet.
  - `cli.grafana.report_dashboards`: optional Grafana-app report-link bindings for `metrics`, `logs`, and `traces`. Each signal value is a single `<folder>/<dashboard>` reference to a chart `defaults.values.dashboards.*` entry. The dashboard entry owns `gnetId` and `datasource`; `validate-sources` fails if a binding references a dashboard that is not declared in defaults, if that dashboard does not declare both fields, or if its datasource is not declared under `cli.grafana.datasources`.
...
- GPU monitoring stays aligned with NVIDIA's current split: For GPU-enabled MK8s, DCGM Exporter must stay enabled in GPU Operator. Omitting `nvidia-gpu-operator.values.dcgmExporter.enabled` is valid because the bundled GPU Operator chart defaults it to enabled; explicitly setting it to `false` is rejected. Scraping and pushing those metrics to Nebius Monitoring happens only when the MK8s observability metrics path is enabled. cxcli does not invent a `ServiceMonitor` dependency. Instead, the bundled catalog declares the `nvidia-dcgm-exporter` service as an app metric target, and cxcli renders it into the Nebius agent's chart-native `config.metrics.additionalTargets` list while preserving any customer-provided `additionalTargets`. On Nebius driverful GPU images (`gpu_stack_source: nebius_image`), nodes can default to `nvidia.com/gpu.deploy.operands=false`, which leaves GPU Operator operand DaemonSets, including `nvidia-dcgm-exporter`, at desired count `0`. When `deploy.targets[].observability.enabled=true`, Kubernetes metrics are enabled, and the GPU Operator app is selected on the `nebius_image` stack, cxcli now materializes the catalog-owned GPU node labels required to run only DCGM Exporter plus the GPU Operator validator while keeping GPU Operator device-plugin/GFD disabled so they do not duplicate the Nebius-managed path. `deploy` also reconciles those labels onto existing live GPU nodes using the catalog-owned selector, because MK8s node-group label updates apply cleanly to future nodes but may not relabel already-running Node objects. The deploy report still tells operators to verify live `nvidia-dcgm-exporter` endpoints after deploy, because the final signal is runtime readiness, not config presence alone.
- Read-side Grafana wiring is automated but still kept out of `config.yaml` secrets. The bundled `grafana` app uses the maintained Grafana community Helm repository, the official `grafana/grafana` image, and pinned chart/image/dashboard versions from `component_sources.yaml`; operators can change the bundled dashboard IDs there when they need a different Grafana.com dashboard set. During `deploy`, cxcli creates or reuses Kubernetes Secrets for the Grafana admin password and the Observability read token, ensures a project service account with `viewer`, issues an Observability static key when the token Secret is missing, and mounts that token into Grafana datasource provisioning. If the token Secret already exists but a catalog-bound Prometheus read endpoint clearly rejects the token with `401` or `403`, cxcli replaces the Secret with a fresh Observability static key. The Grafana admin username, admin Secret name/keys, read-token Secret name/key/env var, datasource names, UIDs, types, default marker, read endpoint bindings, org ID, fallback Explore queries, report dashboard bindings, and idle session timeout are catalog-owned under `cli.grafana`: `Nebius Services` points at the service-provider Monitoring read endpoint, while `Nebius User Metrics` points at the user-ingested Prometheus read endpoint used by Kubernetes workload metrics. The generated deploy report now separates public write endpoints, public read endpoints, and Grafana links per configured target. Endpoint labels, URL templates, inclusion conditions, and bucket expansion are catalog-owned under the global `observability.endpoints` section, so adding a future tenant/project read endpoint plus a matching Grafana datasource is a catalog change rather than a Python allowlist change. Before live Gateway status is captured, target Grafana links are shown as pending; after the Envoy Gateway `Gateway` has an address and `deploy` or `flux apply` reads it, cxcli waits briefly for the address and the report includes clickable Metrics, Logs, Traces, and dashboards links. Metrics, Logs, and Traces links open their catalog-bound dashboards when Grafana has imported them; otherwise they fall back to matching Explore `panes` links using the datasource bound to that report dashboard. The source catalog validates that each `cli.grafana.report_dashboards` signal references a declared dashboard default with `gnetId` and datasource metadata, so report links cannot silently point a signal at an incomplete dashboard definition. All report links are shortened through public Grafana `/goto/...` URLs after cxcli sets the live Grafana root URL to the discovered Gateway/LoadBalancer address so Grafana's own redirect cannot point at `localhost`. The same section includes a `kubectl --context=...` command to retrieve the target cluster's admin password. VM paths still report both the built-in agent's managed-write note and, when the standalone collector is enabled, the public write endpoints that collector actually uses. cxcli keeps static tokens and Grafana passwords out of repo config and generated Flux manifests.
...
- cxcli does not vendor or redistribute third-party binaries, Helm charts, container images, package repositories, or dashboard JSON referenced by the bundled catalog. Those upstream artifacts, including Grafana, Envoy Gateway, NVIDIA operators, Flux, Terraform, Helm, kubectl, and any Nebius/public package repositories used at deploy time, remain governed by their own licenses, support terms, usage terms, and image/chart/package distribution policies. This project's license covers the cxcli source and generated automation, not the operator's deployed use of referenced third-party artifacts.
...
- The canonical GPU role is `nvidia-gpu-operator` for both Nebius-image and `operator_managed` node groups. On Nebius-managed images the CLI disables the driver, the NVIDIA Container Toolkit runtime (`values.toolkit.enabled`), and the Nebius `NVIDIADriver` CRD path. On `operator_managed` stacks it keeps the driver and toolkit enabled but still forces `values.driver.nvidiaDriverCRD.enabled=false`, because the bundled `gpu-operator@v25.10.0` chart's Nebius `NVIDIADriver` CRD template is currently broken during Flux install. The catalog now keeps only those Nebius-specific operator deltas instead of restating live chart defaults.
...
- Built-in cluster-target bundles:
  - `generated/flux/targets/<target_ref>/helm-repositories.yaml`
  - `generated/flux/targets/<target_ref>/namespace-<namespace>.yaml`
  - `generated/flux/targets/<target_ref>/helmrelease-<group>-<release>.yaml`
  - `generated/flux/targets/<target_ref>/kustomization.yaml`
...
- Optional-wizard controls are consistent across component selection, component phase prompts, and field prompts: `q` backs up to the previous step so you can revise an earlier answer, while `qq` stops the wizard immediately and saves the current config state.
...
- When more than one built-in cluster target is enabled, enabled app charts must set `apps.charts[].target_ref` to one target `instance_id`, target-bound app rows must use that same value as their own `instance_id`, and target-scoped deploy settings live under `deploy.targets[]` rows with the same `instance_id`. Render writes Flux manifests under `generated/flux/targets/<target_ref>/`, and `deploy`, `flux apply`, `flux destroy`, and `flux bootstrap` require `--target <instance-id>` or `--all-targets` when the generated bundle contains more than one target that needs Kubernetes access.
...
- Generated Flux artifacts are treated as the deploy truth. If an app chart depends on Terraform-backed component outputs, you must rerender after the needed Terraform state exists before treating the rendered Flux tree (`generated/flux` or `generated/flux/targets/<target_ref>`) as the final GitOps payload.
...
- `flux apply` is safe to rerun sequentially with the same rendered Flux tree (`generated/flux` or `generated/flux/targets/<target_ref>`): it applies the existing rendered manifests, skips Flux controller installation when controllers are already present, and waits for the rendered Flux resources to become `Ready`.
...
- Deploy-time MK8s GPU checks are configured per target under `deploy.targets[].validations.mk8s_gpu.*`, where each row uses `instance_id` to bind to the cluster target. Use `--skip-validations` to bypass all selected target validations for one run, or repeat `--skip-validation <kind>` for targeted one-run skips such as `nccl` or `gpu-visibility`; those CLI flags do not rewrite `config.yaml`. If a validation fails before its normal report is complete, `deploy` still writes a failure JSON report so the combined deploy summary shows `FAIL` with the underlying error instead of `NOT RUN`.
...
- Positional target quick map:
  ...
  - `component`, `validate`, `quota-check`, `quota-request`, `render`, `bootstrap-ci`, `deploy`, `destroy`, `email`: pass the project `config.yaml`.
...
- `component add <config.yaml> [component-selector...]`
  ...
  - Catalog entries are reusable component types. Each infra add creates a new enabled component instance with its own `instance_id`; app chart rows are identified by `<chart-id>@<instance-id>`. For target-bound charts, `target_ref` remains the explicit cluster selector, the app `instance_id` must match that target id, and the same chart can be enabled only once per target.
  ...
  - Repeating the same reusable component id adds another instance when that type allows multiple config rows. You can also request an explicit `config.yaml` `instance_id` with `<component-id>@<instance-id>`, for example `object-storage@logs-bucket` or `mk8s@training-cluster`. For target-bound app charts, the explicit suffix is the cluster target id, becomes `apps.charts[].target_ref`, and duplicate `<chart-id>@<target-id>` installs fail fast instead of creating a second suffixed row. When more than one cluster target is enabled, app charts must be added with an explicit target instance, for example `n8n@cluster2`.
...
- `component remove <config.yaml> [component-selector...]`
  ...
  - The command fails fast when the removal would leave unresolved app dependencies, component input bindings, or target-bound app rows whose `target_ref` no longer points at an enabled cluster target.
  - After the edit, run `validate` and `render` again.
