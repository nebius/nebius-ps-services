...
- The deploy-time MK8s validations are intentionally layered to avoid semantic overlap. `cluster_smoke` is the required read-only all-node Kubernetes inventory gate generated for every MK8s target; it reports node readiness, CPU/GPU totals, node groups, known GPU node-group presence, and known GPU node-count lower bounds without scheduling pods. For GPU-backed targets, `operator_readiness` is the prerequisite GPU stack gate: policy objects plus scheduler-visible GPUs on Ready nodes. `cuda_smoke` is the bounded single-node data-path smoke test that proves a real CUDA workload can execute. `nccl` is the optional multi-node communication and bandwidth validation: Socket/TCPIP on Ethernet-only shapes, RDMA on GPU-cluster / InfiniBand shapes. The ordering is deliberate so common cluster/operator/runtime failures stop before the expensive NCCL phase, while NCCL remains the only bundled check that says anything about distributed GPU communication quality. For cxcli-managed Soperator targets with fixed or nonzero-minimum GPU workers, local `deploy` stages app reconciliation by applying the platform/GPU operator Flux resources first, running the required MK8s node inventory smoke plus GPU stack, CUDA smoke, and NCCL validations while the GPUs are still scheduler-free, then applying the full Soperator bundle. If a managed Soperator GPU worker group autoscaling range starts from zero (`min_node_count: 0`, `max_node_count > 0`), cxcli applies the full Soperator bundle first, runs the required Soperator smoke validation so its Slurm allocations create GPU-host autoscaler pressure, and then runs the MK8s validations against the resumed GPU nodes. Workload validations still use scheduler-free GPUs and record a skipped report rather than failing when existing workloads or already-running Soperator workers reserve every GPU on every Ready GPU node; NCCL also treats a transient `MPIJob` preemption as the same skip condition when those workload pods claim all GPUs while the NCCL launcher is starting. For Soperator targets, the required Soperator smoke validation covers the Slurm-owned path separately by running hostname jobs across every reported Slurm partition, running one-GPU `nvidia-smi -L` allocations across every reported GPU partition, and running one Slurm-owned `mpirun /usr/bin/all_reduce_perf_mpi` benchmark only when selected Slurm GPU nodes are 8-GPU nodes. That Slurm benchmark is not the transient Kubernetes `nccl-test` Helm/MPIJob path; it records the average large-message `busbw` across the 2G, 4G, and 8G rows and is skipped on 1-GPU Slurm test clusters. The `nebius-cxcli-soperator-cluster-validation/v2` JSON report is intentionally line-oriented for high-cardinality clusters: command `stdout`/`stderr` are arrays of lines, and the all-partition hostname and GPU allocation sub-checks also write structured `partition_hostnames` and `gpu_allocations` arrays so thousands of node names can be reviewed vertically without parsing escaped multiline strings.
- Soperator targets use the same cxcli-owned optional MK8s GPU validation prompts as
  other GPU-enabled MK8s targets: `operator_readiness`, `cuda_smoke`, and
  `nccl` are prompted in the create/component wizard and persisted under
  `deploy.targets[].validations.mk8s_gpu.*`. The required MK8s node inventory
  ...
  cxcli deploy-time NCCL are both runnable for the same target, cxcli warns
  because
  the Slurm NCCL workload and Kubernetes `MPIJob` can compete for GPUs and RDMA
  bandwidth, skew results, delay deploy, or skip/fail after Soperator workers
  reserve the GPUs.
- Enabled Soperator targets also get a required `soperator_cluster_smoke`
  ...
  pod with `sinfo` and `squeue`, runs a one-task synchronous `srun` job that
  prefers an idle non-GPU partition when one exists, runs hostname jobs across
  every reported Slurm partition, and runs one-GPU `nvidia-smi -L` allocations
  across every reported GPU partition. The Slurm NCCL benchmark runs only when
  selected Slurm GPU nodes are 8-GPU nodes; single-node fallback applies only
  when the partition has exactly one total 8-GPU Slurm node and it is idle.
...
- Validation cleanup is split deliberately: keep dedicated namespaces for isolation and repeatability, but delete transient validation workloads after each run. For the bounded CUDA smoke test that means applying the `gpu-validation` namespace first, creating a reusable `cuda-smoke-validation` ServiceAccount with token automount disabled, and deleting the sampled pods after the run while retaining the namespace and ServiceAccount. For NCCL that means deleting the transient `MPIJob` and, if cxcli had to install Kubeflow Training Operator only for that run, deleting that transient prerequisite again while retaining the validation namespace.
...
- `upgrade node-template <config.yaml> [infra:mk8s@<target>] [--to-version <major.minor>] [--to-os <os>] [--to-gpu-stack-preset <preset>]`
  ...
  validations such as GPU stack readiness, MK8s node inventory smoke,
  CUDA smoke, and NCCL are the post-upgrade GPU canary phase. Repeated
...
helpers, GPU reservation policy, and target GPU validation prompts are offered.
...
operator to choose a version from the exact committed migration-profile rows or
enter one manually, while non-interactive runs can pass
...
in target Helm values so migration does not attempt immutable PV selector
...
Soperator roles onto them. If the source report contains worker NodeSets such
as `worker-gpu` and `worker-cpu`, migration renders those worker NodeSet names
...
`ext-soperator migrate ... --execute --approve` command. If the report shows
`Pending phase: none` but the post-migration config refresh was skipped, rerun
...
CUDA smoke, and NCCL validations. On reruns, onboarding inspects the live
...
allocation and a Slurm NCCL benchmark when a GPU partition has selected 8-GPU
nodes available. In that ownership model, `deploy-report.md` promotes the
passed Slurm-side GPU allocation or NCCL benchmark result into the matching human validation summary,
...
target-scoped optional `deploy.targets[].validations.mk8s_gpu.*` checks
configured in `config.yaml` such as operator readiness, CUDA smoke, and
NCCL when those checks are enabled, runs the
...
that request each partition's full reported node count to drive autoscaled
workers up to partition capacity, all-GPU-partition `nvidia-smi -L` allocations,
and the same Slurm NCCL benchmark only when selected Slurm GPU nodes are 8-GPU
nodes, writes
...
`ext-soperator migrate` and checkpointed `soperator upgrade` runs from the same
laptop, workdir, and operator account that started them, because the resume
checkpoints are local under `.nebius-cxcli/soperator-migrations/<target>/` and
...
external-migration rollout transitions such as `replacing (cordoned)` and real
...
The Soperator profile keeps the cxcli-owned deploy-time CUDA smoke and NCCL
validation prompts available while keeping Soperator ActiveChecks disabled by
default. The shared `nccl-test` source declares
`usage.lifecycle: transient`, so it remains tied to the deploy-time validation
flow instead of the persistent app selector. If an operator enables Slurm NCCL
ActiveChecks while deploy-time NCCL remains enabled, cxcli emits a
deploy-validation warning that the Slurm NCCL
...
- Transient catalog charts such as `nccl-test` are not selectable app
  components. They declare `usage.lifecycle: transient` plus
  `usage.config.ref`, so selector guidance points operators to the
  customer-facing config field that activates the runtime flow. Operators
  enable NCCL through `deploy.targets[].validations.mk8s_gpu.nccl` or the MK8s
  wizard GPU validation prompts, and cxcli keeps the transient chart source
  internal to the deploy-time validation runner.
...
- For `apps:soperator`, the create/component wizard uses
  ...
  command reads the source discovery report and prints the target remediation
  and compute/storage migration plan from the accepted onboarding analysis.
  Live `--execute`
...
- Applies deploy-time validations from the generated manifest. Every MK8s target gets the required read-only MK8s node inventory smoke, GPU workload checks come from the target-facing `deploy.targets[].validations.mk8s_gpu.*` contract, and enabled Soperator targets get the required `soperator_cluster_smoke` Slurm smoke validation. The optional GPU chain starts with GPU stack readiness before sampled CUDA smoke and NCCL workloads; when generated config or accepted external inventory exposes GPU node-group names or node-count lower bounds, the required MK8s inventory smoke fails under-advertised scheduler-visible GPU groups instead of only reporting them. That required Soperator validation runs after Terraform and Flux have already reconciled compute and storage resources in parallel, then waits for Kubernetes-visible jail storage convergence (`jail-pvc`/`jail-pv` binding, `jail-mount` DaemonSet readiness, and storage-related Pending pods clearing) before Slurm sign-off. Persistent Pending pods still fail the validation and include event causes such as `FailedMount` in the detail report. Observability-enabled MK8s targets get `mk8s_observability_ingestion` when the active settings catalog leaves `primary_agent.validation` enabled. That guardrail verifies the live Nebius Observability Agent HelmRelease, signal config, DaemonSet readiness, and trace OTLP service EndpointSlice readiness. Native ESO MysteryBox sync targets get a required `mysterybox_eso_connectivity` validation that checks in-cluster Nebius API TLS, `ClusterSecretStore Ready=True`, every configured `ExternalSecret Ready=True`, and ESO controller log errors since the current validation started. The generated Markdown report keeps platform/security selections visible in its component summary, including MysteryBox, External Secrets Operator, NVIDIA GPU Operator, NVIDIA Network Operator, and Soperator smoke results when those components are enabled. Local `deploy` can bypass optional validations with `--skip-validations` or a subset with repeatable `--skip-validation <kind>` flags such as `nccl`, `cuda-smoke`, or `observability-ingestion`; required validations still run and those one-run overrides do not rewrite the source config.
...
- `upgrade node-template <config.yaml> [infra:mk8s@<target>] [--to-version <major.minor>] [--to-os <os>] [--to-gpu-stack-preset <preset>]`
  ...
  - After GPU node groups settle, the required MK8s node inventory smoke plus enabled target-scoped deploy validations such as GPU stack readiness, CUDA smoke, and NCCL are the post-upgrade GPU canary phase.
...
- That deploy-time MK8s validation chain now keeps one continuous spinner active and updates its message from the emitted validation progress, so the CLI stays visibly alive while it transitions between node-inventory smoke, operator-readiness, CUDA smoke, and NCCL phases.
