...
- NodeSet profile chooses the Slurm worker layout. `nebius-gpu-v1` is the
  ...
  `nodesets[].slurmd.resources.gpu`. For example, 5 x `1gpu-*` hosts means five
  Slurm worker replicas with `gpu: 1`, while 5 x `8gpu-*` hosts means five
  replicas with `gpu: 8` and 40 total GPUs. cxcli writes and refreshes
  `inputs.soperator.worker_node_groups` entries for the generated worker
  shards, such as `worker-cpu-0` and `worker-gpu-2`, with disabled
  `autoscaling` and `ephemeral_nodes` controls ready to edit. Enabling a
  ...
  shard's autoscaling `min_node_count`, writes finite non-negative
