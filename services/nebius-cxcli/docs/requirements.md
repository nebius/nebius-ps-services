...
- AC-012: Install, onboard, upgrade, and status expose paired `--interactive/--no-interactive` flags; discover is explicit/non-interactive and destroy retains its TTY phrase gate. When a new interactive install or upgrade omits its release selector, cxcli resolves the current official latest release and prompts with a runtime-resolved default such as `latest(4.1.7)`. A fresh non-interactive install requires its explicit release selector. A fresh non-interactive full-stack upgrade requires `--to-release`, `--to-k8s-version`, `--to-os`, and `--to-gpu-stack-preset`; missing selectors fail before configuration or cloud access. Install resume rejects a release selector and all fresh-install identity, profile, network, subnet, and overwrite options, then reuses the exact authority frozen in its saved plan. `install --resume --dry-run --replan` accepts a readable never-executed `planned` receipt or a failed infrastructure apply with start/failure evidence and no infrastructure-complete checkpoint. It preserves the saved project, target, configuration, generated manifest, and frozen release. Failed-apply recovery additionally verifies the exact approved binary plan, retains the failed receipt, preserves infrastructure and shared-group addresses, known desired settings, and resource identities, rejects deletions/replacements, and binds recovery provenance into a new approval fingerprint. Before replanning a failed apply, cxcli may transactionally regenerate root `main.tf` only when module sources, input values, and every other Terraform root artifact still match the frozen manifest. Previously uncreated access permits may be replaced only by current renderer-owned grants bound to the original project and retained groups; existing grants cannot be removed or adopted. Only validated recovery may contain no infrastructure changes. Explicit or accepted `latest` values are frozen before mutation. Interrupted upgrade recovery may omit every selector in both interactive and non-interactive mode; if a selector or policy is repeated it must exactly match the parent campaign receipt, and recovery never prompts or dynamically re-resolves a frozen value. Direct existing-config onboarding rejects project-creation flags and keeps its configured region authoritative; an explicit region is an additional assertion and cannot replace it. Fresh deployments-root onboarding derives an omitted region from the live cluster without prompting and preserves that derivation across an interrupted scaffold through an owner-only marker published first and bound to the exact scaffold config hash. Marker and initial config publication share the config lock, and config creation is atomic and create-only so a competing writer is never overwritten. The marker and hash are re-proved after discovery and under the config lock before the observed region can be written.
...
- NC-005: Do not reinterpret an interrupted install as a no-op after its target release appears; retain its frozen absent source and reject live versions outside that lineage. Do not silently select release, Kubernetes, OS, or GPU driver targets for a fresh non-interactive upgrade. Do not accept release or fresh-install configuration options for install resume, or let any supplied upgrade-recovery selector override or re-resolve the interrupted operation's frozen authority.
- NC-006: Do not replan a missing, corrupt, executing, completed, post-infrastructure, or ambiguous install receipt. Failed infrastructure recovery must not expand resource scope, replace known identities, or discard failed evidence. An earlier approval fingerprint cannot authorize a replacement plan.
...
- AC-002: `/home`, `/data`, `/scripts`, `/models`, and `/opt/soperator-home` are mandatory retained path-specific PVC mounts outside the versioned active/passive rootfs slots. All relevant jail consumers, including bootstrap and check workloads, use the same retained bindings; image population and slot cleanup exclude them. New managed installations use one canonical layout without migration or compatibility handling for an earlier cxcli layout. During first rootfs adoption, the interactive upgrade wizard may add validated existing data-directory mounts without copying their content; later slot-backed upgrades may retain or remove only already-backed optional mounts. The approved normalized set is persisted only with successful canonical upgrade promotion.
...
- AC-006: Canonical runtime authentication requires one strictly owned project service account with exactly project `admin`, sufficient for rendered IAM and node service-account attachment, without tenant-scoped permits. Cached project commands validate that identity read-only before provisioning and direct insufficient permissions to explicit `auth`; they do not elevate permissions or rotate a healthy key. Explicit targeted `auth` validates the cached key and identity, then reconciles managed project roles under operator IAM authority by creating the desired role before deleting obsolete exact project permits. Foreign scope, membership, or identity fails before mutation. CI imports enforce the same contract read-only.
...
- AC-003: Normal `component add`, `component remove`, `render`, `deploy`, Flux apply, and ordinary Helm upgrade work for ordinary apps on managed or onboarded Soperator targets, without special commands, app allowlists, or new execution flags.
- AC-004: Ordinary app changes preserve protected Terraform, upstream Soperator resources, and lifecycle artifacts, including the protected Soperator ownership row during intermediate selection and dependency filtering. Actual protected drift and resource or Helm ownership collisions fail before mutation; mixed-target projects retain exact target isolation.
- AC-005: Ordinary app deployment skips Terraform and Slurm lifecycle hooks, uses existing cluster identity and fencing, and preserves the behavior of ordinary projects. Configuration removal does not imply live pruning or uninstall.
...
- AC-010: Maintenance entry applies the existing selected job policy and TUI actions, retaining the guarded requeue-hold-all default, eligibility checks, wait fallback, timeout supervision and exact hold ownership. Pending or held jobs may remain; all affected allocations, running steps and completion cleanup must clear before passive suppression or disruptive work. Diagnostics never substitute cancellation, requeue or universal waiting for user decisions.
...
- AC-001: Fresh install accepts one values-only YAML file through `--values-file`; interactive questions prefill supplied settings and explicit answers win. Generic catalogs remain Soperator-free and infrastructure selection remains unchanged.
...
- AC-003: Resume rejects the input flag and uses saved configuration without rereading external values. Upgrades preserve saved choices and reject target-incompatible settings without a new input flag.
