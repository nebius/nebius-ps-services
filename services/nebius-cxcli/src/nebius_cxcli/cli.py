...
def render_command(
    ...
    try:
        ...
        try:
            ...
            write_inventory(config, staged_paths, validations=_deploy_validation_specs(config))
            _print_mk8s_gpu_validation_warnings(config)
...
def deploy_command(
    ...
    try:
        ...
        console.print(f"Complete deploy report: {paths.inventory_dir / DEPLOY_REPORT_FILENAME}")
    except Exception as exc:  # pragma: no cover - CLI surface
        _exit_with_error(exc)
