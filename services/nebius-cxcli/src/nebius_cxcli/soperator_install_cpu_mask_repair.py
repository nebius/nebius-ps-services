"""Sealed repair of the exact CPU quota-as-affinity-mask install defect."""
...
from .soperator_install_cpu_mask_recovery import capture_cpu_mask_failure
...
from .soperator_worker_topology import H200_PHYSICAL, h200_static


def cpu_mask_repair_candidate(
    ...
    if len(cm) != 1 or len(outer) != 1:
        raise RuntimeError("CPU mask repair requires unambiguous compiled values")
    values = yaml.safe_load(cm[0]["data"]["values.yaml"])
    if outer[0]["spec"]["values"] != values:
        raise RuntimeError("CPU mask repair requires matching umbrella values")
    nodes = gpu_nodesets(values["nodesets"]["overrideValues"])
    if not nodes:
        raise RuntimeError("CPU mask repair has no GPU workers")
    old = h200_static(32000)
    new = H200_PHYSICAL + " Gres=gpu:8"
    ...
    for node in nodes:
        if (
            ...
            raise RuntimeError("CPU mask repair requires the exact H200 worker quota projection")
...
def cpu_mask_reservation_handoff(
    repair: Mapping[str, Any], *, paths: ProjectPaths, policy: SoperatorChecksPolicy
) -> Mapping[str, Any]:
    current = _files(paths.flux_dir)
    previous = cpu_mask_repair_candidate(current, inverse=True)
    if (
        _file_hashes(current) != repair["replacementFiles"]
        or _file_hashes(previous) != repair["previousFiles"]
        or cpu_mask_repair_candidate(previous) != current
    ):
        raise RuntimeError("CPU mask repair lost its exact reversible input delta")
    ...
    if old.sha256 != handoff["policy"] or checks_digest(after) != policy.values_sha256:
        raise RuntimeError("CPU mask repair changed native check policy")
    return {**handoff, "predecessorPolicy": handoff["policy"], "policy": policy.sha256}


def prepare_install_cpu_mask_repair(**kwargs: Any) -> Mapping[str, Any] | None:
    ...
    return prepare_install_nodeset_binding_repair(
        binding=NodeSetBindingRepair(
            reason=CPU_MASK_REPAIR_REASON,
            slug="cpu-mask",
            candidate=cpu_mask_repair_candidate,
            capture=capture_cpu_mask_failure,
            failure_key="cpuMaskFailure",
        ),
        **kwargs,
    )
