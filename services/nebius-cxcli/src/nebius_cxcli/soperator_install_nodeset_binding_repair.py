"""Shared sealed admission for exact native NodeSet binding repairs."""
...
def prepare_install_nodeset_binding_repair(
    *,
    ...
    repair_path = paths.reports_dir / f"soperator-install-{binding.slug}-repair-{target_ref}.json"
    existing = _files(paths.flux_dir)
    if repair_path.exists():
        saved = read_owner_only_json(repair_path, label="Soperator NodeSet binding repair")
        if (
            not isinstance(saved, Mapping)
            or saved.get("schema") != binding.reason
            or saved.get("targetRef") != target_ref
            or saved.get("replacementFiles") != _file_hashes(existing)
        ):
            raise RuntimeError("saved NodeSet binding repair lost generated authority")
    ...
    if (
        ...
        or slurm is None
        or binding.candidate(candidate, inverse=True) != existing
    ):
        raise RuntimeError(
            "NodeSet binding repair requires exact initial scheduling recovery and runtime delta"
        )
    previous_sha = scheduling_journal["operationSpecSha256"]
    predecessors = [
        row
        for p in paths.reports_dir.glob("soperator-release-reconcile-*.json")
        if isinstance(row := read_owner_only_json(p, label="NodeSet binding predecessor"), Mapping)
        and _digest(row.get("operation", {}).get("spec")) == previous_sha
    ]
    if len(predecessors) != 1:
        raise RuntimeError("NodeSet binding repair requires one exact predecessor")
    ...
    if not check_path.exists():
        raise RuntimeError("NodeSet binding predecessor checks are missing")
    ...
    failure = binding.capture(runner, read_worker)
    ...
    for node in gpu_nodesets(values["nodesets"]["overrideValues"]):
        ...
        if (
            ...
            raise RuntimeError("NodeSet binding repair lost its native NodeSet binding")
    ...
    if any(
        ...
        raise RuntimeError("NodeSet binding failure is outside the admitted NodeSets")
    ...
    if not expected_claims or actual_claims != expected_claims:
        raise RuntimeError("NodeSet binding repair lost approved storage bindings")
    child = _kube_get(
    ...
    if (
        ...
        raise RuntimeError("NodeSet binding repair lost frozen native chart authority")
    if (
        read_owner_only_json(check_path, label="NodeSet binding predecessor checks") != state
        or binding.capture(runner, read_worker) != failure
    ):
        raise RuntimeError("NodeSet binding failure changed during admission")
