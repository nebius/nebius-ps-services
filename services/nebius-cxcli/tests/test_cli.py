...
def _stub_provider_generated_kubeconfig_handoff(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def _registration_identity_payload() -> dict[str, object]:
    return {
        "client_info": {
            "client_name": "client-a",
            "nebius": {
...
def test_create_writes_deployments_gitignore_when_target_is_in_git_repo(tmp_path: Path) -> None:
    ...
    assert "*/*/generated/ssh_known_hosts" in content
...
def test_all_infra_components_can_allocate_additional_instances() -> None:
    for entry in component_entries("infra"):
        ...
        if name_input:
            assert soperator_config._mapping_path_value(row.get("inputs", {}), name_input) == (
                f"{entry.id}-2"
            )
...
def test_component_remove_rejects_all_soperator_lifecycle_state_before_auth_or_config_writes(
    state: str,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    config_path = _project_config_path(deployments_root)
    payload = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if state == "disabled-app":
        payload.setdefault("apps", {}).setdefault("charts", []).append(
            {
                "id": "soperator",
                "instance_id": "mk8s",
                "enabled": False,
    ...
    monkeypatch.setattr(
        cli_module,
        "_load_context",
        lambda *_args, **_kwargs: pytest.fail("component remove reached context loading"),
    )
    monkeypatch.setattr(
        cli_module,
        "_ensure_project_auth_identity",
        lambda *_args, **_kwargs: pytest.fail("component remove reached authentication"),
    )
...
def test_bootstrap_ci_accepts_github_flags(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = tmp_path / "customer-repo"
    repo_root.mkdir(parents=True, exist_ok=True)
    _git_init(repo_root)
