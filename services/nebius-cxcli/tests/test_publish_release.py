...
RUNTIME_VERSION_SOURCE = (
    Path(__file__).resolve().parents[1] / "src" / "nebius_cxcli" / "runtime_version.py"
)
...
def test_publish_release_prep_fails_when_untracked_files_are_present(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir(parents=True, exist_ok=True)
    ...
    (repo_root / "scratch.txt").write_text("untracked\n", encoding="utf-8")
...
def test_publish_release_prep_fails_when_remote_tag_already_exists(
    ...
    _run(["git", "push", "-u", "origin", "main"], cwd=repo_root)
    _run(
        ["git", "tag", "-a", "nebius-cxcli-v0.1.3", "-m", "Release nebius-cxcli-v0.1.3"],
        cwd=repo_root,
    )
...
def test_publish_release_publish_fails_when_release_section_is_empty(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir(parents=True, exist_ok=True)
    ...
    assert result.returncode != 0
    assert (
        "Release changelog section for nebius-cxcli-v0.1.3 in CHANGELOG.md is empty"
        in result.stderr
    )
    remote_tags = subprocess.run(
        ["git", "ls-remote", "--tags", "origin", "refs/tags/nebius-cxcli-v0.1.3"],
...
def test_publish_release_publish_uses_git_version_when_setuptools_scm_is_unavailable(
    ...
    (repo_root / "setuptools_scm.py").write_text(
