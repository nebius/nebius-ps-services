...
from nebius_cxcli.soperator_install_docker_repair import (
    docker_repair_candidate,
    docker_reservation_handoff,
)
...
def test_docker_candidate_changes_only_exact_native_runtime_bindings():
    before, values = compiled()
    after = docker_repair_candidate(before)
    assert {name for name in after if after[name] != before[name]} == {VALUES_FILE, OUTER_FILE}
    assert docker_repair_candidate(after) == after
    assert docker_repair_candidate(after, inverse=True) == before
    expected = copy.deepcopy(values)
    from nebius_cxcli.soperator_worker_docker import materialize_worker_docker

    materialize_worker_docker(expected["nodesets"]["overrideValues"])
...
def test_docker_handoff_proves_exact_delta_and_both_full_values_policies(
    ...
    before = docker_repair_candidate(after, inverse=True)
    ...
    if mutation:
        with pytest.raises(RuntimeError):
            docker_reservation_handoff(repair, paths=paths, policy=target)
    else:
        result = docker_reservation_handoff(repair, paths=paths, policy=target)
