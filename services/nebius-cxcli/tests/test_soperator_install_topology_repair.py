...
from nebius_cxcli.soperator_install_topology_repair import (
    topology_repair_candidate,
...
def compiled():
    ...
    values["nodesets"]["overrideValues"]["nodesets"][0]["nodeConfig"] = {
        "static": "Boards=1 SocketsPerBoard=1 CoresPerSocket=32 ThreadsPerCore=1 Gres=gpu:8"
    }
...
def test_topology_candidate_changes_only_exact_native_runtime_bindings():
    before, values = compiled()
    after = topology_repair_candidate(before)
    assert {name for name in after if after[name] != before[name]} == {VALUES_FILE, OUTER_FILE}
    assert topology_repair_candidate(after) == after
    assert topology_repair_candidate(after, inverse=True) == before
    expected = copy.deepcopy(values)
    from nebius_cxcli.soperator_worker_topology import h200_static

    expected["nodesets"]["overrideValues"]["nodesets"][0]["nodeConfig"]["static"] = h200_static(
        32000
    )
...
def test_topology_handoff_proves_exact_delta_and_both_full_values_policies(
    ...
    before = topology_repair_candidate(after, inverse=True)
    ...
    if mutation:
        with pytest.raises(RuntimeError):
            topology_reservation_handoff(repair, paths=paths, policy=target)
    else:
        result = topology_reservation_handoff(repair, paths=paths, policy=target)
