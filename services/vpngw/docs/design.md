...
- Boundaries and interfaces: `cli.py` parses and preflights the configured exact known-host pins before service-account, `ensure_group`, or provisioning effects and passes one protected validated trust snapshot to every remote operation. During HA removal it acquires the operator token only for read-only cloud access, completes change analysis and operator confirmation, then uses durable shared-allocation evidence rather than the new instance count to discover both former members, identity-rechecks and terminally deactivates both, and only afterward creates or switches to a requested service account or enters provisioning. `deploy/vm_manager.py` owns read-only member discovery and exact Compute snapshot revalidation. `deploy/ssh_push.py` stops HA state on every former member and additionally disables and verifies every product mutation service on a retired member; all OpenSSH and Paramiko sessions consume the same trust snapshot, reject host-key mismatch distinctly from reachability failure, and expose no trust-on-first-use or disabled-verification branch. `agent/main.py` constructs the concrete `RecoverableController`, snapshot port, and idempotent effect port; `routing_guard.py`, `fix_routes.py`, the ordinary agent reconciliation path, route timers, and all systemd assets share one controller-ready forwarding gate; `vpngw_sa.py` accepts only a reviewed action-to-role allowlist. `config_loader.py`, `deploy/vm_ha_cloud.py`, `agent/vm_ha/transport.py`, `deploy/vm_ha_routes.py`, and `deploy/route_manager.py` own the remaining authoritative lower-level contracts. Non-HA commands remain on the existing canonical path after any stale HA state is safely removed.
- Validation: Run CLI, missing/malformed/unusable trust preflight, retained-member identity and management-key ordering, exact-pin mismatch, immutable trust-snapshot, OpenSSH/Paramiko parity, critical-SSH-failure, cloud-init enrollment-anchor, remote-staging replacement, service readiness/failure, guard-bypass, clean-bootstrap, cold-boot, automatic-recovery, HA-to-non-HA deactivation, IAM allowlist, status/recovery/failback, packaging, build, and release tests plus a non-HA golden trace and offline two-node apply/status/recovery trace.
...
- Removing VM HA discovers both former members from durable HA evidence, leaves them untouched on rejected change/confirmation paths, strictly rechecks their identities, deactivates and verifies both before ordinary authentication or provisioning, and disables the agent, health monitor, route-fix timer/service, IPsec, and FRR on a retired member. The reviewed IAM mapping remains action-derived rather than accepting caller-selected broad roles.
