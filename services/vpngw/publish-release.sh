...
show_usage() {
  ...
  printf '%b\n' "  ${S_CYAN}./publish-release.sh${S_RESET} ${S_DIM}--publish X.Y.Z [--allow-non-main]${S_RESET}"
  printf '\n'
  ...
  printf '%b\n' "  ${S_CYAN}./publish-release.sh --prep 0.5.2${S_RESET}"
  printf '%b\n' "  ${S_CYAN}./publish-release.sh --publish 0.5.2${S_RESET}"
  printf '%b\n' "  ${S_CYAN}./publish-release.sh --publish ${TAG_PREFIX}-v0.5.2${S_RESET}"
}
...
from nebius_vpngw import __version__
...
create_and_push_tag() {
  ...
  log_success "GitHub Actions workflow vpngw-release.yml will build, test, and publish the release."
}
