...
class FRRRenderer:
    def render_and_apply(self, cfg: Dict[str, Any]) -> None:
        ...
        and advertises `gateway.local_prefixes`. Supports hold/keepalive and graceful-restart defaults.
        """
        BGPD_CONF.parent.mkdir(parents=True, exist_ok=True)

        gateway = cfg.get("gateway", {})
        local_asn = gateway.get("local_asn", 65010)
        local_prefixes: List[str] = gateway.get("local_prefixes", [])
        ...
        for conn in cfg.get("connections", []):
            ...
            if routing_mode != "bgp":
                continue
            remote_asn = (conn.get("bgp", {}) or {}).get("remote_asn")
        ...
        # Advertise local prefixes
        for pfx in local_prefixes:
            lines.append(f" network {pfx}")

        BGPD_CONF.write_text("\n".join(lines) + "\n", encoding="utf-8")
        print("[FRR] Wrote bgpd.conf")
