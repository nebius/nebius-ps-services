...
from .vm_ha_checkpoint import (
    controller_checkpoint_from_dict as controller_checkpoint_from_dict,
)
from .vm_ha_checkpoint import (
    controller_checkpoint_to_dict as controller_checkpoint_to_dict,
)
...
def _install_vm_ha_cold_start_guard_locked(*, state_dir: Path, boot_id: str) -> dict[str, Any]:
    from ..ha_repair import cold_guard
        ...
        *,
        ...
        try:
            ...
            if vm_ha_config is None:
                ...
                from .local_commands import CURRENT, CommandBudget
                from .ordinary import reconcile_locked

                if lock_fd is None:
                    raise RuntimeError("ordinary reconciliation requires the routing lock")
                try:
                    require_idle()
                except OperationBlocked:
                    if force_reconcile:
                        raise
                    print("[Agent] Ordinary operation pending; waiting without reconciliation")
                    return
                token = CURRENT.set(CommandBudget(time.monotonic() + 300))
                try:
                    outcome = reconcile_locked(
                        cfg, raw_config, verify_only=False, force=force_reconcile
                    )
                    print(f"[Agent] Ordinary configuration {outcome} and verified")
                finally:
                    CURRENT.reset(token)
                return
...
def main() -> None:
    ...
    group.add_argument("--ordinary-apply", metavar="BASE64_JSON", help=argparse.SUPPRESS)
    ...
    if args.ordinary_apply:
        from .ordinary import action

        raise SystemExit(action(args.ordinary_apply))
