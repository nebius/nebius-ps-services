...
def _enforce_routing_sysctls() -> Tuple[int, List[str]]:
    ...
    for key, expected_value in REQUIRED_SYSCTLS.items():
        ...
        if result.returncode != 0:
            logger.warning(
                f"[RoutingGuard] Could not read sysctl {key}: {result.stderr.strip()}"
            )
            continue
        ...
        if current_value != expected_value:
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"[RoutingGuard] Failed to set {key}: {result.stderr.strip()}"
                )

    # Also enforce rp_filter=0 on all XFRM interfaces
    result = subprocess.run(
        ["ip", "link", "show", "type", "xfrm"], capture_output=True, text=True
    )
...
def enforce_routing_invariants(cfg: Dict[str, Any]) -> None:
    ...
    # INVARIANT 4: Clean up unexpected APIPA routes (declarative management)
    ...
    # INVARIANT 5: Ensure /32 routes for BGP peers
...
def _remove_table_220() -> bool:
    ...
    result = subprocess.run(
        ["ip", "rule", "del", "pref", "220"], capture_output=True, text=True
    )
    if result.returncode == 0:
    ...
    if "220" in result.stdout:
        print(
            "[RoutingGuard] ⚠ WARNING: Table 220 rule still present after removal attempt"
        )
...
def _remove_broad_apipa_route() -> bool:
    ...
    if result.stdout.strip():
        ...
        if result.returncode == 0:
            print(
                f"[RoutingGuard] Removed orphan APIPA route: 169.254.0.0/16 (was: {route_info})"
            )
            return True
        else:
...
def _remove_scope_link_local_prefixes(cfg: Dict[str, Any]) -> int:
    ...
    for prefix in local_prefixes:
        ...
        result = subprocess.run(
            ["ip", "route", "show", prefix], capture_output=True, text=True
        )
        ...
        if "scope link" in route_info:
            ...
            result = subprocess.run(
                ["ip", "route", "del", prefix], capture_output=True, text=True
            )
            if result.returncode == 0:
                print(
    ...
    if removed_count > 0:
        print(
            f"[RoutingGuard] ✓ Removed {removed_count} scope link route(s) for local prefixes"
        )
...
def _cleanup_unexpected_apipa_routes(cfg: Dict[str, Any]) -> int:
    ...
    for line in result.stdout.strip().split("\n"):
        ...
        is_expected = (
            prefix in expected_tunnel_cidrs
            and expected_tunnel_cidrs[prefix] == dev_interface
        ) or (
            prefix in expected_tunnel_peers
            and expected_tunnel_peers[prefix] == dev_interface
        )
...
def _ensure_bgp_peer_routes(cfg: Dict[str, Any]) -> int:
    ...
    for idx, iface_name, conn, tun in iter_active_tunnels(cfg):
        ...
        if not inner_remote_ip:
            print(
                f"[RoutingGuard] Tunnel {tunnel_name} missing inner_remote_ip, skipping"
            )
            continue
