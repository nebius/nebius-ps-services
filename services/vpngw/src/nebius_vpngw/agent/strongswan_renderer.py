...
"""
        ...
        # CRITICAL: Protect default route and local VPC prefixes from VTI capture
        # With 0.0.0.0/0 selectors, we must ensure local/default routes never use VTI
        self._protect_critical_routes(cfg, vti_endpoints)

    def _protect_critical_routes(self, cfg: Dict[str, Any], vti_endpoints: List[dict]) -> None:
        """Ensure default route and local VPC prefixes never use VTI interfaces.
        
        This prevents routing loops when using 0.0.0.0/0 traffic selectors.
        Also removes problematic routing rules and broad APIPA routes.
        """
        # CRITICAL FIX #1: Remove routing policy rule for table 220 if it exists
        # This table can override main table and send APIPA traffic to eth0
        ...
        if result.stdout.strip() and "eth0" in result.stdout:
            print("[StrongSwan] FOUND broad 169.254.0.0/16 route via eth0 - removing to allow VTI routes")
            print(f"[StrongSwan] Current route: {result.stdout.strip()}")
            subprocess.run(["ip", "route", "del", "169.254.0.0/16"], check=False)
            # Verify deletion
            result = subprocess.run(["ip", "route", "show", "169.254.0.0/16"], capture_output=True, text=True)
            if not result.stdout.strip():
                print("[StrongSwan] ✓ Successfully removed broad 169.254.0.0/16 route")
            else:
                print(f"[StrongSwan] ⚠ Route still exists: {result.stdout.strip()}")
        
        # Get local prefixes from gateway config
        gateway = cfg.get("gateway", {})
        local_prefixes = gateway.get("local_prefixes", [])
        ...
        # Extract VTI interface names
        vti_names = [vti["name"] for vti in vti_endpoints]
        
        # Check if default route uses a VTI (BAD!)
        for vti_name in vti_names:
            if vti_name in default_route:
                print(f"[StrongSwan] ERROR: Default route uses VTI {vti_name}! This will cause routing loops.")
                print(f"[StrongSwan] Current default route: {default_route}")
                # Try to fix by getting the gateway from eth0
                gw_result = subprocess.run(
                    ["ip", "route", "get", "8.8.8.8"],
                    capture_output=True,
                    text=True,
                )
                if "via" in gw_result.stdout:
                    # Extract gateway IP
                    import re
                    match = re.search(r'via\s+(\S+)', gw_result.stdout)
                    if match:
                        gw_ip = match.group(1)
                        print(f"[StrongSwan] Fixing default route to use gateway {gw_ip} via eth0")
                        subprocess.run(
                            ["ip", "route", "replace", "default", "via", gw_ip, "dev", "eth0"],
                            check=False,
                        )
        
        # Check local VPC prefixes
        for local_prefix in local_prefixes:
            result = subprocess.run(
                ["ip", "route", "show", local_prefix],
                capture_output=True,
                text=True,
            )
            local_route = result.stdout.strip()
            
            for vti_name in vti_names:
                if vti_name in local_route:
                    print(f"[StrongSwan] ERROR: Local prefix {local_prefix} route uses VTI {vti_name}!")
                    print(f"[StrongSwan] Current route: {local_route}")
                    print(f"[StrongSwan] Fixing to use eth0")
                    subprocess.run(
                        ["ip", "route", "replace", local_prefix, "dev", "eth0"],
                        check=False,
                    )
