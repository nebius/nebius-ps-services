...
class VMManager:
    ...
    def _build_cloud_init(self, ssh_key: t.Optional[str] = None) -> str:
        ...
        cloud = (
            ...
            "            \n"
            "            PUBLIC_IF=\"eth0\"\n"
            "            PUBLIC_IP=\"$(curl -s -m 5 http://169.254.169.254/latest/meta-data/public-ipv4 2>/dev/null || hostname -I | awk '{print $1}')\"\n"
            "            \n"
            ...
            "            fi\n"
            "            \n"
            "            # CRITICAL: Do NOT filter VTI interfaces - BGP runs on them\n"
            ...
            "            \n"
            "            # Security: no ICMP redirects (gateway should not redirect traffic)\n"
            ...
            "            \n"
            "            # Reverse path filtering: use loose mode (2) for VTI interfaces\n"
            "            # Mode 0 = disabled (no validation) - dangerous\n"
            "            # Mode 1 = strict (default, checks exact incoming interface) - breaks VTI\n"
            "            # Mode 2 = loose (allows any interface with valid route back) - works with VTI\n"
            "            net.ipv4.conf.default.rp_filter=2\n"
            "            # Keep eth0 in strict mode for internet-facing security\n"
            "            net.ipv4.conf.eth0.rp_filter=1\n"
            "            \n"
            "            # Security: ignore bogus ICMP errors\n"
            "            net.ipv4.icmp_ignore_bogus_error_responses=1\n"
            "            \n"
            "            # Security: log martian packets (packets with impossible addresses)\n"
            ...
            "            \n"
            "            # TCP hardening: SYN cookies for SYN flood protection\n"
            ...
            "  # Setup firewall (initial - will be updated by agent with actual peer IPs)\n"
