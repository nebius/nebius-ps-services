...
def parse(text: str) -> dict:
    ...
    remote_ips = re.findall(
        r"(?i)virtual\s*private\s*gateway\s*inside\s*address\s*[:=]\s*(169\.254\.\d+\.\d+)",
        text,
    )

    # Public endpoints
    local_pub = re.findall(
        r"(?i)customer\s*gateway.*?(?:outside|public)\s*ip\s*address\s*[:=]\s*([0-9\.]+)",
        text,
    )
    remote_pub = re.findall(
        r"(?i)virtual\s*private\s*gateway.*?(?:outside|public)\s*ip\s*address\s*[:=]\s*([0-9\.]+)",
        text,
    )

    # Basic crypto proposals when present
    ike_props = re.findall(
        r"(?i)ike\s*encryption\s*[:=]\s*([A-Za-z0-9\-]+).*?ike\s*integrity\s*[:=]\s*([A-Za-z0-9\-]+).*?dh\s*group\s*[:=]\s*(\d+)",
        text,
        re.S,
    )
    esp_props = re.findall(
        r"(?i)esp\s*encryption\s*[:=]\s*([A-Za-z0-9\-]+).*?esp\s*integrity\s*[:=]\s*([A-Za-z0-9\-]+).*?pfs\s*group\s*[:=]\s*(\d+)",
        text,
        re.S,
    )
    ...
    return {
        "tunnels": [t for t in tunnels if any(v for v in t.values())],
