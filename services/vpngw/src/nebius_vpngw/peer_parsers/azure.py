...
def parse(text: str) -> dict:
    ...
    local_pub = re.findall(
        r"(?i)(customer|local).*?(?:public|gateway)\s*ip\s*address\s*['\"]?([0-9\.]+)",
        text,
    )
    remote_pub = re.findall(
        r"(?i)(azure|vpn\s*gateway).*?(?:public|gateway)\s*ip\s*address\s*['\"]?([0-9\.]+)",
        text,
    )

    ike = re.findall(
        r"(?i)ike.*?encryption\s*[:=]\s*([A-Za-z0-9\-]+).*?integrity\s*[:=]\s*([A-Za-z0-9\-]+).*?dh\s*group\s*[:=]\s*(\d+)",
        text,
        re.S,
    )
    esp = re.findall(
        r"(?i)ipsec|esp.*?encryption\s*[:=]\s*([A-Za-z0-9\-]+).*?integrity\s*[:=]\s*([A-Za-z0-9\-]+)",
        text,
        re.S,
    )
