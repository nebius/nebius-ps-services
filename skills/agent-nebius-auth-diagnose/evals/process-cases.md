...
Fresh routing checks use CSV rows `auth-diagnose-positive-01` through
`auth-diagnose-positive-06` and `auth-diagnose-negative-01` through
`auth-diagnose-negative-05`. The table below assumes the skill has already
been selected and preserves only workflow-state expectations.
