...
Fresh routing checks use CSV rows `auth-setup-positive-01` through
`auth-setup-positive-05` and `auth-setup-negative-01` through
`auth-setup-negative-04`. The table below assumes the skill has already been
selected and preserves only workflow-state expectations.
