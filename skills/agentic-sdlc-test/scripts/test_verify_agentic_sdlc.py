...
class VerifierContractTests(unittest.TestCase):
    ...
    def write_manifest(
        ...
        *,
        ...
        lane_names = verifier.LIVE_LANES if all_lanes else ("golden-path",)
        lanes = {}
        for lane in lane_names:
            ...
            lanes[lane] = {"status": "PASS", "evidence": [relative.as_posix()]}
        value = {
            "schema": verifier.LIVE_RESULTS_SCHEMA,
            "verification_id": verification_id
            or verifier.verification_id(self.ctx, baseline),
            "project_root": str(self.selected),
            "baseline_head": baseline,
            "final_head": final_head or git(self.project, "rev-parse", "HEAD"),
            "lanes": lanes,
        }
    ...
    def test_valid_live_evidence_is_accepted(self) -> None:
        ...
        self.assertEqual(lanes["golden-path"]["status"], "PASS")
        integrity = [
            check
            for check in self.ctx.checks
    ...
    def test_complete_live_evidence_can_reach_pass(self) -> None:
        self.commit_selected_change()
        self.write_manifest(all_lanes=True)
        verifier.add_agent_required_sections(self.ctx)
        self.assertEqual(verifier.final_status(self.ctx), "PASS")
    ...
    def test_live_results_schema_lane_paths_match_runtime(self) -> None:
        ...
        for lane in verifier.LIVE_LANES:
            definition = schema["$defs"][f"{lane}-lane"]
            pattern = definition["properties"]["evidence"]["items"]["pattern"]
            self.assertEqual(pattern, verifier.lane_evidence_pattern(lane))
            self.assertTrue(
