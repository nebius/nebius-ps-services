...
def recommended_fix(source: object, issue: object) -> str:
    ...
    if "environment_defect" in combined and (
        "computer use" in combined or "computer-use" in combined
    ):
        return (
...
def report_text(state: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Remaining resources: {len(cleanup.get('remaining', []))}",
            f"- Project retention state: {retention_state}",
            "- Destroy invocation: "
            f"`$agentic-sdlc-test --verification-root {markdown_escape(verification_root)} --destroy`",
            "",
