...
For `$apply-security --help` or `$apply-security -h` (including native Claude forms), return concise help and stop before
...
Use `$apply-security` in Codex, `/apply-security` in Claude Code, or
`/skills:apply-security` in the Claude plugin. Dollar-prefixed skill examples
