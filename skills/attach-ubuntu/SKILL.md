...
For `$attach-ubuntu --help` or `$attach-ubuntu -h` (including native Claude forms), return concise help and stop before
...
Use `$attach-ubuntu` in Codex, `/attach-ubuntu` in Claude Code, or
`/skills:attach-ubuntu` in the Claude plugin. Dollar-prefixed skill examples
