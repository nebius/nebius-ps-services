...
For `$brainstorm --help` or `$brainstorm -h` (including native Claude forms), return concise help and stop before
...
Use `$brainstorm` in Codex, `/brainstorm` in Claude Code, or
`/skills:brainstorm` in the Claude plugin. Dollar-prefixed skill examples
