...
For `$code-info --help` or `$code-info -h` (including native Claude forms), return concise help and stop before
...
Use `$code-info` in Codex, `/code-info` in Claude Code, or
`/skills:code-info` in the Claude plugin. Dollar-prefixed skill examples
