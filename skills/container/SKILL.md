...
For `$container --help` or `$container -h` (including native Claude forms), return concise help and stop before
...
Use `$container` in Codex, `/container` in Claude Code, or
`/skills:container` in the Claude plugin. Dollar-prefixed skill examples
