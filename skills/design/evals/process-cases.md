...
- Approved application or AI stack layers must not be reopened; only genuinely
  undecided layers route through `app-stack` or `ai-stack`.
- AI-enabled capabilities must be classified as direct calls, deterministic
  workflows, or agents; unknown-count code-owned loops remain deterministic,
  while durability is evaluated independently from agentic control flow.
...
`design-positive-14` and `design-negative-01` through `design-negative-10` in a
