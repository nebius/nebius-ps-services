...
For `$gitignore --help` or `$gitignore -h` (including native Claude forms), return concise help and stop before
...
Use `$gitignore` in Codex, `/gitignore` in Claude Code, or
`/skills:gitignore` in the Claude plugin. Dollar-prefixed skill examples
