#!/usr/bin/env python3
"""Read-only checks for flat catalog discovery and native hook wiring."""
...
_load_skill_support('projector', __file__, 'global-context-management/scripts/check-plugin-manifests.py', source_only=True)

from hook_runtime import HOOKS, OWNERS, payload_files  # noqa: E402 — verified bootstrap precedes runtime imports


def check(root: Path, catalogs: bool = False) -> None:
    expected = ["./" + path.name for path in sorted(root.iterdir())
                if path.is_dir() and not path.name.startswith(".") and (path / "SKILL.md").is_file()]
    files = payload_files(root)
    for agent, variable in (("codex", "PLUGIN_ROOT"), ("claude", "CLAUDE_PLUGIN_ROOT")):
        value = json.loads((root / f".{agent}-plugin/plugin.json").read_text())
        if value.get("name") != "skills" or value.get("version") != "0.1.0":
            raise ValueError("native plugin identity/version mismatch")
        if value.get("skills") != expected:
            raise ValueError("native manifest must enumerate exactly the immediate skill catalog")
        for relative in value["skills"]:
            if not (root / relative).resolve().is_relative_to(root):
                raise ValueError("skill path escapes the package")
        expected_hooks: dict[str, list[dict]] = {}
        for owner in OWNERS:
            source = json.loads((root / owner / "assets/hooks.json.template").read_text())
            for event, entries in source["hooks"].items():
                for entry in entries:
                    for handler in entry["hooks"]:
                        names = re.findall(r"([A-Za-z0-9_-]+\.py)", handler["command"])
                        if len(names) != 1 or names[0] not in HOOKS or names[0] not in files:
                            raise ValueError("unreviewed or missing hook entrypoint")
                        handler["command"] = (
                            f'python3 "${{{variable}}}/global-context-management/scripts/hook_runtime.py" '
                            f'--agent {agent} --hook {names[0]} --plugin-root "${{{variable}}}"'
                        )
                        if agent == "claude":
                            handler.pop("statusMessage", None)
                    if entry not in expected_hooks.setdefault(event, []):
                        expected_hooks[event].append(entry)
        expected_config = expected_hooks if agent == "claude" else {"hooks": expected_hooks}
        if value.get("hooks") != expected_config:
            raise ValueError("native hooks differ from owner manifests")
        if len(expected_hooks.get("Stop", [])) != 1:
            raise ValueError("exactly one Stop arbiter must be registered")
    if catalogs:
        for path, source in ((root.parent / ".agents/plugins/marketplace.json",
                              {"source": "local", "path": "./skills"}),
                             (root.parent / ".claude-plugin/marketplace.json", "./skills")):
            catalog = json.loads(path.read_text())
            if catalog.get("name") != "nebius-ps-services" or len(catalog.get("plugins", [])) != 1:
                raise ValueError("invalid marketplace identity or plugin count")
            plugin = catalog["plugins"][0]
            if plugin.get("name") != "skills" or plugin.get("source") != source:
                raise ValueError("marketplace does not reference the flat skills package")
...
def main() -> int:
    ...
    try:
        check(args.root.resolve(strict=True), args.catalogs)
    except (OSError, ValueError, KeyError, TypeError):
        print("FAIL: invalid native plugin/catalog or hook payload wiring")
        return 1
    print("STATIC_PASS: both native manifests and selected catalogs")
    return 0
