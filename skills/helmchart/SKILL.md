...
For `$helmchart --help` or `$helmchart -h` (including native Claude forms), return concise help and stop before
...
Use `$helmchart` in Codex, `/helmchart` in Claude Code, or
`/skills:helmchart` in the Claude plugin. Dollar-prefixed skill examples
