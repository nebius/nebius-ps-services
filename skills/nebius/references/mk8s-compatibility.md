...
- Rank supported preset and fabric choices with live Capacity Dashboard data when that surface is available.
- Distinguish single-GPU Ethernet-only test shapes from multi-GPU InfiniBand-capable training shapes.
...
4. Query the live compute platform preset inventory before deciding whether GPU clustering is valid. The public Compute docs list the currently cluster-compatible presets, but project support still needs to be resolved live:
...
5. Treat GPU clustering as a preset-level capability, not a platform-level assumption.
   ...
   - For current Nebius GPU-cluster flows, this usually means the supported 8-GPU preset on the selected platform and region.
   - Single-GPU presets are Ethernet-only and should be treated as testing/dev shapes, not InfiniBand or GPUDirect-RDMA production training shapes.

6. If Capacity Dashboard `capacity.v1.ResourceAdvice` is available, use it as ranking enrichment after the compatibility and preset checks succeed.
   - Query it by tenant, region, platform, and preset.
   - Use it to rank which supported preset or fabric currently has on-demand or reserved availability.
   - Use a recommended fabric only for presets that are already confirmed clusterable.
   - If ResourceAdvice returns fabric-scoped rows for a non-clusterable or 1-GPU preset, treat that as availability metadata only, not as permission to set `infiniband_fabric`.

7. Prefer the exact `drivers_preset` string returned by the matrix.
   - Good: `cuda13.0`
   - Avoid: `cuda13`
   - Avoid: `latest`

8. If the matrix yields exactly one valid `drivers_preset`, treat that as the safe default.
...
9. If the matrix yields multiple valid presets, present those exact options to the operator instead of guessing.

10. If the matrix yields no preset for the selected platform/version, treat that as a compatibility gap to resolve before apply.
...
- Source of truth for supported GPU-cluster presets in public docs: Nebius Compute "Presets compatible with GPU clusters", but still re-check the live project inventory before acting.
...
- Do not present single-GPU presets as valid InfiniBand or NCCL-performance shapes; they are Ethernet-only.
- Capacity Dashboard is useful for ranking what is available now, not for overriding compatibility rules.
