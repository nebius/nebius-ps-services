...
For `$optimize-pytest --help` or `$optimize-pytest -h` (including native Claude forms), return concise help and stop before
...
Use `$optimize-pytest` in Codex, `/optimize-pytest` in Claude Code, or
`/skills:optimize-pytest` in the Claude plugin. Dollar-prefixed skill examples
