...
For `$python-project --help` or `$python-project -h` (including native Claude forms), return concise help and stop before
...
Use `$python-project` in Codex, `/python-project` in Claude Code, or
`/skills:python-project` in the Claude plugin. Dollar-prefixed skill examples
