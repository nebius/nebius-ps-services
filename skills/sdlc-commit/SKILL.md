...
For `$sdlc-commit --help` or `$sdlc-commit -h` (including native Claude forms), return concise help and stop before
...
Use `$sdlc-commit` in Codex, `/sdlc-commit` in Claude Code, or
`/skills:sdlc-commit` in the Claude plugin. Dollar-prefixed skill examples
