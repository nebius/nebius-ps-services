...
For `$sdlc-evaluate --help` or `$sdlc-evaluate -h` (including native Claude forms), return concise help and stop before
...
Use `$sdlc-evaluate` in Codex, `/sdlc-evaluate` in Claude Code, or
`/skills:sdlc-evaluate` in the Claude plugin. Dollar-prefixed skill examples
