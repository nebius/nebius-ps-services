...
description: "Use only as part of the Agentic SDLC workflow; gather technical, product, vendor, internal, codebase, layer, and test evidence into a compact feature-specific context pack before design or implementation."
...
For `$sdlc-gather-context --help` or `$sdlc-gather-context -h` (including native Claude forms), return concise help and stop before
...
Use `$sdlc-gather-context` in Codex, `/sdlc-gather-context` in Claude Code, or
`/skills:sdlc-gather-context` in the Claude plugin. Dollar-prefixed skill examples
