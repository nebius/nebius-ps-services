...
For `$sdlc-gui-test --help` or `$sdlc-gui-test -h` (including native Claude forms), return concise help and stop before
...
Use `$sdlc-gui-test` in Codex, `/sdlc-gui-test` in Claude Code, or
`/skills:sdlc-gui-test` in the Claude plugin. Dollar-prefixed skill examples
