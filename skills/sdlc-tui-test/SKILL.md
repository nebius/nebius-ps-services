...
For `$sdlc-tui-test --help` or `$sdlc-tui-test -h` (including native Claude forms), return concise help and stop before
...
Use `$sdlc-tui-test` in Codex, `/sdlc-tui-test` in Claude Code, or
`/skills:sdlc-tui-test` in the Claude plugin. Dollar-prefixed skill examples
