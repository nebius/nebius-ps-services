...
For `$sdlc-update-documents --help` or `$sdlc-update-documents -h` (including native Claude forms), return concise help and stop before
...
Use `$sdlc-update-documents` in Codex, `/sdlc-update-documents` in Claude Code, or
`/skills:sdlc-update-documents` in the Claude plugin. Dollar-prefixed skill examples
