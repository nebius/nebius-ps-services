...
def validate_state(root: Path, state: dict[str, Any]) -> None:
    ...
    reject_symlinks(expected_run / "private" / (agent_name() + "-home"), stop_at=root)
    if not (expected_run / "private" / (agent_name() + "-home")).is_dir():
        raise LifecycleError("Active three-tier lifecycle belongs to another agent.")
...
def prepare(root_value: Path) -> dict[str, Any]:
    ...
    with lifecycle_lock(root):
        ...
        for directory in (
            run_root / "project",
            run_root / "private" / (agent_name() + "-home"),
            run_root / "evidence",
