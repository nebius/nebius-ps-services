...
For `$shell-scripting --help` or `$shell-scripting -h` (including native Claude forms), return concise help and stop before
...
Use `$shell-scripting` in Codex, `/shell-scripting` in Claude Code, or
`/skills:shell-scripting` in the Claude plugin. Dollar-prefixed skill examples
