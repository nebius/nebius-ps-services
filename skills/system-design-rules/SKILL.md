...
For `$system-design-rules --help` or `$system-design-rules -h` (including native Claude forms), return concise help and stop before
...
Use `$system-design-rules` in Codex, `/system-design-rules` in Claude Code, or
`/skills:system-design-rules` in the Claude plugin. Dollar-prefixed skill examples
