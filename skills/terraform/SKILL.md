...
For `$terraform --help` or `$terraform -h` (including native Claude forms), return concise help and stop before
...
Use `$terraform` in Codex, `/terraform` in Claude Code, or
`/skills:terraform` in the Claude plugin. Dollar-prefixed skill examples
