...
_load_skill_support('runtime', __file__, 'worktree/scripts/worktree_interop.py')

from worktree_state import (  # noqa: E402 — verified bootstrap precedes runtime imports
    StateError,
...
def _commit_transaction_root(primary: Path) -> Path:
    ...
    try:
        ...
        raise InteropError("shared Git repository identity is unavailable") from error
    try:
        home = agent_home().resolve(strict=False)
    except ValueError as error:
        raise InteropError(str(error)) from error
    repo_key = hashlib.sha256(str(common).encode()).hexdigest()[:24]
    return home / "commit-transactions" / repo_key
